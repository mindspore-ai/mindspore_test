...
if [ "X${mode}" == "Xall" ]; then
  find mindspore/{ccsrc,core,lite} -type f -name "*" | grep -E "\.(h|cc|c)$" > "${CHECK_LIST_FILE}" || true
elif [ "X${mode}" == "Xchanged" ]; then
  # --diff-filter=ACMRTUXB will ignore deleted files in commit
  git diff --diff-filter=ACMRTUXB --name-only | grep "mindspore/ops\|mindspore/ccsrc\|mindspore/core\|mindspore/lite" | grep -E "\.(h|cc|c)$" > "${CHECK_LIST_FILE}" || true
else  # "X${mode}" == "Xlastcommit"
  git diff --diff-filter=ACMRTUXB --name-only HEAD~ HEAD | grep "mindspore/ops\|mindspore/ccsrc\|mindspore/core\|mindspore/lite" | grep -E "\.(h|cc|c)$" > "${CHECK_LIST_FILE}" || true
fi
