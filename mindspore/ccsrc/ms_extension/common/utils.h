...
#define BIND_GRAPH_CUSTOM(func_name)     \
  void fake_func(const ms::Tensor &x) {} \
  PYBIND11_MODULE(MS_EXTENSION_NAME, m) { m.def(func_name, &fake_func, func_name, pybind11::arg("x")); }
}  // namespace ops
}  // namespace mindspore

#define MS_CUSTOM_OP_REGISTER(OpName, OpFuncImplClass, KernelClass) \
  namespace mindspore {                                             \
  namespace ops {                                                   \
