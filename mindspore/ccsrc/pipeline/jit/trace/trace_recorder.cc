...
bool CheckPrimNeededToSkip(const PrimitivePtr &prim) {
  // Some primitives generated by pynative are not fitting graph process and need to be skipped.
  static std::vector<std::string> PrimitivesNeedToSkip{"InplaceIndexPut", "InplaceCopy", "InplaceMul",
                                                       "InplaceAddsExt"};
  return std::find(PrimitivesNeedToSkip.begin(), PrimitivesNeedToSkip.end(), prim->name()) !=
         PrimitivesNeedToSkip.end();
}
