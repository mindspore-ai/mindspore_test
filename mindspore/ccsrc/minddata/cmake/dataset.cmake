...
function(merge_minddata_files SRC_DIR OUT_FILE_FOLDER OUT_FILE_PREFIX EXCLUDE_FILES_PATTERN INPUT_DIR)
    message(STATUS
        "[merge_minddata_files] From ${SRC_DIR} to ${OUT_FILE_FOLDER}, exclude files: ${EXCLUDE_FILES_PATTERN}")
    ...
    if(${RECURSIVE})
        ...
        # Handle empty INPUT_DIR
        if("${INPUT_DIR}" STREQUAL "")
            file(GLOB SRC_LIST ${SRC_DIR}/*.cc)
        else()
            # Verify INPUT_DIR is a subdirectory of SRC_DIR
            get_filename_component(INPUT_DIR_ABS "${INPUT_DIR}" ABSOLUTE BASE_DIR "${SRC_DIR}")
            get_filename_component(SRC_DIR_ABS "${SRC_DIR}" ABSOLUTE)
            string(FIND "${INPUT_DIR_ABS}" "${SRC_DIR_ABS}" IS_SUBDIR)

            if(IS_SUBDIR EQUAL 0 AND EXISTS "${INPUT_DIR_ABS}")
                file(GLOB SRC_LIST ${SRC_DIR}/*.cc ${INPUT_DIR_ABS}/*.cc)
            else()
                message(WARNING "INPUT_DIR ${INPUT_DIR} is not a subdirectory of SRC_DIR ${SRC_DIR}")
                file(GLOB SRC_LIST ${SRC_DIR}/*.cc)
            endif()
        endif()
    endif()
    ...
    foreach(file_path ${SRC_LIST})
        ...
        if(EXISTS ${file_path} AND (NOT IS_DIRECTORY ${file_path}))
            ...
            message(STATUS "[merge_minddata_files] exclude file: ${orig_file_path}")
            continue()
        endif()
    endforeach()
    ...
    foreach(temp_file ${TEMP_FILES})
        ...
        if(${should_copy})
            ...
            message(STATUS "[merge_minddata_files] Updated ${filename} (MD5 changed)")
        endif()
    endforeach()
