...
std::string &GetRecvStateString(const RecvDataState &state) {
  ...
  MS_LOG_EXCEPTION << "Invalue receive data state " << state;
}
}  // namespace
...
bool MbufDataHandler::ReceiveOneDataset(AclDatasetInfoPtr *ds_info_ptr) {
  ...
  if (receive_state_ == RecvDataState::kWaitForSync) {
    ...
    MS_VLOG(VL_PRINT_DUMP_V0) << "Channel " << channel_name_ << " wait data sync to output dev done "
                              << (is_timeout ? "with" : "without") << " timeout.";
    if (is_timeout) {
