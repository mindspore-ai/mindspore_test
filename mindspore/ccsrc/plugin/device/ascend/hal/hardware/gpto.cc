...
bool IsPassDisableForGPTO() {
  const auto &backend_jit_config = backend::BackendJitConfig::ParseBackendJitConfig();
  auto it = backend_jit_config.gpto_options.find("passes");
  if (it != backend_jit_config.gpto_options.end()) {
    if (it->second == "off") {
      MS_LOG(INFO) << "Some backend parallelism passes will be disabled for GPTO";
      return true;
    } else if (it->second == "on") {
      MS_LOG(INFO) << "Some backend parallelism passes will be enabled for GPTO";
      return false;
    }
  }
  MS_LOG(WARNING) << "Invalid or unset value for 'passes' for GPTO. Enabling parallelism passes by default";
  return false;
}
...
std::tuple<size_t, size_t, size_t, size_t> GetBestSolutions(
  ...
  for (auto &taskSort_val : *output) {
    for (auto &pe_solution : taskSort_val.second) {
      ...
      if (((GetGptoOptionsMode() == kBasic && solution.memory_peak > memory_of_min_makespan) ||
           (GetGptoOptionsMode() != kBasic &&
...
bool MemoryViolated(const GptoTaskPtr &selected_task, const Time &start_time, std::map<Time, Memory> *cur_mem_peak_ptr,
                    const size_t &last_switch_subgraph, bool *exists_subgraph, const size_t &task_sort,
                    const size_t &pes_sort) {
...
void InsertEdges(const KernelGraphPtr &kernel_graph, std::unordered_map<CNodePtr, GptoTaskPtr> *cnode_to_task_map_ptr) {
  ...
  std::unordered_map<CNodePtr, std::set<GptoTaskPtr, GptoTask::SortByIdSharedInverse>> real_children;
  ...
  while (!visit_queue.empty()) {
    ...
    if (it != cnode_to_task.end()) {  // if real kernel, then add edges
      ...
      auto &children = real_children[visit_cnode];
      for (auto it_i = children.begin(); it_i != children.end(); it_i++) {
        const auto &child_i = *it_i;
        bool parent_to_be_inserted = false;
        if (!child_i->condition_switch() && !child_i->condition_gather()) {
          for (auto it_j = std::next(it_i); it_j != children.end(); it_j++) {
            const auto &child_j = *it_j;
            if (child_j->children().find(child_i) != child_j->children().end()) {
        ...
        if (!parent_to_be_inserted) {
          visit_task->AddChild(child_i);
          child_i->AddParent(visit_task);
          visit_task->set_child_is_data_dep(child_i, false);  // initially all assumed as control edge
          MS_LOG(DEBUG) << "Edge " << visit_task->id() << " " << child_i->id();
          MS_LOG(DEBUG) << "Edge (UniqueName)" << visit_cnode->UniqueName() << " " << child_i->cnode()->UniqueName();
        }
      }
      children.clear();
      children.insert(visit_task);
    }
...
Memory MemoryLowerBound(const std::vector<GptoTaskPtr> &tasks,
  ...
  std::vector<Memory> task_lb(tasks.size(), static_cast<Memory>(0));
  try {
    std::vector<common::Task> common_tasks;
    size_t start_index = 0;
    size_t end_index = 0;
    const auto &thread_num = common::ThreadPool::GetInstance().GetSyncRunThreadNum();
    size_t step_size = std::ceil(static_cast<float>(tasks.size()) / thread_num);
    while (end_index < tasks.size()) {
      end_index = (end_index + step_size) > tasks.size() ? tasks.size() : end_index + step_size;
      common::Task common_task = [&tasks, &nodes_dependency, &tensors, &task_lb, start_index, end_index]() {
        for (size_t i = start_index; i < end_index; i++) {
          task_lb[i] = CalculateTaskLowerBound(tasks[i], tensors, nodes_dependency);
        }
        return common::SUCCESS;
      };
      (void)common_tasks.emplace_back(common_task);
  ...
  auto max_lb = std::max_element(task_lb.begin(), task_lb.end());
  MS_LOG(INFO) << "Memory Lower bound for tensors " << *max_lb << " (" << *max_lb / kMBToByte << " MB) at task "
               << tasks[std::distance(task_lb.begin(), max_lb)]->id();
  return *max_lb;
}
