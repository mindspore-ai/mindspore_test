...
void GetAddAndShapeNum(const FuncGraphPtr &graph,
                       std::shared_ptr<std::vector<std::pair<AnfNodePtr, int>>> &rms_norm_out0_users, size_t *add_num,
                       size_t *shape_num, AnfNodePtr *shape_node) {
...
inline bool ParameterNotEqual(const std::string &name, const AnfNodePtr &load0, const AnfNodePtr &load1) {
  ...
  if (param_ptr0 == nullptr || param_ptr1 == nullptr) {
    MS_LOG(INFO) << "One of the paramter is nullptr for " << name << ", param_ptr0: " << param_ptr0
                 << ", param_ptr1: " << param_ptr1;
    return true;
  }

  if (!param_ptr0->has_default() || !param_ptr1->has_default()) {
    MS_LOG(INFO) << "One of the paramter does not have default value for " << name
