...
PassManagerPtr GetBackendFusionGroupPassManager() {
  ...
  pm->AddFusionPass(std::make_shared<opt::AddRmsNormDynamicQuantV2Fusion>(), infer_boost);
  ...
  pm->AddFusionPass(std::make_shared<opt::MoeInitRoutingQuantV2Fusion>(), infer_boost);
