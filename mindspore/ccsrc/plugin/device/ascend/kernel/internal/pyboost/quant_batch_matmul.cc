...
namespace mindspore {
namespace kernel {
internal::InternalOpPtr InternalKernelInfoQuantBatchMatmul::CreateKernel(const internal::InputsImmutableInfoList &inputs,
...
void InternalKernelInfoQuantBatchMatmul::Call(const std::shared_ptr<pyboost::OpRunner> &op, const ValuePtrList input_values) {
  ...
  auto op_key = CalcInternalOpApiHash(kernel_name_, inputs, transpose_a_, transpose_b_, has_bias_);
  GetOrCreateKernel(inputs, outputs, op_key);
  LAUNCH_INTERNAL(kernel_name_, op, internal_op_, inputs, outputs, tiling_info_);
}
MS_INTERNAL_KERNEL_INFO_FACTORY_REG(QuantBatchMatmul, internal::kInternalMatMulOpName, InternalKernelInfoQuantBatchMatmul);
}  // namespace kernel
}  // namespace mindspore
