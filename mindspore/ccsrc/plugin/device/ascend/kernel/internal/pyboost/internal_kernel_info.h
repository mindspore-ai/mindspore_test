...
class InternalKernelInfo {
 public:
  ...
  bool Init(const BaseTensorPtrList &input_tensors, std::vector<BaseTensorPtr> *inputs,
            std::vector<BaseTensorPtr> *outputs, const std::vector<BaseTensorPtr> &op_outputs);
  void GetOrCreateKernel(const std::shared_ptr<pyboost::OpRunner> &op, const std::vector<BaseTensorPtr> &inputs,
