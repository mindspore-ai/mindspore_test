...
namespace mindspore {
namespace kernel {
internal::InternalOpPtr InternalKernelInfoMul::CreateKernel(const internal::InputsImmutableInfoList &inputs,
                                                            const internal::OutputsImmutableInfoList &outputs) {
  return internal::CreateMulOp(inputs, outputs, internal::kInternalMulOpName);
}

void InternalKernelInfoMul::Call(const std::shared_ptr<pyboost::OpRunner> &op, const BaseTensorPtr &input_tensor,
                                 const BaseTensorPtr &other_tensor) {
  std::vector<BaseTensorPtr> inputs = {input_tensor, other_tensor};
...
MS_INTERNAL_KERNEL_INFO_FACTORY_REG(Mul, internal::kInternalMulOpName, InternalKernelInfoMul);
}  // namespace kernel
}  // namespace mindspore
