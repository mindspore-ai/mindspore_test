...
void InternalKernelInfoRmsNorm::Call(const std::shared_ptr<pyboost::OpRunner> &op, const BaseTensorPtr &x,
                                     const BaseTensorPtr &gamma, const float &eps) {
  std::vector<BaseTensorPtr> inputs = {x, gamma};
