...
#include "plugin/device/ascend/kernel/rts/to_remote.h"
...
namespace mindspore {
namespace kernel {
bool ToRemoteKernel::Init(const AnfNodePtr &anf_node) {
...
bool ToRemoteKernel::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &,
                            const std::vector<KernelTensor *> &outputs, void *stream_ptr) {
  MS_LOG(ERROR) << "call to_remote kernel start.";
  ...
  return input_device_addr->UpdateDeviceToRemote(device_id_, device_ptr, inputs[0]->size(), stream_ptr, offset, sync);
}
}  // namespace kernel
}  // namespace mindspore
