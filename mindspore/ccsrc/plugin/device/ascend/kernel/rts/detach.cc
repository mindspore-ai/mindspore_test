...
#include "plugin/device/ascend/kernel/rts/detach.h"
...
bool DetachKernel::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &,
  ...
  MS_LOG(ERROR) << "call detach kernel mode start.";
  ...
  return input_device_addr->DetachDevice(device_id_, device_ptr, sync, true);
}
}  // namespace kernel
}  // namespace mindspore
