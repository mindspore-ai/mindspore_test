...
bool HcomMatMulReduceScatterKernel::Init(const std::vector<KernelTensor *> &inputs,
  ...
  if (inputs.size() != kMatMulReduceScatterInputNum) {
    MS_LOG(EXCEPTION) << "Input number of MatMulReduceScatter should be 2, but got " << inputs.size();
  }
...
bool HcomMatMulReduceScatterKernel::Launch(const std::vector<KernelTensor *> &inputs,
  ...
  if ((inputs.size() < kMatMulReduceScatterInputNum) || outputs.empty() || workspace.empty() ||
      hccl_data_type_list_.empty()) {
    MS_LOG(ERROR) << "Invalid ReduceScatter input, output or data type size (" << inputs.size() << ", "
                  << outputs.size() << ", " << workspace.size() << ", " << hccl_data_type_list_.size() << ").";
    return false;
  }
