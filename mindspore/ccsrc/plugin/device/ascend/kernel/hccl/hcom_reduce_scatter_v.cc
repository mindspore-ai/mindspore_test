...
#include "plugin/device/ascend/kernel/hccl/hcom_reduce_scatter_v.h"

namespace mindspore {
namespace kernel {
bool HcomReduceScatterVKernel::Init(const std::vector<KernelTensor *> &inputs,
...
int64_t HcomReduceScatterVKernel::GetReduceScatterVParam(const std::vector<int64_t> &input_split_sizes) {
  ...
  for (size_t i = 0; i < input_split_sizes.size(); i++) {
    ...
    offset += count;
    if (rank_id_ == static_cast<int64_t>(i)) {
      params_.recv_count = input_split_sizes[i];
    }
  }
  return params_.recv_count;
}

int HcomReduceScatterVKernel::Resize(const std::vector<KernelTensor *> &inputs,
                                     const std::vector<KernelTensor *> &outputs) {
  output_size_list_.clear();
  std::vector<int64_t> input_split_sizes;
  if (inputs.size() == kInputNum2) {
    input_split_sizes = inputs[kIndex1]->GetValueWithCheck<std::vector<int64_t>>();
  } else {
    MS_LOG(INTERNAL_EXCEPTION) << "ReduceScatterV input size must be 2, but got " << inputs.size();
  }

  int64_t output_numel = GetReduceScatterVParam(input_split_sizes);
  ShapeVector shape;
  if (output_numel != 0) {
...
bool HcomReduceScatterVKernel::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &,
  ...
  auto hccl_result = hccl::HcclAdapter::GetInstance().HcclReduceScatterV(send_buf, recv_buf, params_, data_type_,
                                                                         op_type_, stream_ptr, comm_);
  if (hccl_result != HCCL_SUCCESS) {
    MS_LOG(ERROR) << "HcclReduceScatterV failed, ret:" << hccl_result;
    return false;
  }
  return true;
}
