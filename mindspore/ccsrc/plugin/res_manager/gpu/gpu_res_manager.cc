...
bool GPUResManager::Copy(const DeviceSyncPtr &dst_device_sync, const DeviceSyncPtr &src_device_sync, size_t stream_id,
  ...
  MS_LOG(DEBUG) << "Async copy from device address:" << src_device_address->PrintInfo()
  ...
  if (dst_device_address->GetSize() == 0 || src_device_address->GetSize() == 0) {
    MS_LOG(DEBUG) << "No need copy for size 0, from device address:" << src_device_address->PrintInfo()
                  << " to:" << dst_device_address->PrintInfo();
    return true;
  }
