...
bool AscendDeviceAddress::UpdateRemoteToDevice(int32_t device_id, void *addr, size_t size, void *stream_ptr,
                                               size_t offset, bool sync) const {
  MS_LOG(WARNING) << "try to fetch tensor from remote memory to device.";
  return true;
}

bool AscendDeviceAddress::UpdateDeviceToRemote(int32_t device_id, void *addr, size_t size, void *stream_ptr,
                                               size_t offset, bool sync) const {
  MS_LOG(WARNING) << "try to fetch tensor from device to remote memory.";
  return true;
}

bool AscendDeviceAddress::DetachDevice(int32_t device_id, void *addr, bool sync, bool free_device) const {
  MS_LOG(WARNING) << "try to detach device address.";
  return true;
}
