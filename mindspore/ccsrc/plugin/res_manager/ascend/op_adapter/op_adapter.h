...
class OpAdapter : public BaseOpAdapter {
 ...
 private:
  ...
  static std::vector<int64_t> ConvertAny(const ValuePtr &value, const AnyTraits<std::vector<int64_t>>,
    ...
    if (value->isa<MeTensor>()) {
      ...
      if (type == kNumberTypeInt32) {
        ...
        for (size_t i = 0; i < size; i++) {
          (void)v.emplace_back(static_cast<int64_t>(data[i]));
        }
        return v;
      }
    } else {
  ...
  static std::string ConvertAny(const ValuePtr &value, const AnyTraits<GERotatedIouMode> anyTraitsGE) {
    return ConvertAnyUtil(value, anyTraitsGE);
  }
  ...
  static std::string GetAttrType(const AnyTraits<GERotatedIouMode> anyTraitsGE) {
    std::string ret{};
    return ret;
  }
