...
#include "plugin/ascend/kernel_executor/rts/hierarchical_memory/update_to_remote.h"
...
namespace mindspore {
namespace kernel {
bool UpdateToRemoteKernel::Init(const AnfNodePtr &anf_node) {
...
bool UpdateToRemoteKernel::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &,
  ...
  // Copy device memory to remote device.
  return device::ascend::AscendUvmHal::GetInstance().UpdateDeviceToRemote(device_id_, device_ptr, inputs[0]->size(),
                                                                          stream_ptr, offset, sync);
}
}  // namespace kernel
}  // namespace mindspore
