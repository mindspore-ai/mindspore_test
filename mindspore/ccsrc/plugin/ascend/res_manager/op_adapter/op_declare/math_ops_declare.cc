...
#include "plugin/ascend/res_manager/op_adapter/op_declare/math_ops_declare.h"
#include <string>
#include <vector>
...
// HFFT
CUST_INPUT_MAP(HFFT) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(n)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(HFFT) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(HFFT) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(HFFT, prim::kPrimHFFT->name(), CUST_ADPT_DESC(HFFT));

// HFFT2
CUST_INPUT_MAP(HFFT2) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(s)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(HFFT2) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(HFFT2) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(HFFT2, prim::kPrimHFFT2->name(), CUST_ADPT_DESC(HFFT2));

// HFFTN
CUST_INPUT_MAP(HFFTN) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(s)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(HFFTN) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(HFFTN) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(HFFTN, prim::kPrimHFFTN->name(), CUST_ADPT_DESC(HFFTN));

// IHFFT
CUST_INPUT_MAP(IHFFT) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(n)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(IHFFT) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(IHFFT) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(IHFFT, prim::kPrimIHFFT->name(), CUST_ADPT_DESC(IHFFT));

// IHFFT2
CUST_INPUT_MAP(IHFFT2) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(s)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(IHFFT2) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(IHFFT2) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(IHFFT2, prim::kPrimIHFFT2->name(), CUST_ADPT_DESC(IHFFT2));

// IHFFTN
CUST_INPUT_MAP(IHFFTN) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(s)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(IHFFTN) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(IHFFTN) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(IHFFTN, prim::kPrimIHFFTN->name(), CUST_ADPT_DESC(IHFFTN));
...
// RFFT2
CUST_INPUT_MAP(RFFT2) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(s)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(RFFT2) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(RFFT2) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(RFFT2, prim::kPrimRFFT2->name(), CUST_ADPT_DESC(RFFT2));

// IRFFT2
CUST_INPUT_MAP(IRFFT2) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(s)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(IRFFT2) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(IRFFT2) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(IRFFT2, prim::kPrimIRFFT2->name(), CUST_ADPT_DESC(IRFFT2));

// RFFTN
CUST_INPUT_MAP(RFFTN) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(s)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(RFFTN) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(RFFTN) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(RFFTN, prim::kPrimRFFTN->name(), CUST_ADPT_DESC(RFFTN));

// IRFFTN
CUST_INPUT_MAP(IRFFTN) = {{1, INPUT_DESC(input)}, {2, INPUT_DESC(s)}, {3, INPUT_DESC(dim)}, {4, INPUT_DESC(norm)}};
CUST_ATTR_MAP(IRFFTN) = EMPTY_ATTR_MAP;
CUST_OUTPUT_MAP(IRFFTN) = {{0, OUTPUT_DESC(y)}};
REG_ADPT_DESC(IRFFTN, prim::kPrimIRFFTN->name(), CUST_ADPT_DESC(IRFFTN));
