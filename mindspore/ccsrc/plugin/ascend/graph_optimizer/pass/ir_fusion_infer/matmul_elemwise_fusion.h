...
namespace mindspore {
namespace opt {
class MatmulElemFusion : public PatternProcessPass {
 public:
  explicit MatmulElemFusion(bool multigraph = true, const string &pass_name = "matmul_elemwise_fusion")
      : PatternProcessPass(pass_name, multigraph) {}
  ~MatmulElemFusion() override = default;
  const BaseRef DefinePattern() const override;
  const AnfNodePtr Process(const FuncGraphPtr &graph, const AnfNodePtr &node, const EquivPtr &equiv) const override;

 private:
  std::string GetElemwiseType(const CNodePtr &elemwise_node) const;
