...
const BaseRef MatMulAllReduceAddRmsNormFusionForGe::DefinePattern() const {
...
bool MatMulAllReduceAddRmsNormFusionForGe::IsSupport(const AnfNodePtr &node, const FuncGraphPtr &graph) const {
  auto ms_context = MsContext::GetInstance();
  MS_EXCEPTION_IF_NULL(ms_context);

  if (common::IsExecuteSimulation()) {
    MS_LOG(INFO) << "Not support MatMulAllReduceAddRmsNormFusionForGe when MS_SIMULATION_LEVEL=3.";
    return false;
  }

  if (!ms_context->IsEnableInferBoost()) {
    MS_LOG(INFO) << "for MatMulAllReduceAddRmsNormFusionForGe ops, infer_boost must be enabled.";
    return false;
  }

  bool is_enable_lccl = EnableLccl();
  if (is_enable_lccl) {
    MS_LOG(INFO) << "disable MatMulAllReduceAddRmsNormFusionForGe when lccl is enabled.";
    return false;
  }
...
std::vector<std::string> MatMulAllReduceAddRmsNormFusionForGe::MustExistPrimitiveName() const {
...
CNodePtr MatMulAllReduceAddRmsNormFusionForGe::CreateMatMulAllReduceAddRmsNormNode(
  const FuncGraphPtr &func_graph, const AnfNodePtr &node, const EquivPtr &equiv, const TypeId &add_result_type) const {
...
const AnfNodePtr MatMulAllReduceAddRmsNormFusionForGe::Process(const FuncGraphPtr &graph, const AnfNodePtr &node,
                                                               const EquivPtr &equiv) const {
  MS_LOG(INFO) << "start to process MatMulAllReduceAddRmsNormFusionForGe.";
  ...
  MS_LOG(INFO) << "process MatMulAllReduceAddRmsNormFusionForGe success.";
  return matmul_allreduce_addrmsnorm_cnode;
}
}  // namespace opt
}  // namespace mindspore
