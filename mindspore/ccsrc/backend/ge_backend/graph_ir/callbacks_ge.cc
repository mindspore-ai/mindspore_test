...
static TensorPtr GetMeTensorTransformed(uint32_t graph_id, const std::string &parameter_name,
  ...
  FuncGraphPtr anf_graph = backend::ge_backend::GetAnfGraph(graph_id);
  ...
  return backend::ge_backend::ConvertGeTensor(ge_tensor_ptr, *parameter_shape_ptr, true);
}
...
static TensorPtr GetMeTensorForSummary(const std::string &name, const std::shared_ptr<ge::Tensor> &ge_tensor_ptr) {
  ...
  if (tname == "[:Scalar]") {
    ...
    return backend::ge_backend::ConvertGeTensor(ge_tensor_ptr, shape);
  }
  if (tname == "[:Tensor]" || tname == "[:Histogram]") {
    ...
    return backend::ge_backend::ConvertGeTensor(ge_tensor_ptr);
  }
  if (tname == "[:Image]") {
