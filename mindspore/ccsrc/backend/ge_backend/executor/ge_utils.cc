...
void StorageWeights(std::vector<tensor::TensorPtr> *copy_weights, const std::vector<AnfNodePtr> &storage_part,
  ...
  for (const auto &node : storage_part) {
    ...
    if (common::AnfAlgo::IsParameterWeight(param)) {
      ...
      ge_res_manager->DeviceToDeviceCopy(std::make_shared<tensor::Tensor>(*tensor), target_tensor);
      if (*first_save) {
