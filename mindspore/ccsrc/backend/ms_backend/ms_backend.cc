...
void MSBackend::UnifyMindIR(const FuncGraphPtr &root_graph) const {
...
void MSBackend::ProcessNotSupportCnode(const FuncGraphPtr &func_graph, const mindspore::device::DeviceType &old_target,
...
void MSBackend::CacheFuncGraphWithKernelGraphId(const FuncGraphPtr &func_graph, const GraphId &graph_id,
...
void MSBackend::CompileGraphFromSegment(const GraphSegmentPtr &segment) {
...
uint32_t MSBackend::backend_graph_id_ = 0;
void MSBackend::TransformGraphToActorDAG(const GraphCompilerInfo &graph_compiler_info) {
...
void MSBackend::CompileKernelGraph(const KernelGraphPtr &kernel_graph,
...
void MSBackend::CompileGraph(const FuncGraphPtr &func_graph) {
  MS_EXCEPTION_IF_NULL(func_graph);
...
void MSBackend::CompileSubGraph(const FuncGraphPtr &func_graph) {
...
bool MSBackend::CompileGraphsByKbkCache(const FuncGraphPtr &func_graph, DeviceContext *device_context) {
...
bool MSBackend::DumpBackendInfo() {
...
bool MSBackend::LoadBackendInfo() {
...
bool MSBackend::CacheCompileGraphs() {
  MS_EXCEPTION_IF_NULL(graph_compiler_);
  try {
...
std::shared_ptr<GraphCompilerInfo> MSBackend::ConstructGraphCompilerInfo(const FuncGraphPtr &root_graph) {
  MS_EXCEPTION_IF_NULL(root_graph);
  MS_EXCEPTION_IF_NULL(graph_compiler_);
...
void MSBackend::ParseControlNodes(const GraphCompilerInfo &graph_compile_info) {
  MS_EXCEPTION_IF_NULL(graph_compiler_);
...
bool MSBackend::CheckEnableGraphPipeline(const std::shared_ptr<GraphCompilerInfo> &graph_compiler_info) {
...
void MSBackend::BindCoreForMainThread() {
...
void MSBackend::WaitMultiStream(const GraphCompilerInfo &graph_compiler_info) {
...
BaseRef MSBackend::ConstructOutputByAbstract(const abstract::AbstractBasePtr &abstract,
...
void MSBackend::ConstructOutputByTupleTensor(tensor::TensorPtr output_tensor,
...
void MSBackend::ConstructOutputs(const AnfNodePtr &output_node, const std::vector<tensor::TensorPtr> &output_tensors,
...
void MSBackend::ConstructOutputs(runtime::ActorSet *actor_set, VectorRef *outputs, const FuncGraphPtr &root_graph) {
...
void MSBackend::ContiguousArgs(const VectorRef &args, const GraphCompilerInfo &) {
...
#ifdef ENABLE_DEBUGGER
void MSBackend::SetDebuggerInit() const {
...
BackendGraphId MSBackend::Build(const FuncGraphPtr &func_graph) {
  // TODO(pynative)
  WaitTaskFinish();
  MS_EXCEPTION_IF_NULL(graph_compiler_);
  MS_EXCEPTION_IF_NULL(func_graph);
...
RunningStatus MSBackend::Run(BackendGraphId graph_id, const VectorRef &inputs, VectorRef *outputs) {
