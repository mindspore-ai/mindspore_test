...
BaseRef CreateNodeOutputTensor(const session::KernelWithIndex &node_output_pair, const KernelGraphPtr &graph,
  ...
  if (is_internal_output) {
    ...
    if (tensor == nullptr) {
      tensor = tensor::from_spec(type_id, shape, device::DeviceType::kCPU);
...
void CheckInputTensorShape(const tensor::TensorPtr &tensor, const CNodePtr &kernel, size_t input_index) {
  MS_EXCEPTION_IF_NULL(tensor);
...
ValuePtr SessionBasic::GetValueNodeOutput(const AnfNodePtr &node, size_t output_index) const {
  ...
  if (value->isa<ValueTuple>()) {
    ...
    tensor::TensorPtr tensor = tensor::from_buffer(kObjectTypeString, shape, value_string.data(), value_string.size());
    MS_EXCEPTION_IF_NULL(tensor);
...
tensor::TensorPtr SessionBasic::GetCNodeOutputTensor(
  const KernelWithIndex &kernel_with_index, const std::map<KernelWithIndex, tensor::TensorPtr> &op_output) const {
...
void SessionBasic::GetModelInputsInfo(uint32_t graph_id, std::vector<tensor::TensorPtr> *inputs,
  ...
  for (size_t i = 0; i < kernel_graph_inputs.size(); ++i) {
    ...
    if (!common::AnfAlgo::IsParameterWeight(parameter)) {
      ...
      auto ms_tensor = tensor::from_spec(data_type, input_shape, device::DeviceType::kCPU);
