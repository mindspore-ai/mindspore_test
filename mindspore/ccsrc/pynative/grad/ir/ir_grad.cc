...
#include "pynative/grad/ir/ir_grad.h"
#include <algorithm>
#include <memory>
#include <string>
#include <utility>
...
GradParamPtr ConstructCustomBpropInputs(const CustomContext &context) {
  ...
  std::transform(context.inputs.begin(), context.inputs.end(), std::back_inserter(op_grad_info->input_abs),
                 [](const auto &value) { return CommonUtils::SetAbstractValueToAnyValue(value->ToAbstract()); });
  op_grad_info->out_value = context.output;
  op_grad_info->out_abs = CommonUtils::SetAbstractValueToAnyValue(context.output->ToAbstract());
...
FuncGraphPtr IrGrad::Finish(const tensor::BaseTensorPtrList &weights, const std::vector<size_t> &grad_position,
  ...
  CommonUtils::DumpGraphIR("before_final_opt.ir", ad_param()->tape_);
  ...
  if (value->isa<tensor::BaseTensor>()) {
    ...
    if (auto_grad_meta_data->input_type() == InputType::kParameter && variable == nullptr) {
      (void)ir_bprop_->AddParameterNode(sens_tensor,
                                        CommonUtils::SetAbstractValueToAnyValue(sens_tensor->ToAbstract()));
...
void IrGrad::ReplacePrimalParameter(bool has_sens_arg) {
  CommonUtils::DumpGraphIR("replace_param.ir", ad_param()->tape_);
...
void IrGrad::UpdateTapeParameter(const tensor::BaseTensorPtr &tensor) const {
  ...
  if (param == nullptr) {
    param = ir_bprop_->CreateTapeParameter(tensor, CommonUtils::SetAbstractValueToAnyValue(tensor->ToAbstract()));
  }
  ...
  p->set_abstract(CommonUtils::SetAbstractValueToAnyValue(tensor->ToAbstract()));
}
}  // namespace autograd
}  // namespace pynative
}  // namespace mindspore
