...
void IrPassForward::ReversePassFuncGraph(const FuncGraphPtr &func_graph) {
  ...
  CommonUtils::DumpGraphIR("reverse_cnode_graph.ir", func_graph);
}
...
void IrPassForward::PassForHookOp(const OpGradInfoPtr &op_grad_info) {
  ...
  for (size_t i = 0; i < op_grad_info->input_value.size(); ++i) {
    ...
    if (val->isa<ValueDictionary>()) {
      ...
      op_grad_info->input_abs[i] = CommonUtils::SetAbstractValueToAnyValue(op_grad_info->input_value[i]->ToAbstract());
    }
  }
  if (op_grad_info->out_value->isa<ValueDictionary>()) {
    ...
    op_grad_info->out_abs = CommonUtils::SetAbstractValueToAnyValue(op_grad_info->out_value->ToAbstract());
