...
void RegOpCall(const py::module *m) {
  ...
  (void)py::class_<TensorOverloadCall, std::shared_ptr<TensorOverloadCall>>(*m, "TensorOverloadCall")
    ...
    .def("__repr__",
         [](const std::shared_ptr<TensorOverloadCall> &op_call) { return "<OpCall name=" + op_call->name() + ">"; });
  (void)py::class_<FunctionalOverloadCall, std::shared_ptr<FunctionalOverloadCall>>(*m, "FunctionalOverloadCall")
    .def(py::init<>())
    .def("__call__", &FunctionalOverloadCall::operator())
    .def_property_readonly("name", &FunctionalOverloadCall::name)
    .def("__str__",
         [](const std::shared_ptr<FunctionalOverloadCall> &op_call) { return "<OpCall name=" + op_call->name() + ">"; })
    .def("__repr__", [](const std::shared_ptr<FunctionalOverloadCall> &op_call) {
      return "<OpCall name=" + op_call->name() + ">";
    });
}
