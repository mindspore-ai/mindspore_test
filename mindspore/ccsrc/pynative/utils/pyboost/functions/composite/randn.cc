...
namespace kernel {
namespace pyboost {
tensor::TensorPtr randn(const ValueTuplePtr &shape, const tensor::TensorPtr &seed, const tensor::TensorPtr &offset,
                        const std::optional<Int64ImmPtr> &dtype, const std::optional<Int64ImmPtr> &device) {
  tensor::TensorPtr output_empty = empty(shape, dtype, device);
