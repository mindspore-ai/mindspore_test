...
void ForwardExecutor::CreateViewOutputTensor(const FrontendOpRunInfoPtr &op_run_info,
  ...
  auto kernel_tensor = std::make_shared<kernel::KernelTensor>(
    std::make_shared<abstract::TensorShape>(output_tensor->shape()),
    std::make_shared<TensorType>(TypeIdToType(output_tensor->data_type())), nullptr, nullptr,
    input_device_address->GetSize(), "DEFAULT_FORMAT", output_tensor->data_type(), output_tensor->shape(),
