...
FuncGraphPtr ListAppend::GenerateFuncGraph(const abstract::AbstractBasePtrList &args_list) {
  ...
  res_graph->debug_info()->set_name("append");
  AnfNodePtr arg0_node = res_graph->add_parameter();
  AnfNodePtr arg1_node = res_graph->add_parameter();
  ...
  for (size_t i = 0; i < arg0_length; ++i) {
    elems.push_back(
      res_graph->NewCNode({NewValueNode(prim::kPrimListGetItem), arg0_node, NewValueNode(SizeToLong(i))}));
  }
  elems.push_back(arg1_node);

  res_graph->set_output(res_graph->NewCNode(elems));
  return res_graph;
}
...
FuncGraphPtr ListInsert::GenerateFuncGraph(const abstract::AbstractBasePtrList &args_list) {
  ...
  res_graph->set_flag(FUNC_GRAPH_FLAG_CORE, true);
  MS_EXCEPTION_IF_NULL(res_graph->debug_info());
  res_graph->debug_info()->set_name("insert");
  AnfNodePtr arg0_node = res_graph->add_parameter();
  AnfNodePtr insert_index_node = res_graph->add_parameter();
  AnfNodePtr insert_obj_node = res_graph->add_parameter();
  ...
  if (fallback::EnableFallbackListDictInplace() && fallback::HasObjInExtraInfoHolder(arg0_list) &&
    ...
    AnfNodePtrList list_inplace_inputs = {NewValueNode(prim::kPrimListInplaceInsert), arg0_node, insert_index_node,
                                          insert_obj_node};
    auto list_inplace_node = res_graph->NewCNodeInOrder(list_inplace_inputs);
    list_inplace_node->set_has_side_effect_node(true);
    res_graph->set_output(list_inplace_node);
  ...
  for (int64_t i = 0; i < insert_position; ++i) {
    auto value = res_graph->NewCNode({NewValueNode(prim::kPrimListGetItem), arg0_node, NewValueNode(i)});
  ...
  auto out = res_graph->NewCNode(elems);
...
FuncGraphPtr ListPop::GenerateFuncGraph(const abstract::AbstractBasePtrList &args_list) {
  ...
  res_graph->set_flag(FUNC_GRAPH_FLAG_CORE, true);
  MS_EXCEPTION_IF_NULL(res_graph->debug_info());
  res_graph->debug_info()->set_name("pop");
  AnfNodePtr arg0_node = res_graph->add_parameter();
  AnfNodePtr pop_index_node = res_graph->add_parameter();
  ...
  auto pop_node = res_graph->NewCNode({NewValueNode(prim::kPrimListGetItem), arg0_node, NewValueNode(pop_position)});
  for (int64_t i = pop_position + 1; i < len; ++i) {
    auto value = res_graph->NewCNode({NewValueNode(prim::kPrimListGetItem), arg0_node, NewValueNode(i)});
    elems.push_back(value);
  }

  auto new_list = res_graph->NewCNode(elems);
  auto out = res_graph->NewCNode({NewValueNode(prim::kPrimMakeTuple), new_list, pop_node});
  res_graph->set_output(out);
  return res_graph;
}
...
FuncGraphPtr ListClear::GenerateFuncGraph(const abstract::AbstractBasePtrList &args_list) {
  ...
  res_graph->set_output(NewValueNode(std::make_shared<ValueList>(empty_list)));
  return res_graph;
}
...
FuncGraphPtr ListExtend::GenerateFuncGraph(const abstract::AbstractBasePtrList &args_list) {
  ...
  res_graph->set_flag(FUNC_GRAPH_FLAG_CORE, true);
  MS_EXCEPTION_IF_NULL(res_graph->debug_info());
  res_graph->debug_info()->set_name("extend");
  ...
  AddNodeToElems(abs_current_list, res_graph, &elems);
  AddNodeToElems(abs_extend, res_graph, &elems);
...
FuncGraphPtr ListReverse::GenerateFuncGraph(const abstract::AbstractBasePtrList &args_list) {
  ...
  res_graph->set_flag(FUNC_GRAPH_FLAG_CORE, true);
  MS_EXCEPTION_IF_NULL(res_graph->debug_info());
  res_graph->debug_info()->set_name("reverse");
  AnfNodePtr arg_node = res_graph->add_parameter();
  // List inplace operation do not support:
  // 1. The python object of list is not found.
  // 2. List operation is generated by Vmap, will be opened after Vmap function of list inplace operation is provided.
  ...
  for (int64_t i = arg_length - 1; i >= 0; --i) {
    elems.push_back(res_graph->NewCNode({NewValueNode(prim::kPrimListGetItem), arg_node, NewValueNode(i)}));
  }
