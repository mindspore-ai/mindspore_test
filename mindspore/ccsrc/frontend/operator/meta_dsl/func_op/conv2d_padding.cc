...
#include "mindspore/ccsrc/frontend/operator/meta_dsl/func_op/conv2d_padding.h"
...
namespace {
constexpr auto kConv2dPaddingGap2 = 2;
constexpr auto kConv2dPaddingSize2 = 2;
}  // namespace
inline void CheckConv2DPaddingPositiveVector(const string &arg_name, const ArrayValue<int64_t> &array,
...
void CheckConv2DShape(const PrimitivePtr &primitive, const ShapeVector &input_shape_param,
...
void CheckConv2DPaddingInputs(const PrimitivePtr &primitive, const AbstractBasePtrList &input_args) {
  ...
  if (!IsDynamicRank(input_shape) && input_rank != kIndex4 && input_rank != kIndex3) {
    MS_EXCEPTION(ValueError) << "Conv2d expects input to have a 3 or 4 dimensions, but got input with " << input_rank
                             << " dimension(s).";
  }
  if (!IsDynamicRank(weight_shape) && weight_rank != kIndex4) {
    MS_EXCEPTION(ValueError) << "Conv2d expects weight to have a 4 dimensions, but got weight with " << weight_rank
                             << " dimension(s).";
  }
  if (padding_opt.has_value()) {
    ...
    if (padding_enum != PadMode::VALID && padding_enum != PadMode::SAME) {
      MS_EXCEPTION(ValueError) << "For primitive[Conv2DPadding], input padding string must be one of {'same', 'valid'}";
    }
  }
    ...
    if (padding_enum == PadMode::SAME) {
      for (int i = 0; i < SizeToLong(stride.size()); i++) {
        if (!stride.IsValueUnknown(i) && stride[i] != 1) {
          MS_EXCEPTION(ValueError) << "For primitive[Conv2DPadding], stride must be 1 when padding is same.";
        }
      }
    }
  }
  if (stride_opt.has_value()) {
    const auto &stride = stride_opt.value();
    CheckConv2DPaddingPositiveVector("stride", stride, prim_name, true);
  }
  if (!dilation_opt.has_value()) {
    return;
  }
  auto dilation = dilation_opt.value();
  CheckConv2DPaddingPositiveVector("dilation", dilation, prim_name, true);

  if (!IsDynamic(input_shape) && !IsDynamic(weight_shape)) {
    CheckConv2DShape(primitive, input_shape, weight_shape, input_args);
  }
}

NodePtr Conv2DPaddingMetaImpl::CalcPadding(const NodePtr &in_shape, const NodePtr &w_shape, const NodePtr &stride,
  ...
  res_list = For(calc_dim_padding, Tuple(Value(2), Value(3)), res_list);
...
BeginFunction(Conv2DPadding, input, weight, bias, stride, padding, dilation, groups) {
  // illegal padding
  ...
  auto process_dims = [&]() {
    ...
    auto is_4d = Equal(input_rank, Value(4));

    auto process_3d_input_branch = [&]() { Return(Call(Prim(ExpandDims), input, Value(0))); };
    auto input_branch = [&]() { Return(input); };
    auto processed_input = If(is_4d, input_branch, process_3d_input_branch);
    auto needs_squeeze = Not(is_4d);
    return Tuple(processed_input, needs_squeeze);
  };

  // expand stride/dilation (x) -> (x, x)
  auto expand_params = [&](const NodePtr &param) {
    auto p_num = Call(Prim(SequenceLen), param);
    auto expand_condition = Equal(p_num, Value(1));
    auto true_branch = [&]() {
      auto repeated = Tuple(GetItem(param, Value(0)), GetItem(param, Value(0)));
      Return(repeated);
    };
  ...
  // main compute
  auto compute_conv = [&]() {
    // 1. processed input dimension
    auto processed_input_needs_squeeze_is_batch_match = process_dims();
    ...
    auto stride_expanded = expand_params(stride);
    auto dilation_expanded = expand_params(dilation);
    ...
    auto same_mode_branch = [&]() {
      ...
      auto is_sym_branch = [&]() {
        auto pad_nd =
          Tuple(GetItem(pad_l, Value(0)), GetItem(pad_l, Value(1)), GetItem(pad_r, Value(0)), GetItem(pad_r, Value(1)));
        Return(Tuple(pad_l, pad_nd, Value(1)));
      };
      auto not_sym_branch = [&]() {
        auto pad_nd = List(Value(0), Value(0), Value(0), Value(0));
        auto calc_pad_nd = [&](const NodePtr &index, const NodePtr &item, const NodePtr &res) {
          ...
          auto pad_idx = ScalarMul(ScalarSub(Value(1), index), Value(2));
        ...
        pad_nd = For(calc_pad_nd, Tuple(Value(0), Value(1)), pad_nd);
    ...
    auto valid_mode_branch = [&]() {
      Return(Tuple(Tuple(Value(0), Value(0)), Tuple(Value(0), Value(0), Value(0), Value(0)), Value(1)));
    };
    ...
    auto is_symmetric_branch = [&]() {
      auto output = Call(Prim(Convolution), processed_input, weight, bias, stride_expanded, pad_params,
                         dilation_expanded, Value(false), Tuple(Value(0), Value(0)), groups);
      Return(output);
    };
