...
class BeforeOptARewriter : public BaseRewriter {
 ...
 protected:
  ...
  bool CheckDictUserHasFuncGraph(const AnfNodePtr &node, const FuncGraphPtr &func) const {
    MS_EXCEPTION_IF_NULL(node);
    MS_EXCEPTION_IF_NULL(func);
    if (!IsValueNode<ValueDictionary>(node)) {
      return false;
    }
    auto mng = func->manager();
  ...
  bool ConvertDictToTuple(const AnfNodePtr &node, const FuncGraphPtr &fg) const {
    return !is_dict_output_ && !has_dict_inplace_ && !CheckUserHasPyExecute(node, fg) &&
           !CheckDictUserHasFuncGraph(node, fg);
  }
...
class AfterOptARewriter : public BaseRewriter {
 ...
 protected:
  ...
  AnfNodePtr BuildFlattenedInputJoinedStr(const CNodePtr &node) const {
    ...
    MS_LOG(ERROR) << "Start to flattened JoinedStr node: " << node->DebugString(2);
    const auto &inputs = node->inputs();
    ...
    MS_LOG(ERROR) << "Result flattened JoinedStr node: " << ret->DebugString(2);
    return ret;
  }
