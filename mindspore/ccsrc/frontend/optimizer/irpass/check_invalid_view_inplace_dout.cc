...
bool CheckInvalidDoutOfBprop(const FuncGraphPtr &func_graph, bool need_throw_exception = true) {
  ...
  for (auto &node : all_nodes) {
    ...
    auto seq_abs = abstract->cast<abstract::AbstractSequencePtr>();
    MS_EXCEPTION_IF_NULL(seq_abs);
    auto seq_nodes = seq_abs->sequence_nodes();
    if (seq_nodes == nullptr) {
      continue;
    }
    for (auto seq_node : *seq_nodes) {
      const auto &bprop_output_indexes = GetSequenceNodeElementsUseFlags(seq_node.lock());
