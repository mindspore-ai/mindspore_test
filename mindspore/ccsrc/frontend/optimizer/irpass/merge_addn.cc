...
AnfNodePtr MergeAddN::operator()(const OptimizerPtr &optimizer, const AnfNodePtr &node) {
  Reset();
...
void MergeAddN::Visit(const CNodePtr &cnode) {
...
bool MergeAddN::is_unique(const AnfNodePtr &node) {
...
void MergeAddN::UpdateDumpFlag(const AnfNodePtr &node) {
  if (node == nullptr) {
    return;
  }
...
AnfNodePtr AddNZeroFilter::operator()(const OptimizerPtr &, const AnfNodePtr &node) {
  Reset();
  AnfVisitor::Match(prim::kPrimAddN, {IsCNode})(node);
...
bool AddNZeroFilter::IsReshapeZeros(const AnfNodePtr &node) {
...
void AddNZeroFilter::Visit(const CNodePtr &cnode) {
  if (!IsPrimitiveCNode(cnode, prim::kPrimMakeTuple)) {
    return;
  }
...
void AddNCheckDump::Visit(const CNodePtr &cnode) {
  if (!IsPrimitiveCNode(cnode, prim::kPrimMakeTuple)) {
    return;
  }
...
void AddNCheckDump::Reset() { set_dump_ = false; }
