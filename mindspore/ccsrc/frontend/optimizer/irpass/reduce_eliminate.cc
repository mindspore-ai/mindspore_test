...
AnfNodePtr ReduceOneEliminater::operator()(const OptimizerPtr &, const AnfNodePtr &node) {
  Reset();
...
void ReduceOneEliminater::Visit(const AnfNodePtr &node) {
...
void ReduceOneEliminater::Visit(const ValueNodePtr &vnode) {
  if (x_shape_.empty()) {
    return;
  }
...
void ReduceOneEliminater::Reset() {
  axis_.clear();
  x_shape_.clear();
  x_ = nullptr;
  is_axis_one_ = false;
