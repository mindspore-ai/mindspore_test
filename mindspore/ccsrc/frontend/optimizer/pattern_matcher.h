...
class PConstant : public PBase<PConstant<T> > {
 public:
  ...
  AnfNodePtr NewTensorFilledWithData(const AnfNodePtr &node, const AnfNodePtr &x = nullptr) const {
    ...
    if (x == nullptr) {
      auto new_tensor_ptr = tensor::from_spec(tensor_type_ptr->type_id(), tensor_shape, device::DeviceType::kCPU);
 ...
 private:
  tensor::TensorPtr GetNewTensor(const AnfNodePtr &vnode_1, const AnfNodePtr &vnode_2, const AnfNodePtr &node_3) const {
    ...
    if ((tensor_1_abstract->shape()->shape() == tensor_2_abstract->shape()->shape()) &&
      ...
      return tensor::from_spec(tensor_1_type_ptr->type_id(), tensor_out_shape, device::DeviceType::kCPU);
    } else {
      ...
      return tensor::from_spec(tensor_3_type_ptr->type_id(), tensor_out_shape, device::DeviceType::kCPU);
    }
  }
};
