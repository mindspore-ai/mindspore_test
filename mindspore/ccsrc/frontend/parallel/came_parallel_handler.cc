...
std::pair<bool, const CNodePtr> BackwardSearchCNode(const CNodePtr &bottom_node,
  ...
  for (const auto &call_param : bwd_calls) {
    ...
    if (cnode_name != node_name) {
      MS_LOG(DEBUG) << "[CAME] backward search failed, expect node name: " << node_name << " but got " << cnode_name;
      return {false, bottom_node};
    }
    const auto &param_node = target_node->input(idx + 1);
    if (!param_node) {
      MS_LOG(DEBUG) << "[CAME] backward search failed, expect param at index: " << (idx + 1) << " but got null";
      return {false, bottom_node};
    }
    if (!param_node->isa<CNode>()) {
      MS_LOG(DEBUG) << "[CAME] param node is not a cnode!";
      return {false, bottom_node};
    }
  ...
  if (cnode_name != target_name) {
    MS_LOG(DEBUG) << "[CAME] backward search failed, expect target node name: " << target_name << " but got "
                  << cnode_name;
    return {false, bottom_node};
  }
...
std::pair<bool, std::vector<CNodePtr>> ForwardSearchCNode(const CNodePtr &start_node,
  ...
  if (!start_node) {
    MS_LOG(DEBUG) << "[CAME] forward search start is null!";
    return {false, {}};
  }
  if (fwd_calls.empty()) {
    MS_LOG(DEBUG) << "[CAME] gives empty forward calls!";
    return {false, {}};
  }
