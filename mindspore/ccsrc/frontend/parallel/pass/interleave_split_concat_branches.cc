...
void PrintBranchNodes(const std::vector<CNodePtr> &branch_nodes, size_t branch_id) {
  MS_LOG(INFO) << "Branch " << branch_id << " node name:";
  ...
  MS_LOG(INFO) << "End print branch " << branch_id << " nodes.";
}
...
void InterleaveBranchesForCommunicationOverlap(const InterLeaveScopePtr &interleave_scope, const FuncGraphPtr &graph) {
  ...
  for (size_t i = 0; i < interleave_branch_num; ++i) {
    ...
    PrintBranchNodes(branch_nodes, i);
  }
