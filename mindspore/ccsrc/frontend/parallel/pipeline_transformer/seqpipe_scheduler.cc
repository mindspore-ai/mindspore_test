...
namespace mindspore {
namespace parallel {
bool SortFuncInsideMicroSeqpipe(const Border &b_i, const Border &b_j) {
...
std::pair<Border, Border> SeqpipeScheduler::SeqpipeBorder(const std::vector<Border> &borders, int64_t seq_chunk,
  ...
  if (candidates.size() > 1) {
    std::sort(candidates.begin(), candidates.end(), SortFuncInsideMicroSeqpipe);
  ...
  for (size_t chunk = 0; chunk < chunk_num; ++chunk) {
    for (size_t micro = 0; micro < micro_size; ++micro) {
      for (size_t seq_chunk = 0; seq_chunk < seq_chunk_size; ++seq_chunk) {
        ...
        MS_LOG(DEBUG) << "micro:" << micro << ", seq_chunk:" << seq_chunk << ", chunk:" << chunk
                      << ", border:" << border.first.border->DebugString();
...
std::vector<Triplet> SeqpipeScheduler::ExecuteOrder() {
  if (micro_size_ % stage_num_ != 0) {
    MS_LOG(EXCEPTION) << "Current, Seq pipeline only support micro step size can be divisible by pipeline stage num.";
  }
  int64_t bias = chunk_num_ == 1 ? 1 : 2;
  // < means warm up phase
  warm_up_size_ = LongToSize((stage_num_ - 1 - stage_) * bias + (chunk_num_ - 1) * stage_num_ * seq_chunk_size_ +
                             seq_chunk_size_ - 1);
  ...
  size_t micro_size = LongToSize(micro_size_);
  size_t seq_chunk_size = LongToSize(seq_chunk_size_);
  size_t stage_num = LongToSize(stage_num_);
  for (size_t index = 0; index < micro_size * chunk_num * seq_chunk_size; ++index) {
    size_t seq_chunk = index % seq_chunk_size;
    size_t micro = (index % (stage_num * seq_chunk_size)) / seq_chunk_size +
                   stage_num * (index / (seq_chunk_size * stage_num * chunk_num));
    size_t chunk = (index / (seq_chunk_size * stage_num)) % chunk_num;
    if (micro < micro_size) {
...
void SeqpipeScheduler::SendRecvControl(const std::pair<BorderStruct, BorderStruct> &send,
  ...
  if (send.first.border->HasPrimalAttr(kPrimalAttrForwardUniqueId) !=
      recv.first.border->HasPrimalAttr(kPrimalAttrForwardUniqueId)) {
    auto prior = send.first.border->HasPrimalAttr(kPrimalAttrForwardUniqueId) ? send : recv;
    auto post = send.first.border->HasPrimalAttr(kPrimalAttrForwardUniqueId) ? recv : send;
...
void SeqpipeScheduler::SpecialCalmDownControl(const std::pair<BorderStruct, BorderStruct> &fp_send_node,
                                              const std::pair<BorderStruct, BorderStruct> &send,
                                              const std::pair<BorderStruct, BorderStruct> &recv) {
  if (IsPrimitiveCNode(fp_send_node.second.border, prim::kPrimSend) &&
      IsPrimitiveCNode(send.first.border, prim::kPrimSend)) {
    ControlOrder(fp_send_node.second, send.first, "calm_down");
  }
  if (IsPrimitiveCNode(fp_send_node.second.border, prim::kPrimSend) &&
      IsPrimitiveCNode(recv.first.border, prim::kPrimReceive)) {
    ControlOrder(fp_send_node.second, recv.first, "calm_down");
  }
...
void SeqpipeScheduler::ExtractDataStruct() {
  ...
  // >= means calm down phase
  calm_down_index_ = LongToSize(2 * micro_size_ * chunk_num_ * seq_chunk_size_) - warm_up_size_;
}
...
BorderPair SeqpipeScheduler::GetBorderNodeRecv(size_t index) {
  ...
  if (chunk_num_ > 1 && stage_ == 0 && index >= LongToSize(stage_num_) && !execute_order_[recv_node_index].is_bp) {
    // distance to pre_fetched in fp_execute_order_ is seq_chunk_size_ * micro_size - stage_num
    auto fp_index = GetOrderIndex(seq_chunk, micro, chunk, false, "fp");
    auto prefetched_fp_index = fp_index + LongToSize(seq_chunk_size_ * stage_num_ - stage_num_);
  ...
  if (chunk_num_ > 1 && stage_ == stage_num_ - 1 && index >= last_stage_pre_fetch_index_ &&
      execute_order_[recv_node_index].is_bp) {
    auto bp_index = GetOrderIndex(seq_chunk, micro, chunk, true, "bp");
  ...
  if (recv_node_index >= execute_order_.size() ||
      (index == recv_node_index &&
       std::find(advanced_recv_indexs_.begin(), advanced_recv_indexs_.end(), index) != advanced_recv_indexs_.end())) {
...
BorderPair SeqpipeScheduler::GetBorderNode(const std::string &border_type, size_t index) {
  ...
  auto seq_chunk = execute_order_[index].seq_chunk;
  if (border_type == "Send") {
    size_t send_node_index =
      chunk_num_ == 1 || index < warm_up_size_ + 1 || index >= calm_down_index_ - 1 ? index : index - 1;
    if (chunk_num_ > 1 && index == warm_up_size_ && index + 2 < calm_down_index_) {
      return BorderPair({Border({nullptr, 0, 0, 0}), Border({nullptr, 0, 0, 0})});
    }
    auto s_chunk = execute_order_[send_node_index].chunk;
    auto s_micro = execute_order_[send_node_index].micro;
    auto s_seq_chunk = execute_order_[send_node_index].seq_chunk;
    auto send_node = execute_order_[send_node_index].is_bp ? sorted_bwd_end_[s_chunk][s_micro][s_seq_chunk]
                                                           : sorted_fwd_end_[s_chunk][s_micro][s_seq_chunk];
    return send_node;
  }
  if (border_type == "Receive") {
...
void SeqpipeScheduler::Reorder() {
  ...
  ControlCleanAssigns();
  int64_t bias = chunk_num_ == 1 ? 1 : 2;
  MS_LOG(INFO) << "seq pp calm_down_index_:" << calm_down_index_;
  size_t warm_up_size_stage0 =
    LongToSize((stage_num_ - 1) * bias + (chunk_num_ - 1) * stage_num_ * seq_chunk_size_ + seq_chunk_size_ - 1);
  // stage0 first bp send position is warm_up_size_stage0 + 1 if pp == micro else warm_up_size_stage0 + 2
  // Thus, the last stage doing bp receive position should match the stage0 send position and delay one bp block.
  auto pre_recv_index_last_stage = warm_up_size_stage0 + 1;
  last_stage_pre_fetched_bp_index_ =
    chunk_num_ > 1 && stage_ == stage_num_ - 1
      ? GetOrderIndex(LongToSize(seq_chunk_size_ - 1), 0, LongToSize(chunk_num_ - 2), true, "bp")
      : 0;
  last_stage_pre_fetch_bp_index_ = chunk_num_ > 1 && stage_ == stage_num_ - 1
                                     ? GetOrderIndex(execute_order_[pre_recv_index_last_stage].seq_chunk,
                                                     execute_order_[pre_recv_index_last_stage].micro,
                                                     execute_order_[pre_recv_index_last_stage].chunk, true, "bp") +
                                         1
                                     : 0;
  last_stage_pre_fetch_index_ = chunk_num_ > 1 && stage_ == stage_num_ - 1
                                  ? GetOrderIndex(bp_execute_order_[last_stage_pre_fetch_bp_index_].seq_chunk,
                                                  bp_execute_order_[last_stage_pre_fetch_bp_index_].micro,
                                                  bp_execute_order_[last_stage_pre_fetch_bp_index_].chunk, true)
                                  : 0;
  MS_LOG(INFO) << "last_stage_pre_fetch_index_:" << last_stage_pre_fetch_index_
               << ", last_stage_pre_fetch_bp_index_:" << last_stage_pre_fetch_bp_index_
               << ", last_stage_pre_fetched_bp_index_" << last_stage_pre_fetched_bp_index_
               << ", pre_recv_index_last_stage:" << pre_recv_index_last_stage;

  for (size_t index = 0; index < execute_order_.size() - 1; ++index) {
    // cell to recv
    auto prior_cell = GetBorderNode("Cell", index);
    auto post_recv = GetBorderNode("Receive", index + 1);
    auto next_cell = GetBorderNode("Cell", index + 1);
    ...
    auto send = GetBorderNode("Send", index);
    auto recv = GetBorderNode("Receive", index + 1);
    bool special_calm = index == calm_down_index_ - 1 && index >= warm_up_size_ + 1;
    if (special_calm) {
      auto fp_send_node = execute_order_[index - 1].is_bp
                            ? sorted_bwd_end_[execute_order_[index - 1].chunk][execute_order_[index - 1].micro]
                                             [execute_order_[index - 1].seq_chunk]
                            : sorted_fwd_end_[execute_order_[index - 1].chunk][execute_order_[index - 1].micro]
                                             [execute_order_[index - 1].seq_chunk];

      SpecialCalmDownControl(fp_send_node, send, recv);
    }
    if (!special_calm && IsPrimitiveCNode(recv.first.border, prim::kPrimReceive) &&
        IsPrimitiveCNode(send.second.border, prim::kPrimSend)) {
      SendRecvControl(send, recv);
    }
    // send to call
    auto post_call = GetBorderNode("Call", index + 1);
    if (IsPrimitiveCNode(send.second.border, prim::kPrimSend)) {
      ControlOrder(send.second, post_call.first, "send_call");
    }
...
void SeqpipeScheduler::SetCleanAssignsMicro() {
  auto add_micro = [&](std::vector<Border> &border_vector) {
    ...
    if (std::find_if(micro_map.begin(), micro_map.end(),
      ...
      MS_LOG(ERROR) << "The micro size of cleaning assign of kv_cache:"
                    << "is not equal to the real micro size:" << micro_size_;
      return false;
    }
    return true;
  };
...
void SeqpipeScheduler::GetCleanAssigns() {
  ...
  for (auto &node : all_nodes) {
    ...
    if (param_name.find("mask_cache") != std::string::npos) {
    ...
    if (param_name.find("seq_chunk") != std::string::npos) {
...
void SeqpipeScheduler::ControlCleanAssigns() {
  if (seq_chunk_size_ < 2) {
    MS_LOG(EXCEPTION) << "The seq_chunk size:" << seq_chunk_size_
                      << " is small than 2, thus cannot apply seq pipeline.";
  }
  // in same chunk, all_assign0->fwd_cell0s->all_assign1->fwd_cell1s
