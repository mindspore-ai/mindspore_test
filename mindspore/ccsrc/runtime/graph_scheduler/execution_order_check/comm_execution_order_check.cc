...
void Process::ProcessKernels(int step) {
  ...
  for (const auto &item : kernels) {
    if (item.type() == typeid(CNodePtr)) {
      ...
      if (kernel->primitive == kDistCommSendPrimName || kernel->primitive == kInnerCommSendPrimName ||
          kernel->primitive == kDistCommRecvPrimName || kernel->primitive == kInnerCommRecvPrimName) {
...
void Process::ProcessSendReceive(ProcessResult *result, const std::string &group, const KernelVariant &kernel,
  ...
  std::string value_string =
    (primitive_str == "Send" || primitive_str == kDistCommSendPrimName || primitive_str == kInnerCommSendPrimName)
