...
void SuperKernelActor::RunGraphKernelByKernel(OpContext<KernelTensor> *const context) {
  ...
  if (ActorDispatcher::enable_parallel_dispatch_kernel_for_cur_actor_set()) {
    ...
    if (!need_capture_graph && !need_replay_graph) {
      ...
      auto result = std::async(std::launch::async, [this, context]() {
        return GraphCaptureManager::GetInstance().CheckWeightAndKVCacheNotChange(context, static_cast<size_t>(0));
      });
      GraphCaptureManager::GetInstance().UpdateFixAddressBeforeReplayGraph(context, 0, &memory_free_lists_);
