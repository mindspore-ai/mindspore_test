...
bool SyncAllStreamForDeviceAddress(const DeviceTensorPtr &dst_device_tensor, const DeviceTensorPtr &src_device_tensor,
  ...
  static bool disable_sync_stream_on_demand = []() -> bool {
    static char sync_stream_on_demand[] = "sync_stream_on_demand";
    auto ret = runtime::IsDisableRuntimeConfig(sync_stream_on_demand);
    MS_LOG(INFO) << "Runtime config, sync stream on demand : " << runtime::GetRuntimeConfigValue(sync_stream_on_demand);
    return ret;
  }();
  if (disable_sync_stream_on_demand || !sync_stream_on_demand) {
...
bool SyncStreamOnDemandForDeviceAddress(const DeviceTensorPtr &dst_device_tensor,
  ...
  if (src_device_tensor->GetDeviceType() != device::DeviceType::kCPU) {
    if (stream_id != src_device_tensor->stream_id()) {
