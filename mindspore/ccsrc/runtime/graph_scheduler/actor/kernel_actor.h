...
class KernelActor : public DebugAwareActor {
 ...
 protected:
  ...
  void MakeInputContiguous(OpContext<DeviceTensor> *const context);
  ...
  // Store the tensor generated by MakeInputContiguous.
  std::vector<DeviceTensorPtr> contiguous_tensors_;
  // Store the input infos for recover after launch.
  std::map<size_t, TensorStorageInfoPtr> storage_store_;
  std::map<size_t, DeviceTensor *> input_device_tensors_store_;
  std::map<size_t, KernelTensor *> input_kernel_tensors_store_;
