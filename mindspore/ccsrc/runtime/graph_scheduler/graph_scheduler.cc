  ...
  if (graph_compiler_info.strategy_ == GraphExecutionStrategy::kPipeline) {
    for (size_t index = 0; index < graph_compiler_info.graphs_.size(); ++index) {
      ...
      for (auto &kernel : execution_order) {
        ...
        bool is_fusion_op = common::AnfAlgo::GetCNodeName(kernel) == kMatMulAllReduceOpName;
        is_fusion_op |= common::AnfAlgo::GetCNodeName(kernel) == kMatmulReduceScatterOpName;
        is_fusion_op |= common::AnfAlgo::GetCNodeName(kernel) == kAllGatherMatmulOpName;
        if (!common::AnfAlgo::IsCommunicationOp(kernel) || is_fusion_op) {
          continue;
        }
        auto key =
  ...
  for (size_t i = 0; i < graph_compiler_info.graphs_.size(); ++i) {
    ...
    for (auto &kernel : execution_order) {
      ...
      if (IsKernelActor(kernel, graph_compiler_info.strategy_) && (!IsSkippedKernelActor(kernel))) {
        ...
        bool is_not_fusion_op = common::AnfAlgo::GetCNodeName(kernel) != kMatMulAllReduceOpName;
        is_not_fusion_op &= common::AnfAlgo::GetCNodeName(kernel) != kMatmulReduceScatterOpName;
        is_not_fusion_op &= common::AnfAlgo::GetCNodeName(kernel) != kAllGatherMatmulOpName;
        kernel_actor->inputs_continuous_memory_ = (common::AnfAlgo::IsCommunicationOp(kernel) && is_not_fusion_op) &&
