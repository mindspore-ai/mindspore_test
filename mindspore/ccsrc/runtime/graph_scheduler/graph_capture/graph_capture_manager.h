...
class BACKEND_EXPORT GraphCaptureManager {
 public:
  ...
  void UpdateFixAddressBeforeReplayGraph(OpContext<KernelTensor> *const context, size_t stream_id,
                                         std::queue<std::vector<KernelTensorPtr>> *memory_free_lists);
  ...
  bool CheckWeightAndKVCacheNotChange(OpContext<KernelTensor> *const context, size_t stream_id);
