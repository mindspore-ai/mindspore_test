...
void SetSelectKernelBuildInfo(const KernelBuildInfoPtr &select_kernel_build_info, AnfNode *node) {
 ...
 private:
  ...
  void CreateKernelInfo(kernel::Processor processor) {
    ...
    for (size_t index = 1; index < inputs.size(); ++index) {
      ...
      if (common::AnfAlgo::IsFeatureMapOutput(node.first)) {
        feature_map_input_indexs.push_back(index - 1);
      }
    }
    ...
    SetSelectKernelBuildInfo(builder->Build(), cnode_.get());
  }
  ...
  ValueNodePtr DecodeValueNode(const nlohmann::json &value_json, const FuncGraphPtr &func_graph) const {
    ...
    SetSelectKernelBuildInfo(builder->Build(), value_node.get());
...
ParameterPtr AkgKernelJsonDecoder::DecodeParameter(const nlohmann::json &parameter_json,
  ...
  SetSelectKernelBuildInfo(builder->Build(), new_parameter.get());
