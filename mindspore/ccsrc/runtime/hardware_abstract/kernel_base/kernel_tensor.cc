...
bool SyncCopy(kernel::KernelTensor *const dst_kernel_tensor, kernel::KernelTensor *const src_kernel_tensor,
  ...
  DeviceAddressInfo src_info = {true, src_kernel_tensor->format(), src_kernel_tensor->dtype_id(),
                                src_kernel_tensor->GetShapeVector()};
  DeviceAddressInfo dst_info = {true, dst_kernel_tensor->format(), dst_kernel_tensor->dtype_id(),
                                dst_kernel_tensor->GetShapeVector()};
  return SyncCopy(dst_kernel_tensor->device_address(), src_kernel_tensor->device_address(), stream_id, src_info,
                  dst_info);
}
...
bool SyncCopy(kernel::KernelTensor *const dst_kernel_tensor, tensor::Tensor *const src_tensor, size_t stream_id) {
  ...
  DeviceAddressInfo src_info = {true, kernel::GetFormatFromStrToEnum(src_tensor->format()), src_tensor->data_type(),
                                src_tensor->shape()};
  DeviceAddressInfo dst_info = {true, dst_kernel_tensor->format(), dst_kernel_tensor->dtype_id(),
                                dst_kernel_tensor->GetShapeVector()};
  return SyncCopy(dst_kernel_tensor->device_address(), src_tensor->device_address(), stream_id, src_info, dst_info);
}
...
bool AsyncCopy(kernel::KernelTensor *const dst_kernel_tensor, tensor::Tensor *const src_tensor, size_t stream_id,
  ...
  DeviceAddressInfo dst_info = {true, dst_kernel_tensor->format(), dst_kernel_tensor->dtype_id(),
                                dst_kernel_tensor->GetShapeVector()};
  return AsyncCopy(dst_kernel_tensor->device_address(), src_tensor->device_address(), stream_id, keep_src, src_info,
                   dst_info);
}
...
bool SyncCopy(const tensor::TensorPtr &dst_tensor, kernel::KernelTensor *const src_kernel_tensor, size_t stream_id) {
  ...
  DeviceAddressInfo src_info = {true, src_kernel_tensor->format(), src_kernel_tensor->dtype_id(),
                                src_kernel_tensor->GetShapeVector()};
  DeviceAddressInfo dst_info = {true, kernel::GetFormatFromStrToEnum(dst_tensor->format()), dst_tensor->data_type(),
                                dst_tensor->shape()};
  return SyncCopy(dst_tensor->device_address(), src_kernel_tensor->device_address(), stream_id, src_info, dst_info);
}
...
bool AsyncCopy(const tensor::TensorPtr &dst_tensor, kernel::KernelTensor *const src_kernel_tensor, size_t stream_id,
  ...
  DeviceAddressInfo dst_info = {true, kernel::GetFormatFromStrToEnum(dst_tensor->format()), dst_tensor->data_type(),
                                dst_tensor->shape()};
  return AsyncCopy(dst_tensor->device_address(), src_kernel_tensor->device_address(), stream_id, keep_src, src_info,
                   dst_info);
}
}  // namespace mindspore
