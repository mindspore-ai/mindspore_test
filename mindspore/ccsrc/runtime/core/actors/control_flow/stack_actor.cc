...
#include "runtime/core/actors/control_flow/stack_actor.h"
...
void StackActor::RunOpData(OpData<KernelTensor> *const input_data, OpContext<KernelTensor> *const context) {
  ...
  MS_LOG(DEBUG) << "Actor(" << GetAID().Name() << ") receive the input index:" << input_data->index_
                << " data:" << input_data->data_->ToString();
