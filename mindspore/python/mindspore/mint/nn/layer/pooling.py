...
class MaxUnpool3d(Cell):
    r"""
    Computes the inverse of `Maxpool3d`.

    `MaxUnpool3d` keeps the maximal value and sets all position of non-maximal values to zero.
    ...
    Args:
        kernel_size (Union[int, tuple[int]]): The size of kernel used to take the maximum value,
            If `kernel_size` is an integer, the kernel size of depth, height and width are the same,
            equal to this integer. If `kernel_size` is a tuple, the three values in the tuple represent depth,
        ...
        stride (Union[int, tuple[int]], optional): The distance of kernel moving. If `stride` is an integer,
            ...
            Default: ``None`` , which indicates the moving step is `kernel_size` .
        padding (Union[int, tuple[int]], optional): The pad value to be filled. Default: ``0`` .
            If `padding` is an integer, the paddings of depth, height and width are the same, equal to padding.
            If `padding` is a tuple, the three values in the tuple represent the size of the fill in the depth,
    ...
    Inputs:
        ...
        indices (Tensor): Max values' index represented by the indices. Tensor of shape must be same with input 'input'.
            ...
            Data type must be in int32 or int64.
        output_size (tuple[int], optional): The output size. Default: ``None`` . If output_size is ``None``,
            then the shape of output computed by `kernel_size`, `stride` and `padding`. If output_size is not ``None``,
            then output_size must be :math:`(N, C, D, H, W)` or :math:`(D, H, W)` and output_size must belong to
    ...
    Examples:
        ...
        >>> input = Tensor(np.array([[[[[0, 1], [8, 9]]]]]).astype(np.float32))
        >>> indices = Tensor(np.array([[[[[0, 1], [2, 3]]]]]).astype(np.int64))
        >>> net = mint.nn.MaxUnpool3d(kernel_size=2, stride=1, padding=0)
        >>> output = net(input, indices)
        >>> print(output)
        [[[[[0. 1. 8.]
            [9. 0. 0.]
    ...
    def construct(self, input, indices, output_size=None):
        return mint.nn.functional.max_unpool3d(input, indices,
                                               self.kernel_size, self.stride,
                                               self.padding, output_size)
