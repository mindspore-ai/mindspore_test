...
def aclnn_cache(enable_global_cahce=False, cache_queue_length=10000):
    ...
    Args:
        enable_global_cahce (bool): Set the calnn cache to global when GRAPH_MODE.
            Default: ``False``.
    ...
    if enable_global_cahce not in cache_cfgs:
        raise ValueError(
            f"For 'aclnn_cache', the config must be one of {cache_cfgs}, but got {enable_global_cahce}."
        )
    AscendOpTuningConf.get_instance().set_aclnn_global_cache(enable_global_cahce)
    if cache_queue_length < 0:
        raise ValueError(
            f"For 'aclnn_cache', the config must greater than 0, but got {enable_global_cahce}."
        )
