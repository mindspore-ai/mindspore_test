...
from mindspore.parallel.shard import Layout, DistributedTensorInfo
...
def _redistribute(tensor, dst_dtensor_info):
    ...
    Args:
        ...
        dst_dtensor_info (DistributedTensorInfo): The destination sharding strategy.
    ...
    if not isinstance(dst_dtensor_info, DistributedTensorInfo):
        raise TypeError(
            "dst_dtensor_info should be DistributedTensorInfo type, but got {}".format(type(dst_dtensor_info)))
    ...
    if tensor.dtensor_info is None:
        all_dev_num = get_group_size()
        dev_mat = Layout((all_dev_num,), ("replica",))
        tensor_map = ["None"] * len(tensor.shape)
        layout = dev_mat(*tensor_map)
        tensor.dtensor_info = DistributedTensorInfo(layout)
    if len(tensor.dtensor_info.layout.to_dict()["rank_list"]) < len(dst_dtensor_info.layout.to_dict()["rank_list"]):
        ...
        if get_rank() in dst_dtensor_info.layout.to_dict()["rank_list"]:
            comm_tensor_data_func = CommTensorDataForPP(tensor_data, tensor.dtensor_info, dst_dtensor_info)
            tensor_data = comm_tensor_data_func()
            all_reduce_data = True
    dataset_strategy = (_insert_virtual_pp_dim(tensor.dtensor_info.layout),)
    if get_rank() not in tensor.dtensor_info.layout.to_dict()["rank_list"] and not all_reduce_data:
    ...
    redist_func = RedistributionCell(tensor.dtensor_info.layout, dst_dtensor_info.layout)
    ...
    redist_tensor_data.dtensor_info = dst_dtensor_info
