...
from ..auto_generate import (CeLU, Flatten, LogSoftmax, LogSoftmaxExt, GLU, ReLU, ReLU6, Dense, Tanh,
                             ...
                             IncreFlashAttention, MSELossExt,
                             GridSampler3D, GridSampler2D, LayerNorm, LayerNormExt,
                             HShrink, AdamWeightDecay, Dropout,
                             ApplyRotaryPosEmb, GroupTopk, PagedAttention,
                             PagedAttentionMask, ReshapeAndCache,
                             FlashAttentionScore, PromptFlashAttention,
                             Embedding, UpsampleNearest1D, UpsampleNearest2D,
                             ...
                             SmoothL1Loss, ApplyMomentum, ApplyAdaMax, ApplyAdadelta, ApplyProximalAdagrad,
                             SparseApplyProximalAdagrad, ApplyCenteredRMSProp, ApplyAddSign, ApplyRMSProp,
                             PadV3, ApplyPowerSign, ApplyProximalGradientDescent, LARSUpdate, ApplyFtrl,
                             SparseApplyCenteredRMSProp, ApplyKerasMomentum, ApplyAdagrad, ApplyAdagradV2,
                             ApplyAdagradDA, ApplyGradientDescent,
                             SparseApplyAdagradDA, SparseApplyMomentum,
                             SparseApplyProximalGradientDescent, ApplyAdamWithAmsgrad,
                             ApplyAdamWithAmsgradV2, Adam, SparseApplyRMSProp)
...
class FractionalMaxPool3DWithFixedKsize(Primitive):
    ...
    Args:
        ksize (Union[int, tuple]): Size of the pooling window. `ksize` can be a tuple of three values specify a
        ...
        output_shape (Union[int, tuple]): The target output shape. `output_shape` can be a tuple of three values
            specify a shape :math:`(D_{out}, H_{out}, W_{out})`, or a single int `S` for :math:`(S, S, S)`.
        data_format (str, optional): The optional value for data format.
...
class FractionalMaxPoolWithFixedKsize(Primitive):
    ...
    Args:
        ...
        output_shape (Union[int, tuple[int]]): The target output shape. `output_shape` can be a
          tuple of two values specify a shape :math:`(H_{out}, W_{out})`, or a single int `S` for :math:`(S, S)`.
