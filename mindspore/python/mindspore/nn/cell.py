...
class Cell(Cell_):
    ...
    def shard(self, in_strategy, out_strategy=None, parameter_plan=None):
        ...
        if self._in_strategy is not None:
            logger.warning(f"For '{self._cell_tag}', 'Shard' has been configured more than once. The existing "
                           f"in_strategy is {self._in_strategy} and the existing out_strategy is {self._out_strategy}."
                           f" The new in_strategy {shard_fn.in_strategy} and out_strategy {shard_fn.out_strategy}"
                           f" may not take effect. It is recommended to configure 'Shard' only once.")
