...
class Cell(Cell_):
    ...
    def _check_parallel_strategy(self, stategy, stratey_key_words):
        if not isinstance(stategy, tuple):
            raise TypeError(f"The type of {stratey_key_words} must be tuple, but got {type(stategy)}")
        for layout in stategy:
            if not isinstance(layout, Layout):
                raise TypeError(f"The type of {stratey_key_words} item must be Layout, but got {type(layout)}")

    def shard(self, in_strategy, out_strategy=None, optional_in_strategy=None, parameter_plan=None):
        """
        ...
        Args:
            ...
            out_strategy (Union[None, tuple]): Define the layout of outputs similar with in_strategy.
                                               Default: ``None`` .
            optional_in_strategy (Union[None, tuple]): Define the layout of optional inputs with in_strategy.
                                                       Default: ``None`` .
