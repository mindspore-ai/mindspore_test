...
class PyboostInternalFunctionsHeaderGenerator(BaseGenerator):
    ...
    def _get_convert_dtype(self, arg_dtype, is_optional=False):
        ...
        type_convert = {
            ...
            'tuple[tensor]': 'std::vector<BaseTensorPtr>',
            'list[int]': 'std::vector<int64_t>',
            'list[float]': 'std::vector<float>',
            'list[bool]': 'std::vector<bool>',
            'list[tensor]': 'std::vector<BaseTensorPtr>',
        }

        optional_tensor_type_convert = {
            'tensor': 'std::optional<mindspore::tensor::BaseTensorPtr>',
            'tuple[tensor]': 'std::optional<mindspore::tensor::BaseTensorPtr>',
            'list[tensor]': 'std::optional<mindspore::tensor::BaseTensorPtr>'
        }
