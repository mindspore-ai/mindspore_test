...
struct DeviceAddressInfo {
  ...
  std::string ToString() const {
    std::ostringstream ofs;
    ofs << this << " enable:" << is_enable << " format:" << kernel::GetFormatFromEnumToStr(format_)
        << " type id:" << TypeIdLabel(dtype_id_) << " shape: {";
...
using SyncCopyFunc = std::function<bool(const DeviceAddressPtr &, const DeviceAddressPtr &, size_t,
                                        const DeviceAddressInfo &, const DeviceAddressInfo &)>;
...
MS_CORE_API bool CopyToHost(device::DeviceType device_type, void *dst, const void *src, uint64_t size,
                            size_t stream_id);
// DeviceAddressInfo record the type shape and format information of the device address. If not provided, the
