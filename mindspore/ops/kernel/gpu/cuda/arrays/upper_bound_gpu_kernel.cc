...
#include "kernel/gpu/cuda/arrays/upper_bound_gpu_kernel.h"

namespace mindspore {
namespace kernel {
constexpr int64_t INPUT_DIMS = 2;
bool UpperBoundGpuKernelMod::Init(const std::vector<KernelTensor *> &inputs,
...
int UpperBoundGpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
                                   const std::vector<KernelTensor *> &outputs) {
