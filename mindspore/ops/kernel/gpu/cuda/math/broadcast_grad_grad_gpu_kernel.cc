...
#include "kernel/gpu/cuda/math/broadcast_grad_grad_gpu_kernel.h"
#include <algorithm>
#include <functional>
#include <memory>
...
int BroadcastOpGradGradGpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
  ...
  // For x1_shape, x2_shape, dy1_shape, it's validation has been done in ops/infer/xxx.cc.
  // But we need check shape rank less equal to 7D.
  if (!AlignedBroadCastShape(kMaxDim, &sopd_grad_shape_, &x1_shape_, &x2_shape_)) {
    MS_LOG(ERROR)
      << "For '" << kernel_name_
