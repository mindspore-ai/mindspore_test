...
#include "kernel/gpu/cuda/nn/fast_gelu_grad_kernel.h"
#include <algorithm>
#include <functional>
#include <memory>
...
template <typename T>
bool FastGeLUGradGpuKernelMod::LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
...
int FastGeLUGradGpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
                                     const std::vector<KernelTensor *> &outputs) {
  int ret = KRET_OK;
