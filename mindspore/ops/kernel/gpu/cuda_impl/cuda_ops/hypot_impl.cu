...
template <typename T>
struct HypotFunc {
  __device__ __host__ __forceinline__ T operator()(const T &x1, const T &x2) { return hypotf(x1, x2); }
};

template <>
struct HypotFunc<double> {
  __device__ __host__ __forceinline__ double operator()(const double &x1, const double &x2) { return hypot(x1, x2); }
};
...
template <typename T, typename Func>
__global__ void BroadcastHypotKernel(const size_t l0, const size_t l1, const size_t l2, const size_t l3,
...
template <typename T>
cudaError_t CalHypot(size_t size, const T *x1, const T *x2, T *y, const uint32_t &device_id, cudaStream_t cuda_stream) {
  CalHypotKernel<T, HypotFunc<T>>
...
template <typename T>
cudaError_t BroadcastHypot(const std::vector<size_t> &x1_shape, const std::vector<size_t> &x2_shape,
  ...
  BroadcastHypotKernel<T, HypotFunc<T>><<<CUDA_BLOCKS(device_id, size), CUDA_THREADS(device_id), 0, cuda_stream>>>(
...
template CUDA_LIB_EXPORT cudaError_t CalHypot<float>(size_t, const float *, const float *, float *, const uint32_t &,
                                                     cudaStream_t cuda_stream);
template CUDA_LIB_EXPORT cudaError_t CalHypot<double>(size_t, const double *, const double *, double *,
                                                      const uint32_t &, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t BroadcastHypot<float>(const std::vector<size_t> &, const std::vector<size_t> &,
                                                           const std::vector<size_t> &, const float *, const float *,
                                                           float *, const uint32_t &, cudaStream_t cuda_stream);
template CUDA_LIB_EXPORT cudaError_t BroadcastHypot<double>(const std::vector<size_t> &, const std::vector<size_t> &,
