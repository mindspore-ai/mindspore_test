...
__global__ void SparseApplyAdagradV2Update(const size_t size, const size_t indices_size, const float learning_rate,
  ...
  for (int pos = blockIdx.x * blockDim.x + threadIdx.x; pos < static_cast<int>(size); pos += gridDim.x * blockDim.x) {
    ...
    const T scratch1 = static_cast<T>(learning_rate) * RsqrtFunc(accumulation[cur_pos] + static_cast<T>(epsilon));
