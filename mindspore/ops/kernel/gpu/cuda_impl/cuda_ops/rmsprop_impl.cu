...
__global__ void RmsPropKernel(const size_t batch_size, const size_t input_elements, const T *learning_rate,
  ...
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < (all_elements); i += blockDim.x * gridDim.x) {
    ...
    moment[i] = momentum[0] * moment[i] + learning_rate[batch] *
                                            static_cast<T>(rsqrt(static_cast<double>(mean_square[i] + epsilon[0]))) *
                                            gradients[i];
    variable[i] -= moment[i];
  }
}
...
__global__ void RmsPropKernel(const size_t batch_size, const size_t input_elements,
  ...
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < (all_elements); i += blockDim.x * gridDim.x) {
    auto batch = i / input_elements;
    mean_square[i] =
      decay[0] * mean_square[i] + (static_cast<Complex<double>>(1.0) - decay[0]) * gradients[i] * gradients[i];
...
template <>
__global__ void RmsPropKernel(const size_t batch_size, const size_t input_elements, const Complex<float> *learning_rate,
                              const Complex<float> *decay, const Complex<float> *momentum,
                              const Complex<float> *epsilon, Complex<float> *variable, Complex<float> *mean_square,
                              Complex<float> *moment, Complex<float> *gradients, const size_t size) {
  auto all_elements = batch_size * input_elements;
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < (all_elements); i += blockDim.x * gridDim.x) {
    auto batch = i / input_elements;
    mean_square[i] =
      decay[0] * mean_square[i] + (static_cast<Complex<float>>(1.0) - decay[0]) * gradients[i] * gradients[i];
    moment[i] = momentum[0] * moment[i] + learning_rate[batch] / SqrtFunc(mean_square[i] + epsilon[0]) * gradients[i];
    variable[i] -= moment[i];
  }
}
...
template <typename T>
cudaError_t RmsProp(const size_t batch_size, const size_t input_elements, const T *learning_rate, const T *decay,
...
__global__ void RmsPropCenterKernel(const size_t batch_size, const size_t input_elements, const T *learning_rate,
  ...
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < (all_elements); i += blockDim.x * gridDim.x) {
    auto batch = i / input_elements;
    mean_gradients[i] = decay[batch] * mean_gradients[i] + (static_cast<T>(1.0) - decay[batch]) * gradients[i];
...
__global__ void RmsPropCenterKernel(const size_t batch_size, const size_t input_elements, const half *learning_rate,
  ...
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < (all_elements); i += blockDim.x * gridDim.x) {
    auto batch = i / input_elements;
    mean_gradients[i] = decay[batch] * mean_gradients[i] + (static_cast<half>(1.0) - decay[batch]) * gradients[i];
    mean_square[i] =
      decay[batch] * mean_square[i] + (static_cast<half>(1.0) - decay[batch]) * gradients[i] * gradients[i];
    moment[i] = momentum[batch] * moment[i] +
                learning_rate[batch] *
                  static_cast<half>(rsqrt(
                    static_cast<float>(mean_square[i] - mean_gradients[i] * mean_gradients[i] + epsilon[batch]))) *
                  gradients[i];
    variable[i] -= moment[i];
  }
}
...
template <typename T>
cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements, const T *learning_rate, const T *decay,
                          const T *momentum, const T *epsilon, T *variable, T *mean_gradients, T *mean_square,
...
template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const float *learning_rate, const float *decay, const float *momentum,
                                             const float *epsilon, float *variable, float *mean_square, float *moment,
                                             float *gradients, const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const half *learning_rate, const half *decay, const half *momentum,
                                             const half *epsilon, half *variable, half *mean_square, half *moment,
                                             half *gradients, const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const double *learning_rate, const double *decay, const double *momentum,
                                             const double *epsilon, double *variable, double *mean_square,
                                             double *moment, double *gradients, const size_t size,
                                             cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const int8_t *learning_rate, const int8_t *decay, const int8_t *momentum,
                                             const int8_t *epsilon, int8_t *variable, int8_t *mean_square,
                                             int8_t *moment, int8_t *gradients, const size_t size,
                                             cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const int16_t *learning_rate, const int16_t *decay,
                                             const int16_t *momentum, const int16_t *epsilon, int16_t *variable,
                                             int16_t *mean_square, int16_t *moment, int16_t *gradients,
                                             const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const int32_t *learning_rate, const int32_t *decay,
                                             const int32_t *momentum, const int32_t *epsilon, int32_t *variable,
                                             int32_t *mean_square, int32_t *moment, int32_t *gradients,
                                             const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const int64_t *learning_rate, const int64_t *decay,
                                             const int64_t *momentum, const int64_t *epsilon, int64_t *variable,
                                             int64_t *mean_square, int64_t *moment, int64_t *gradients,
                                             const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const uint8_t *learning_rate, const uint8_t *decay,
                                             const uint8_t *momentum, const uint8_t *epsilon, uint8_t *variable,
...
template CUDA_LIB_EXPORT cudaError_t RmsProp(const size_t batch_size, const size_t input_elements,
                                             const Complex<double> *learning_rate, const Complex<double> *decay,
                                             const Complex<double> *momentum, const Complex<double> *epsilon,
                                             Complex<double> *variable, Complex<double> *mean_square,
                                             Complex<double> *moment, Complex<double> *gradients, const size_t size,
                                             cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const float *learning_rate, const float *decay,
                                                   const float *momentum, const float *epsilon, float *variable,
                                                   float *mean_gradients, float *mean_square, float *moment,
                                                   float *gradients, const size_t size, cudaStream_t cuda_stream);
...
template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const double *learning_rate, const double *decay,
                                                   const double *momentum, const double *epsilon, double *variable,
                                                   double *mean_gradients, double *mean_square, double *moment,
                                                   double *gradients, const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const int8_t *learning_rate, const int8_t *decay,
                                                   const int8_t *momentum, const int8_t *epsilon, int8_t *variable,
                                                   int8_t *mean_gradients, int8_t *mean_square, int8_t *moment,
                                                   int8_t *gradients, const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const int16_t *learning_rate, const int16_t *decay,
                                                   const int16_t *momentum, const int16_t *epsilon, int16_t *variable,
                                                   int16_t *mean_gradients, int16_t *mean_square, int16_t *moment,
                                                   int16_t *gradients, const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const int32_t *learning_rate, const int32_t *decay,
                                                   const int32_t *momentum, const int32_t *epsilon, int32_t *variable,
                                                   int32_t *mean_gradients, int32_t *mean_square, int32_t *moment,
                                                   int32_t *gradients, const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const int64_t *learning_rate, const int64_t *decay,
                                                   const int64_t *momentum, const int64_t *epsilon, int64_t *variable,
                                                   int64_t *mean_gradients, int64_t *mean_square, int64_t *moment,
                                                   int64_t *gradients, const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const uint8_t *learning_rate, const uint8_t *decay,
                                                   const uint8_t *momentum, const uint8_t *epsilon, uint8_t *variable,
                                                   uint8_t *mean_gradients, uint8_t *mean_square, uint8_t *moment,
                                                   uint8_t *gradients, const size_t size, cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const uint16_t *learning_rate, const uint16_t *decay,
                                                   const uint16_t *momentum, const uint16_t *epsilon,
                                                   uint16_t *variable, uint16_t *mean_gradients, uint16_t *mean_square,
                                                   uint16_t *moment, uint16_t *gradients, const size_t size,
                                                   cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const uint32_t *learning_rate, const uint32_t *decay,
                                                   const uint32_t *momentum, const uint32_t *epsilon,
                                                   uint32_t *variable, uint32_t *mean_gradients, uint32_t *mean_square,
                                                   uint32_t *moment, uint32_t *gradients, const size_t size,
                                                   cudaStream_t cuda_stream);

template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   const uint64_t *learning_rate, const uint64_t *decay,
                                                   const uint64_t *momentum, const uint64_t *epsilon,
                                                   uint64_t *variable, uint64_t *mean_gradients, uint64_t *mean_square,
...
template CUDA_LIB_EXPORT cudaError_t RmsPropCenter(const size_t batch_size, const size_t input_elements,
                                                   ...
                                                   Complex<float> *gradients, const size_t size,
                                                   cudaStream_t cuda_stream);
