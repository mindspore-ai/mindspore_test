...
template <typename T>
CUDA_LIB_EXPORT cudaError_t CalSort(const int &inner, const T *data_in, T *data_out, int *index_buff, T *data_buff,
                                    int box_size_, const uint32_t &device_id, cudaStream_t stream);

template <typename T>
CUDA_LIB_EXPORT cudaError_t CalPreprocess(const int num, int *sel_idx, bool *sel_boxes, const T *input, T *output,
                                          int *index_buff, int box_size_, bool *row_mask, const uint32_t &device_id,
                                          cudaStream_t cuda_stream);

template <typename T>
CUDA_LIB_EXPORT cudaError_t CalPreprocess(const int num, int *sel_idx, int *sel_boxes, const T *input, T *output,
                                          int *index_buff, int box_size_, bool *row_mask, const uint32_t &device_id,
                                          cudaStream_t cuda_stream);

template <typename T>
CUDA_LIB_EXPORT cudaError_t CalNms(const int num, const float IOU_value, T *output, bool *sel_boxes, int box_size_,
...
template <typename T>
CUDA_LIB_EXPORT cudaError_t CalNms(const int num, const float IOU_value, T *output, int *sel_boxes, int box_size_,
                                   bool *row_mask, const uint32_t &device_id, cudaStream_t cuda_stream);
