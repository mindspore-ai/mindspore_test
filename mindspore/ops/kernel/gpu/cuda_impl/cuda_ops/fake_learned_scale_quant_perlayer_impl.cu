...
__global__ void FakeLearnedScaleQuantPerLayer(float *output, const int size, float *input_alpha, float *input_quant) {
...
cudaError_t CalFakeLearnedScaleQuantPerLayer(float *output, const int size, float *input_alpha, float *input_quant,
...
cudaError_t CalFakeLearnedScaleQuantPerLayerGrad(float *grad_input, float *grad_alpha, const float *gradient,
                                                 const int size, const float *input_div_alpha, const float *input_quant,
                                                 const bool neg_trunc, cudaStream_t cuda_stream) {
  FakeLearnedScaleQuantPerLayerGrad<<<GET_BLOCKS(size), GET_THREADS, 0, cuda_stream>>>(
    grad_input, grad_alpha, gradient, size, input_div_alpha, input_quant, neg_trunc);
  return GetCudaStatus();
}

cudaError_t CalLSQNudgePerLayer(const float *input, const int size, float *input_alpha, float *input_quant_max,
                                float *input_div_alpha, float *input_quant, const bool neg_trunc,
                                cudaStream_t cuda_stream) {
