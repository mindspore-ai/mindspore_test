...
#ifndef MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_PAD_V3_GRAD_HELPER_H_
#define MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_PAD_V3_GRAD_HELPER_H_
#include <memory>
#include <string>
#include <vector>
#include <utility>
...
class PadV3GradAttr : public GpuKernelAttrBase {
 public:
  PadV3GradAttr() = default;
  ~PadV3GradAttr() override = default;
...
class PadV3GradHelperGpuKernel : public GpuKernelHelperBase {
 public:
  explicit PadV3GradHelperGpuKernel(const std::string &kernel_name, const uint32_t &device_id)
      : GpuKernelHelperBase(kernel_name, device_id) {
    paddings_contiguous_ = 0;
    is_null_input_ = false;
  }

  virtual ~PadV3GradHelperGpuKernel() = default;
  ...
  int Process(const std::vector<void *> &input_ptrs, const std::vector<void *> &output_ptrs,
    ...
    if (mode_ == ops::kReflect) {
      ...
      CalEdgePadGrad3d(input_size_, input_ptr, input_shape_5d_[0], input_shape_5d_[1], input_shape_5d_[2],
                       input_shape_5d_[3], input_shape_5d_[4], output_shape_5d_[2], output_shape_5d_[3],
                       output_shape_5d_[4], paddings_3d_[0].first, paddings_3d_[1].first, paddings_3d_[2].first,
                       output_ptr, device_id_, reinterpret_cast<cudaStream_t>(cuda_stream));
  ...
  void SetKernelParam(const GpuKernelAttrBasePtr &kernel_attr) override {
    attr_ptr_ = std::dynamic_pointer_cast<PadV3GradAttr>(kernel_attr);
  }
      ...
      if (mode_ == ops::kReflect) {
        ...
        if (before >= output_size || after >= output_size) {
          MS_EXCEPTION(ValueError)
            << "For 'PadV3Grad' of 'reflect' mode, paddings must be less than the input dimension size: " << before
 ...
 private:
  ...
  std::shared_ptr<PadV3GradAttr> attr_ptr_;
...
#endif  // MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_PAD_V3_GRAD_HELPER_H_
