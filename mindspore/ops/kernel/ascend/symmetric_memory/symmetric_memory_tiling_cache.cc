...
TilingCacheItemPtr SymmetricMemoryTilingCache::Bind(uint64_t key) {
...
void SymmetricMemoryTilingCache::Unbind(const TilingCacheItemPtr &item) {
  if (item != nullptr) {
    item->ref_count_--;
...
std::vector<TilingCacheItemPtr> SymmetricMemoryTilingCache::CombOutSuspectedUselessItems() {
...
bool SymmetricMemoryTilingCache::Insert(uint64_t key, const TilingCacheItemPtr &ti_ptr) {
