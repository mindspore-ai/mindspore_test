...
void GatherInfo(const ScalarPtr &scalar) {
  ...
  if (scalar->isa<BoolImm>()) {
    ...
    MemcpyToBuf(&value, sizeof(int16_t));
  } else if (scalar->isa<FP16Imm>()) {
    auto value = GetValue<float16>(scalar);
    MemcpyToBuf(&value, sizeof(float16));
  } else {
...
void RefreshAddr(mindspore::kernel::KernelTensor *tensor) {
  ...
  static const auto add_tensor_addr_to_cached_list = device::ascend::GetOpApiFunc("AddTensorAddrToCachedList");
  if (add_tensor_addr_to_cached_list == nullptr) {
    MS_LOG(EXCEPTION) << "AddTensorAddrToCachedList not in " << device::ascend::GetOpApiLibName() << ", please check!";
  }
