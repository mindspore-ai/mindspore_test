...
#include "kernel/ascend/hccl/hcom_all_gather_matmul.h"
...
namespace mindspore {
namespace kernel {
bool HcomAllGatherMatMulKernel::Init(const std::vector<KernelTensor *> &inputs,
  ...
  if (use_lccl_) {
    transpose_a_ = inputs[mindspore::ops::kAllGatherMatmulInputTransInputIndex]->GetValueWithCheck<bool>();
    transpose_b_ = inputs[mindspore::ops::kAllGatherMatmulInputTransX2Index]->GetValueWithCheck<bool>();

    if (GetHcclDataType() != HCCL_DATA_TYPE_FP16 && GetHcclDataType() != HCCL_DATA_TYPE_BFP16) {  //
      MS_LOG(EXCEPTION) << "AllGatherMatMul only support data type fp16 or bf16.";
    }
    ...
    all_gather_matmul_func_ = DlsymFuncObj(AllGatherMatmul, lowlatency_comm_lib_handle_);
    MS_EXCEPTION_IF_NULL(all_gather_matmul_func_);
  }
#endif
  return true;
}
...
bool HcomAllGatherMatMulKernel::Launch(const std::vector<KernelTensor *> &inputs,
                                       const std::vector<KernelTensor *> &workspace,
                                       const std::vector<KernelTensor *> &outputs, void *stream_ptr) {
  MS_LOG(DEBUG) << "AllGatherMatMul launch";
  if (outputs.empty() || workspace.empty() || hccl_data_type_list_.empty()) {
    MS_LOG(ERROR) << "Invalid AllReduce output or data type size (" << outputs.size() << ", " << workspace.size()
  ...
  if (use_lccl_) {
    ...
    Lcal_ms::CoCOutputPkg coc_output_args = {outputs[0]->device_ptr(), outputs[1]->device_ptr()};
    auto lccl_result =
    ...
    if (lccl_result != Lcal_ms::LCAL_SUCCESS) {
      MS_LOG(EXCEPTION) << "LCOC AllGatherMatmul failed.";
    }
  }
#endif
  return true;
