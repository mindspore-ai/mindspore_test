...
tensor::BaseTensorPtr SoftShrinkAscendCustomize(const std::shared_ptr<OpRunner> &op, const BaseTensorPtr &input_tensor,
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, lambd]() {
    MS_LOG(DEBUG) << "Run device task Softshrink start";
    ...
    LAUNCH_ACLNN(aclnnSoftshrink, device_context, op->stream_id(), input_tensor, lambd, outputs[0]);
    MS_LOG(DEBUG) << "Run device task Softshrink end";
  }));
  return op->output(0);
}
}  // namespace pyboost
