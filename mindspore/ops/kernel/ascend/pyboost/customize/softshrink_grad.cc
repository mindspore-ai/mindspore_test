...
tensor::BaseTensorPtr SoftShrinkGradAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  PyBoostUtils::DispatchRun(
    std::make_shared<runtime::PyBoostDeviceTask>([op, input_grad_tensor, input_x_tensor, lambd]() {
      MS_LOG(DEBUG) << "Run device task SoftShrinkGrad start";
      auto device_context = op->device_context();
      const auto &outputs = op->outputs();
      // Malloc for input tensors
      PyBoostUtils::MallocOpInputs(device_context, input_grad_tensor, input_x_tensor);
      // Malloc for output tensors
      PyBoostUtils::MallocOpOutputs(device_context, outputs);
      LAUNCH_ACLNN(aclnnSoftshrinkBackward, device_context, op->stream_id(), input_grad_tensor, input_x_tensor, lambd,
                   outputs[0]);
      MS_LOG(DEBUG) << "Run device task SoftShrinkGrad end";
    }));
  return op->output(0);
}
}  // namespace pyboost
