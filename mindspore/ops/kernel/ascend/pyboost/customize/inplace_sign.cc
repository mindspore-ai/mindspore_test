...
namespace kernel {
namespace pyboost {
tensor::BaseTensorPtr InplaceSignAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor]() {
    ...
    LAUNCH_ACLNN(aclnnSign, device_context, op->stream_id(), input_tensor, input_tensor);
    MS_LOG(DEBUG) << "Launch InplaceSign end";
  }));
  return op->output(0);
}
}  // namespace pyboost
