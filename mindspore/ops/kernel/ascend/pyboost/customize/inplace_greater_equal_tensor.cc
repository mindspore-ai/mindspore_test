...
#include "kernel/ascend/pyboost/customize/inplace_greater_equal_tensor.h"
#include <memory>
...
tensor::BaseTensorPtr InplaceGreaterEqualTensorAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, other_tensor]() {
    ...
    LAUNCH_ACLNN(aclnnInplaceGeTensor, device_context, op->stream_id(), input_tensor, other_tensor);
