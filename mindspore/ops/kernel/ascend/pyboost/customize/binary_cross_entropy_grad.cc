...
#include "kernel/ascend/pyboost/customize/binary_cross_entropy_grad.h"
#include <memory>
...
tensor::BaseTensorPtr BinaryCrossEntropyGradAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  PyBoostUtils::PrepareOpInputs(op->device_context(), op->stream_id(), input_tensor, target_tensor, grad_output_tensor,
                                weight_tensor);
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>(
    [op, grad_output_tensor, input_tensor, target_tensor, weight_tensor, reduction_value]() {
      MS_LOG(DEBUG) << "Run device task BinaryCrossEntropyGrad start";
      ...
      MS_LOG(DEBUG) << "Run device task BinaryCrossEntropyGrad end";
    }));
  return op->output(0);
}
}  // namespace pyboost
