...
#include "kernel/ascend/pyboost/customize/threshold_grad.h"
#include <memory>
...
tensor::BaseTensorPtr ThresholdGradAscendCustomize(const std::shared_ptr<OpRunner> &op, const BaseTensorPtr &dy_tensor,
  ...
  // Async
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, dy_tensor, y_tensor, threshold]() {
    MS_LOG(DEBUG) << "Run device task ThresholdGrad start";
    ...
    MS_LOG(DEBUG) << "Run device task ThresholdGrad end";
  }));
  return op->output(0);
}
}  // namespace pyboost
