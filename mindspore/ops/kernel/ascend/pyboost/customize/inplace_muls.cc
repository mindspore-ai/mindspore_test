...
tensor::BaseTensorPtr InplaceMulsAscendCustomize(const std::shared_ptr<OpRunner> &op, const BaseTensorPtr &input_tensor,
  ...
  if (other->isa<FP32Imm>()) {
    double other_imm = static_cast<double>(GetValue<float>(other));
    other_scalar_aclnn = std::make_shared<FP64Imm>(other_imm);
  } else {
    other_scalar_aclnn = other;
  }
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, other_scalar_aclnn]() {
    ...
    LAUNCH_ACLNN(aclnnInplaceMuls, device_context, op->stream_id(), input_tensor, other_scalar_aclnn);
