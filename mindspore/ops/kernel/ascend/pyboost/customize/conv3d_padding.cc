...
#include "kernel/common/pyboost/auto_generate/convolution_str.h"
...
tensor::BaseTensorPtr Conv3DPaddingAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  auto is_batchify = ConvNDBatchify(input_shape, 3, "conv3d_padding");
  ...
  auto convolution_op = CREATE_PYBOOST_OP(ConvolutionStr, op->device_context()->device_context_key_.device_name_);
  if (is_batchify) {
    auto output_imm = convolution_op->Call(input_tensor, weight_tensor, bias_tensor, stride, padding_enum, dilation,
