...
#define NEXTAFTER_COMPUTE_CASE(DTYPE, TYPE, CTX)                      \
  case (DTYPE): {                                                     \
    ...
    if (result != KERNEL_STATUS_OK) {                                 \
      CUST_KERNEL_LOG_ERROR(ctx, "NextAfter kernel compute failed."); \
      return result;                                                  \
    }                                                                 \
    break;                                                            \
  }
...
uint32_t NextAfterCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kInputNum, kOutputNum),
                           "NextAfter check input and output number failed.");
  CUST_KERNEL_HANDLE_ERROR(ctx, NextAfterParamCheck(ctx), "NextAfter check params failed.");
  auto data_type = ctx.Input(0)->GetDataType();
  switch (data_type) {
    NEXTAFTER_COMPUTE_CASE(DT_FLOAT, float, ctx)
    NEXTAFTER_COMPUTE_CASE(DT_DOUBLE, double, ctx)
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "NextAfter kernel data type [%s] not support.", DTypeStr(data_type).c_str());
...
*/
template <typename T>
void NextAfterCpuKernel::SpecialCompute(CpuKernelContext &ctx, BcastShapeType type, int64_t start, int64_t end,
...
uint32_t NextAfterCpuKernel::NoBcastCompute(CpuKernelContext &ctx) {
  ...
  if (data_num >= kParallelDataNumSameShape) {
    ...
    CUST_KERNEL_HANDLE_ERROR(ctx,
                             CpuKernelUtils::ParallelFor(ctx, data_num, data_num / max_core_num, sharder_nextafter),
