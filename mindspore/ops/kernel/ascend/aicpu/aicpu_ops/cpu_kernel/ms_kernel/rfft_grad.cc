...
#include "cpu_kernel/ms_kernel/rfft_grad.h"
#include <securec.h>
...
const char *kRFFTGrad = "RFFTGrad";
const std::string op_prefix = "Cust";
using complex64 = std::complex<float>;
using complex128 = std::complex<double>;
using NormMode = mindspore::NormMode;

#define RFFTGRAD_COMPUTE_CASE(DTYPE, INTYPE, MIDTYPE, OUTTYPE, CTX)   \
  case (DTYPE): {                                                     \
...
namespace aicpu {
uint32_t RFFTGradCpuKernel::Compute(CpuKernelContext &ctx) {
...
#define SWITCH_DIM_CALCULATE(T1, T2)                                                                          \
  if (x_rank == 1) {                                                                                          \
    ComputeRFFTGrad<T1, T2, 1>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element, \
                               tensor2_shape);                                                                \
  } else if (x_rank == 2) {                                                                                   \
    ComputeRFFTGrad<T1, T2, 2>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element, \
                               tensor2_shape);                                                                \
  } else if (x_rank == 3) {                                                                                   \
    ComputeRFFTGrad<T1, T2, 3>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element, \
                               tensor2_shape);                                                                \
  } else if (x_rank == 4) {                                                                                   \
    ComputeRFFTGrad<T1, T2, 4>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element, \
                               tensor2_shape);                                                                \
  } else if (x_rank == 5) {                                                                                   \
    ComputeRFFTGrad<T1, T2, 5>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element, \
                               tensor2_shape);                                                                \
  } else if (x_rank == 6) {                                                                                   \
    ComputeRFFTGrad<T1, T2, 6>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element, \
                               tensor2_shape);                                                                \
  } else if (x_rank == 7) {                                                                                   \
    ComputeRFFTGrad<T1, T2, 7>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element, \
                               tensor2_shape);                                                                \
  } else {                                                                                                    \
    ComputeRFFTGrad<T1, T2, 8>(calculate_input, output_ptr, norm, n, dim, calculate_shape, calculate_element, \
                               tensor2_shape);                                                                \
  }

double RFFTGradGetnormalized(int64_t element_nums, NormMode norm_type_) {
  double result = 1.0;
...
bool ComputeRFFTGrad(T_in *input_ptr, T_out *output_ptr, NormMode norm_type, int64_t n, int64_t dim,
...
uint32_t RFFTGradCpuKernel::RFFTGradCompute(CpuKernelContext &ctx) {
...
REGISTER_MS_CPU_KERNEL(kRFFTGrad, RFFTGradCpuKernel);
}  // namespace aicpu