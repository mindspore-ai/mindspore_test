...
#define HISTOGRAM_SINGLE_COMPUTE_FLOAT16(begin, end, y_data)                                                         \
  for (int64_t i = (begin); i < (end); ++i) {                                                                        \
    auto elt = static_cast<float>(x_data[i]);                                                                        \
    if (elt < static_cast<float>(leftmost_edge) || elt > static_cast<float>(rightmost_edge)) {                       \
      continue;                                                                                                      \
    }                                                                                                                \
    int64_t pos = static_cast<int64_t>((elt - static_cast<float>(leftmost_edge)) / step * static_cast<float>(bins)); \
    pos = std::min(pos, nbins_minus_1);                                                                              \
    (y_data)[pos] += 1;                                                                                              \
  }
}  // namespace
...
uint32_t HistogramCpuKernel::DoComputeFloat16(CpuKernelContext &ctx) {
  Tensor *x = ctx.Input(0);
  Tensor *y = ctx.Output(0);
  auto x_data = reinterpret_cast<Eigen::half *>(x->GetData());
  auto y_data = reinterpret_cast<float *>(y->GetData());
  int64_t x_num = x->NumElements();
  int32_t y_num = y->NumElements();
  ...
  const float step = static_cast<float>(rightmost_edge) - static_cast<float>(leftmost_edge);
  const int64_t nbins_minus_1 = bins - 1;
  // paraller for calculate
  if (x_num >= kParallelDataNum) {
    uint32_t min_core_num = 1;
    ...
    CUST_KERNEL_HANDLE_ERROR(ctx,
                             CpuKernelUtils::ParallelFor(ctx, x_num, x_num / max_core_num,
                                                         [&](int64_t start, int64_t end) {
                                                           // Allocates a tensor for the thread's local results
                                                           std::vector<int32_t> hist_local(y_num, 0);
                                                           HISTOGRAM_SINGLE_COMPUTE_FLOAT16(start, end, hist_local)
                                                           // Locks and updates the common output
                                                           const std::lock_guard<std::mutex> lock(hist_mutex);
                                                           std::transform(hist_local.begin(), hist_local.end(), y_data,
                                                                          y_data, std::plus<int32_t>());
    ...
    HISTOGRAM_SINGLE_COMPUTE_FLOAT16(0, x_num, y_data)
  }
  return KERNEL_STATUS_OK;
}
