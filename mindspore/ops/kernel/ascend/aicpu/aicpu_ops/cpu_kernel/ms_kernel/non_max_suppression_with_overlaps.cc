...
uint32_t NonMaxSuppressionWithOverlapsCpuKernel::GetInputAndCheck(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (max_output_size_ >= 0), KERNEL_STATUS_PARAM_INVALID,
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, (scores_rank == 1), KERNEL_STATUS_PARAM_INVALID,
                          "The input dim size of scores must be 1-D, while %d.", scores_rank);
  CUST_KERNEL_CHECK_FALSE(ctx, (scores_shape->GetDimSize(0) == num_boxes_), KERNEL_STATUS_PARAM_INVALID,
...
uint32_t NonMaxSuppressionWithOverlapsCpuKernel::DoNonMaxSuppressionWithOverlapsOp(CpuKernelContext &ctx) {
  CUST_KERNEL_LOG_INFO(ctx, "DoNonMaxSuppressionWithOverlapsOp start!!");
  ...
  CUST_KERNEL_LOG_INFO(ctx, "DoNonMaxSuppressionWithOverlapsOp end!!");
  return KERNEL_STATUS_OK;
}

uint32_t NonMaxSuppressionWithOverlapsCpuKernel::Compute(CpuKernelContext &ctx) {
  CUST_KERNEL_LOG_INFO(ctx, "NonMaxSuppressionWithOverlaps kernel in.");
  uint32_t res = GetInputAndCheck(ctx);
  if (res != KERNEL_STATUS_OK) {
    return res;
  }
  res = DoNonMaxSuppressionWithOverlapsOp<float, float>(ctx);
  CUST_KERNEL_CHECK_FALSE(ctx, (res == KERNEL_STATUS_OK), res, "Compute failed.");
  CUST_KERNEL_LOG_INFO(ctx, "Compute end!!");
  return KERNEL_STATUS_OK;
}
