...
uint32_t MaskedScatterCpuKernel::Compute(CpuKernelContext &ctx) {
  // check params
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kMaskedScatterInputNum, kMaskedScatterOutputNum),
                           "[%s] check params failed.", kMaskedScatter);
  CUST_KERNEL_HANDLE_ERROR(ctx, InputCheck(ctx), "[%s] check input failed.", kMaskedScatter);
...
uint32_t MaskedScatterCpuKernel::InputCheck(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_CHECK_FALSE(ctx, mask_numElements > 0, KERNEL_STATUS_PARAM_INVALID,
                          "Number of elements of mask must > 0.");
  CUST_KERNEL_CHECK_FALSE(ctx, (input_Totaldim_a >= input_Totaldim_b), KERNEL_STATUS_PARAM_INVALID,
  ...
  for (int64_t i = 0; i < input_Totaldim_b; ++i) {
    CUST_KERNEL_CHECK_FALSE(ctx, (input_dims_b_inverse[i] == 1 || input_dims_b_inverse[i] == input_dims_a_inverse[i]),
...
uint32_t MaskedScatterCpuKernel::MaskedScatterCompute(CpuKernelContext &ctx) {
  ...
  if (x_shape == mask_shape) {
    ...
    for (int64_t i = 0; i < x_numElements; i++) {
      if (mask[i]) {
        CUST_KERNEL_CHECK_FALSE(ctx, j < updates_numElements, KERNEL_STATUS_PARAM_INVALID,
