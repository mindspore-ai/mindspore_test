...
#define Tanh_COMPUTE_CASE(DTYPE, TYPE, CTX)                      \
  case (DTYPE): {                                                \
    ...
    if (result != KERNEL_STATUS_OK) {                            \
      CUST_KERNEL_LOG_ERROR(ctx, "Tanh kernel compute failed."); \
      return result;                                             \
    }                                                            \
    break;                                                       \
  }
...
uint32_t TanhCpuKernel::Compute(CpuKernelContext &ctx) {
  // check params
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kInputNum, kOutputNum), "[%s] check input and output failed.", kTanh);
  ...
  switch (data_type) {
    Tanh_COMPUTE_CASE(DT_COMPLEX64, std::complex<float>, ctx)
      Tanh_COMPUTE_CASE(DT_COMPLEX128, std::complex<double>, ctx) Tanh_COMPUTE_CASE(DT_FLOAT, float, ctx)
        ...
        : CUST_KERNEL_LOG_ERROR(ctx, "Tanh kernel data type [%s] not support.", DTypeStr(data_type).c_str());
...
uint32_t TanhCpuKernel::TanhCompute(CpuKernelContext &ctx) {
  ...
  if (data_size <= kParallelDataNums) {
    ...
    CUST_KERNEL_HANDLE_ERROR(ctx, CpuKernelUtils::ParallelFor(ctx, data_num, data_num / max_core_num, shard_Tanh),
                             "Tanh Compute failed.");
  }
  return KERNEL_STATUS_OK;
}
