...
#define LESS_EQUAL_COMPUTE_CASE(DTYPE, TYPE, CTX)                      \
  case (DTYPE): {                                                      \
    ...
    if (result != KERNEL_STATUS_OK) {                                  \
      CUST_KERNEL_LOG_ERROR(ctx, "Less Equal kernel compute failed."); \
      return result;                                                   \
    }                                                                  \
    break;                                                             \
  }
...
uint32_t LessEqualCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, NormalCheck(ctx, kInputNum, kOutputNum),
                           "Less Equal check input and output number failed.");
  CUST_KERNEL_HANDLE_ERROR(ctx, LessEqualParamCheck(ctx), "Less Equal check params failed.");
  ...
  switch (data_type) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "Less Equal kernel data type [%s] not support.", DTypeStr(data_type).c_str());
...
template <typename T>
void LessEqualCpuKernel::SpecialCompute(CpuKernelContext &ctx, BcastShapeType type, int64_t start, int64_t end,
...
uint32_t LessEqualCpuKernel::NoBcastCompute(CpuKernelContext &ctx) {
  ...
  if (data_num >= kParallelDataNumSameShape) {
    ...
    if (max_core_num == 0) {
      CUST_KERNEL_LOG_ERROR(ctx, "max_core_num can not be zero");
    } else {
      CUST_KERNEL_HANDLE_ERROR(ctx, CpuKernelUtils::ParallelFor(ctx, data_num, data_num / max_core_num, sharder_less),
...
template <typename T>
uint32_t LessEqualCpuKernel::BcastCompute(CpuKernelContext &ctx, const Bcast &bcast) {
  auto in0 = reinterpret_cast<T *>(ctx.Input(0)->GetData());
  auto in1 = reinterpret_cast<T *>(ctx.Input(1)->GetData());
  auto out = reinterpret_cast<bool *>(ctx.Output(0)->GetData());
  int64_t data_num = ctx.Output(0)->NumElements();
