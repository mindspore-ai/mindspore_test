...
#define DO_COMPUTE_CASE(DTYPE, TYPE, ITYPE, CTX)                                               \
  case (DTYPE): {                                                                              \
    ...
    if (ret != KERNEL_STATUS_OK) {                                                             \
      CUST_KERNEL_LOG_ERROR(ctx, "SparseApplyProximalGradientDescent kernel compute failed."); \
      return ret;                                                                              \
    }                                                                                          \
    break;                                                                                     \
  }
...
uint32_t SparseApplyProximalGradientDescentCpuKernel::Compute(CpuKernelContext &ctx) {
  ...
  CUST_KERNEL_HANDLE_ERROR(ctx, ValidParam(ctx), "[%s] check params failed.", kSparseApplyProximalGradientDescent);
  ...
  switch (data_type) {
    ...
    default:
      CUST_KERNEL_LOG_ERROR(ctx, "SparseApplyProximalGradientDescent kernel data type[%s] not support.",
