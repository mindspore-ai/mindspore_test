 ...
 */
#include "kernel/ascend/aclnn/kernel_mod_impl/customize/cross_entropy_loss_aclnn_kernel.h"
#include <tuple>
#include <algorithm>
#include <vector>
#include <map>
...
void CrossEntropyLossAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  label_smoothing_ = static_cast<double>(device::ascend::ConvertKernelTensor<float>(inputs[kIndex5]));
  lse_square_scale_for_zloss_ = static_cast<double>(device::ascend::ConvertKernelTensor<float>(inputs[kIndex6]));
