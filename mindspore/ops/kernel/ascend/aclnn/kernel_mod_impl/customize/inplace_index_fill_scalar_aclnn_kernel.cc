 ...
 */
#include "kernel/ascend/aclnn/kernel_mod_impl/customize/inplace_index_fill_scalar_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <functional>
...
void InplaceIndexFillScalarAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
                                                    const std::vector<KernelTensor *> &outputs) {
  auto index_shape = inputs[kIndex2]->GetShapeVector();
  if (MS_UNLIKELY(index_shape.size() > 1)) {
    MS_LOG(EXCEPTION) << "For [InplaceIndexFillScalar], the rank of input 'index'"
  ...
  if (index_type_id == kNumberTypeInt64) {
    ...
    MS_LOG(EXCEPTION) << "For [InplaceIndexFillScalar], the input 'index'"
...
bool InplaceIndexFillScalarAscend::Launch(const std::vector<KernelTensor *> &inputs,
...
MS_ACLNN_KERNEL_FACTORY_REG(InplaceIndexFillScalar, InplaceIndexFillScalarAscend);
}  // namespace kernel
}  // namespace mindspore
