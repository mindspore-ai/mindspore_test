 ...
 */
#include "kernel/ascend/aclnn/kernel_mod_impl/customize/mse_loss_grad_ext_aclnn_kernel.h"
#include <vector>
...
void MSELossGradExtAclnnKernelMod::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  const std::vector<int64_t> &input_shape = inputs[kIndex1]->GetShapeVector();
  const std::vector<int64_t> &target_shape = inputs[kIndex2]->GetShapeVector();
  broadcast_shape_ = ops::CalBroadCastShapeV3(input_shape, target_shape);

  KernelTensor *input_ptr = inputs[kIndex1];
  KernelTensor *target_ptr = inputs[kIndex2];
  if (input_shape != broadcast_shape_) {
    SetExpandTensor(&input_expand_, inputs, kIndex1);
    input_ptr = &input_expand_;
    GetWorkspaceForResizeDoExpandInput(inputs[kIndex1], broadcast_shape_, &input_expand_);
    expand_indices_.emplace_back(kIndex1);
  }
  if (target_shape != broadcast_shape_) {
    SetExpandTensor(&target_expand_, inputs, kIndex2);
    target_ptr = &target_expand_;
    GetWorkspaceForResizeDoExpandTarget(inputs[kIndex2], broadcast_shape_, &target_expand_);
    expand_indices_.emplace_back(kIndex2);
  }

  GetWorkspaceForResizeMSELossGrad(inputs[kIndex0], input_ptr, target_ptr, reduction_value_, outputs[kIndex0]);
...
bool MSELossGradExtAclnnKernelMod::Launch(const std::vector<KernelTensor *> &inputs,
  ...
  for (size_t i = 0; i < expand_count_; i++) {
    ...
    if (expand_indices_[i] == kIndex1) {
      input_expand_.set_device_ptr(workspace[workspace.size() - expand_count_ + i]->device_ptr());
      output_ptr = &input_expand_;
      input_ptr = &input_expand_;
      RunOpDoExpandInput(stream_ptr, workspace, inputs[kIndex1], broadcast_shape_, output_ptr);
    } else {
      target_expand_.set_device_ptr(workspace[workspace.size() - expand_count_ + i]->device_ptr());
      output_ptr = &target_expand_;
      target_ptr = &target_expand_;
      RunOpDoExpandTarget(stream_ptr, workspace, inputs[kIndex2], broadcast_shape_, output_ptr);
    }
  }

  RunOpMSELossGrad(stream_ptr, workspace, inputs[kIndex0], input_ptr, target_ptr, reduction_value_, outputs[kIndex0]);
  return true;
}
