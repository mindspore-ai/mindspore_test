 ...
 */
#include "mindspore/ops/kernel/ascend/aclnn/kernel_mod_impl/customize/index_fill_scalar_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <functional>
...
std::vector<int64_t> IndexFillScalarAscend::GetIndexArray(const std::vector<KernelTensor *> &inputs) {
  ...
  switch (index_dtype_id) {
    ...
    default:
      MS_LOG(EXCEPTION) << "IndexFillScalar only support int32, int64, int16, int8, and uint8, but got "
