 ...
 */
#include "kernel/ascend/aclnn/kernel_mod_impl/customize/argmin_ext_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <functional>
...
void ArgMinAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  if (dim_is_none_) {
    ...
    int input_flatten_size = std::accumulate(input_real_shape_.begin(), input_real_shape_.end(),
