 ...
 */
#include "kernel/ascend/aclnn/kernel_mod_impl/customize/conv1d_padding_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <string>
...
std::vector<int64_t> Conv1DPaddingAscend::GetOriginStrides(const std::vector<int64_t> &shape) {
  if (shape.empty()) {
    return {};
  }
...
TensorStorageInfoPtr Conv1DPaddingAscend::CreateTensorStorageInfoPtr(
...
template <typename T>
void Conv1DPaddingAscend::SetTensorStorageInfo(T kernel_tensor, const ShapeVector &new_shape) {
...
void Conv1DPaddingAscend::SetExpandTensor(KernelTensor *input_tensor, const std::vector<KernelTensor *> &inputs) {
  input_tensor->SetType(inputs[kIndex0]->GetType());
  input_tensor->SetShape(std::make_shared<abstract::TensorShape>(pad_nd_shape_));
}

bool Conv1DPaddingAscend::CalcPaddingMode(std::vector<int64_t> &padding_l, std::vector<int64_t> &padding_r,
...
void Conv1DPaddingAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  if (padding_ == PadMode::SAME) {
    MS_LOG(INFO) << "Conv1DPaddingAscend: padmode is same";
    ...
    if (symmetric_padding) {
      MS_LOG(INFO) << "Conv1DPaddingAscend: symmetric padding.";
    ...
    MS_LOG(INFO) << "Conv1DPaddingAscend: padmode is valid";
...
bool Conv1DPaddingAscend::Launch(const std::vector<KernelTensor *> &inputs,
...
MS_ACLNN_KERNEL_FACTORY_REG(Conv1DPadding, Conv1DPaddingAscend);
}  // namespace conv1d_padding
}  // namespace kernel
}  // namespace mindspore
