...
#include "kernel/ascend/aclnn/pyboost_impl/customize/inplace_remainder_tensor_tensor.h"
#include <memory>
...
tensor::TensorPtr InplaceRemainderTensorTensorAscendCustomize(const std::shared_ptr<OpRunner> &op,
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, other_tensor]() {
    ...
    LAUNCH_ACLNN(aclnnInplaceRemainderTensorTensor, device_context, op->stream_id(), input_tensor, other_tensor);
