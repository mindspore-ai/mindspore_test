...
#include "kernel/ascend/aclnn/pyboost_impl/customize/inplace_div.h"
#include <memory>
...
namespace kernel {
namespace pyboost {
tensor::TensorPtr InplaceDivAscendCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_tensor,
                                            const TensorPtr &other_tensor) {
  MS_LOG(DEBUG) << "Call InplaceDivAscendCustomize start";
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, other_tensor]() {
    MS_LOG(DEBUG) << "Run device task InplaceDiv start";
    ...
    LAUNCH_ACLNN(aclnnInplaceDiv, device_context, op->stream_id(), input_tensor, other_tensor);
    MS_LOG(DEBUG) << "Run device task InplaceDiv end";
  }));
  return op->output(0);
}
}  // namespace pyboost
