...
#include "kernel/ascend/aclnn/pyboost_impl/customize/mm_ext.h"
#include <memory>
...
namespace kernel {
namespace pyboost {
tensor::TensorPtr MmAscendCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_tensor,
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, mat2_tensor]() {
    MS_LOG(DEBUG) << "Run device task Mm start";
    ...
    LAUNCH_ACLNN(aclnnMm, device_context, op->stream_id(), input_tensor, mat2_tensor, outputs[0], cube_math_type);
    MS_LOG(DEBUG) << "Run device task Mm end";
  }));
  return op->output(0);
}
}  // namespace pyboost
