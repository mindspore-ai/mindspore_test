...
#include "kernel/ascend/aclnn/pyboost_impl/customize/sub.h"
#include <memory>
...
tensor::TensorPtr SubAscendCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &x_tensor,
  ...
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, x_tensor, y_tensor]() {
    MS_LOG(DEBUG) << "Run device task Sub start";
    ...
    LAUNCH_ACLNN(aclnnSub, device_context, op->stream_id(), x_tensor, y_tensor, alpha, outputs[0]);
    MS_LOG(DEBUG) << "Run device task Sub end";
  }));
  return op->output(0);
}
}  // namespace pyboost
