...
#include "kernel/ascend/aclnn/pyboost_impl/customize/var.h"
...
tensor::TensorPtr VarAscendCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_tensor,
  ...
  PyBoostUtils::DispatchRun(
    std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, dim_vector, correction_imm, keepdim_imm]() {
      ...
      LAUNCH_ACLNN(aclnnVarCorrection, device_context, op->stream_id(), input_tensor, dim_vector, correction_imm,
                   keepdim_imm, outputs[0]);
