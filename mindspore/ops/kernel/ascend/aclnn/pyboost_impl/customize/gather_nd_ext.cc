...
tensor::TensorPtr GatherNdExtAscendCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_x,
  ...
  // Async
  PyBoostUtils::DispatchRun(std::make_shared<runtime::PyBoostDeviceTask>([op, input_x, indices, negative_index]() {
    auto device_context = op->device_context();
    const auto &outputs = op->outputs();
    // Malloc for input tensors
    PyBoostUtils::MallocOpInputs(device_context, input_x, indices);
    // Malloc for output tensors
    PyBoostUtils::MallocOpOutputs(device_context, outputs);
    MS_LOG(DEBUG) << op->primitive()->name() << " Call start";
    LAUNCH_ACLNN(aclnnGatherNd, device_context, op->stream_id(), input_x, indices, negative_index, outputs[0]);
