...
#include "kernel/ascend/aclnn/pyboost_impl/customize/conv3d_padding.h"
#include <memory>
#include <algorithm>
#include <string>
...
bool Conv3DPaddingBatchify(const ShapeVector &input_shape, const int64_t num_spatial_dims,
...
bool Conv3DGetSymmetricPadding(std::vector<int64_t> &padding_l, std::vector<int64_t> &padding_r,
...
tensor::TensorPtr Conv3DPaddingAscendCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_tensor,
  ...
  Conv3DExpandParamIfNeeded(&stride_vector, spatial_len);
  std::vector<int64_t> dilation_vector = ConvertValueTupleToVector<int64_t>(dilation);
  Conv3DExpandParamIfNeeded(&dilation_vector, spatial_len);
  // Convert ValuePtr to c++ scalar
  auto padding_enum_imm = GetValue<int64_t>(padding_enum);
  auto input_shape = input_tensor->shape();
  auto is_batchify = Conv3DPaddingBatchify(input_shape, 3, "conv3d");
  ...
  if (padding_enum_imm == PadMode::SAME) {
    ...
    bool symmetric_padding =
      Conv3DGetSymmetricPadding(padding_l, padding_r, stride_vector, dilation_vector, input_sizes, weight_sizes, dim);
