...
#include "kernel/ascend/aclnn/pyboost_impl/customize/unique_dim.h"
#include <memory>
...
std::tuple<tensor::TensorPtr, tensor::TensorPtr, tensor::TensorPtr> UniqueDimAscendCustomize(
  ...
  PyBoostUtils::DispatchRun(
    std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, sorted_imm, return_inverse_imm, dim_imm]() {
      ...
      const auto &all_acl_tensor =
        LAUNCH_ACLNN_SYNC(aclnnUniqueDim, device_context, op->stream_id(), input_tensor, sorted_imm, return_inverse_imm,
                          dim_imm, outputs[kIndex0], outputs[kIndex1], outputs[kIndex2]);
      // update shape
