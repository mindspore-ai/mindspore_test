...
#include "kernel/ascend/aclnn/pyboost_impl/customize/addmm.h"
#include <memory>
...
tensor::TensorPtr AddmmAscendCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_tensor,
  ...
  PyBoostUtils::DispatchRun(
    std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, mat1_tensor, mat2_tensor, beta, alpha]() {
      MS_LOG(DEBUG) << "Run device task Addmm start";
      ...
      LAUNCH_ACLNN(aclnnAddmm, device_context, op->stream_id(), input_tensor, mat1_tensor, mat2_tensor, beta, alpha,
                   outputs[0], cube_math_type);
      MS_LOG(DEBUG) << "Run device task Addmm end";
    }));
  return op->output(0);
}
}  // namespace pyboost
