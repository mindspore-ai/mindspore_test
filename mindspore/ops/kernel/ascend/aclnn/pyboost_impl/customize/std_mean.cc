...
#include "kernel/ascend/aclnn/pyboost_impl/customize/std_mean.h"
#include <memory>
...
void StdMeanAscendCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_tensor,
  ...
  PyBoostUtils::DispatchRun(
    std::make_shared<runtime::PyBoostDeviceTask>([op, input_tensor, dim_vector, correction_imm, keepdim_imm]() {
      MS_LOG(DEBUG) << "Run device task StdMean start";
      ...
      LAUNCH_ACLNN(aclnnStdMeanCorrection, device_context, op->stream_id(), input_tensor, dim_vector, correction_imm,
                   keepdim_imm, outputs[kIndex0], outputs[kIndex1]);
      MS_LOG(DEBUG) << "Run device task StdMean end";
    }));
}
}  // namespace pyboost
}  // namespace kernel
