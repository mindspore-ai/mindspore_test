 ...
 */
#include "kernel/ascend/opapi/aclnn/conv1d_ext_aclnn_kernel.h"
#include <algorithm>
#include <vector>
#include <memory>
#include <functional>
...
std::vector<int64_t> Conv1DExtAscend::GetOriStrides(const std::vector<int64_t> &shape) {
  if (shape.empty()) {
    return {};
  }
...
TensorStorageInfoPtr Conv1DExtAscend::CreateTensorStorageInfoPtr(const std::vector<int64_t> &new_shape,
...
template <typename T>
void Conv1DExtAscend::SetTensorStorageInfo(T kernel_tensor, ShapeVector new_shape) {
...
void Conv1DExtAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  _is_batchify = (in_shape.size() == weight_shape.size());
  if (_is_batchify) {
...
bool Conv1DExtAscend::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &workspace,
...
MS_ACLNN_KERNEL_FACTORY_REG(Conv1DExt, Conv1DExtAscend);
}  // namespace conv1d_ext
}  // namespace kernel
}  // namespace mindspore
