 ...
 */
#include "kernel/ascend/opapi/aclnn/addr_aclnn_kernel.h"
#include <vector>
#include "ir/tensor.h"
...
void AddrAclnnKernelMod::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
...
bool AddrAclnnKernelMod::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &workspace,
                                const std::vector<KernelTensor *> &outputs, void *stream_ptr) {
  MS_EXCEPTION_IF_NULL(stream_ptr);
  RunOp(stream_ptr, workspace, inputs[kIndex0], inputs[kIndex1], inputs[kIndex2], beta_, alpha_, outputs[kIndex0]);
  return true;
}
MS_ACLNN_KERNEL_FACTORY_REG(Addr, AddrAclnnKernelMod);
}  // namespace kernel
}  // namespace mindspore
