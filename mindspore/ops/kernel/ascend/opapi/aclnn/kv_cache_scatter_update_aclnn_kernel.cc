 ...
 */
#include "kernel/ascend/opapi/aclnn/kv_cache_scatter_update_aclnn_kernel.h"
#include "ir/tensor.h"
...
bool KVCacheScatterUpdateAscend::Launch(const std::vector<KernelTensor *> &inputs,
                                        const std::vector<KernelTensor *> &workspace,
                                        const std::vector<KernelTensor *> &outputs, void *stream_ptr) {
  MS_EXCEPTION_IF_NULL(stream_ptr);
  RunOp(stream_ptr, workspace, outputs[kIndex0], inputs[kIndex1], inputs[kIndex2], axis_);
  return true;
}
