 ...
 */
#include "kernel/ascend/opapi/aclnn/upsample_nearest2d_grad_aclnn_kernel.h"
...
void UpsampleNearest2DGradAscend::GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
  ...
  GetWorkspaceForResize(inputs[kIndex0], output_size_, input_size_, scales_h_, scales_w_, outputs[kIndex0]);
}
...
bool UpsampleNearest2DGradAscend::Launch(const std::vector<KernelTensor *> &inputs,
  ...
  RunOp(stream_ptr, workspace, inputs[kIndex0], output_size_, input_size_, scales_h_, scales_w_, outputs[kIndex0]);
  return true;
}
