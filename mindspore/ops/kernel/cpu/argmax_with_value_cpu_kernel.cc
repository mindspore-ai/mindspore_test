...
bool check_validation(const std::vector<size_t> &shape, const size_t num_before_axis, const size_t num_after_axis,
  ...
  size_t index_size = output_num * sizeof(int64_t);
  size_t value_size = output_num * data_size;
  ...
  if (outputs[index_output_idx]->size() != index_size) {
    MS_LOG(EXCEPTION) << "For '" << kKernelName << "', the memory size of the 1st output must be " << index_size
                      << ", but got the memory size is " << outputs[0]->size();
  }
  if (outputs[value_output_idx]->size() != value_size) {
    MS_LOG(EXCEPTION) << "For '" << kKernelName << "', the memory size of the 2nd output must be " << value_size
...
bool ArgMaxWithValueCpuKernelMod::LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
  ...
  (void)check_validation<T>(shape_, num_before_axis_, num_after_axis_, inputs, outputs, index_output_idx,
                            value_output_idx);

  auto input = reinterpret_cast<T *>(inputs[0]->device_ptr());
  auto index = reinterpret_cast<int64_t *>(outputs[index_output_idx]->device_ptr());
  auto value = reinterpret_cast<T *>(outputs[value_output_idx]->device_ptr());
  ...
  auto task = [&](size_t start, size_t end) {
    for (size_t pos = start; pos < end; pos++) {
      ...
      index[dst_index] = max_index;
      value[dst_index] = max_value;
    }
  };
