...
class LuSolveCpuKernelMod : public NativeCpuKernelMod {
 ...
 private:
  ...
  void LuSolve(const std::vector<kernel::KernelTensor *> &inputs, const std::vector<kernel::KernelTensor *> &outputs,
               T1 *b_working_ptr, T1 *lu_working_ptr, int32_t *pivots_working_ptr, size_t b_stride, size_t a);
  template <typename T, typename S>
  bool LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
                    const std::vector<kernel::KernelTensor *> &outputs);
  using LuSolveFunc = std::function<bool(LuSolveCpuKernelMod *, const std::vector<kernel::KernelTensor *> &,
