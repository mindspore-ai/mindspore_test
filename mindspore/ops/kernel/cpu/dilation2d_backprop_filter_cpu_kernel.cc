 ...
 */
#include "kernel/cpu/dilation2d_backprop_filter_cpu_kernel.h"
#include <limits>
...
constexpr size_t kOutputIdxf = 0;
...
int Dilation2DBackpropFilterCpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
  ...
  f_output_shape_ = outputs[kOutputIdxf]->GetShapeVector();
  CheckKernelParam();
  return KRET_OK;
}
...
bool Dilation2DBackpropFilterCpuKernelMod::LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
  ...
  T *output = static_cast<T *>(outputs[kOutputIdxf]->device_ptr());
