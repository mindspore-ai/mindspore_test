...
bool BesselY1CpuKernelMod::Init(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs) {
  ...
  switch (input_dtype_) {
    ...
    case kNumberTypeFloat16:
      kernel_func_ = &BesselY1CpuKernelMod::LaunchKernel<Eigen::half>;
      break;
    default:
...
int BesselY1CpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
...
template <typename T>
bool BesselY1CpuKernelMod::LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
  ...
  auto task = [this, &input, &output](size_t start, size_t end) {
    ...
    output.slice(offsets, extends) = input.slice(offsets, extends).bessel_y1();
  };
