...
#include "kernel/cpu/eigen/bessel_i1e_cpu_kernel.h"

#include "mindspore/ops/infer/bessel_i1e.h"
...
bool BesselI1eCpuKernelMod::Init(const std::vector<KernelTensor *> &inputs,
...
int BesselI1eCpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
                                  const std::vector<KernelTensor *> &outputs) {
  int ret = 0;
...
template <typename T>
bool BesselI1eCpuKernelMod::LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
