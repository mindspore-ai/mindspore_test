...
#include "kernel/cpu/eigen/bessel_i0_cpu_kernel.h"

#include "mindspore/ops/infer/bessel_i0.h"
...
bool BesselI0CpuKernelMod::Init(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs) {
  ...
  switch (input_dtype_) {
    ...
    case kNumberTypeFloat16:
      kernel_func_ = &BesselI0CpuKernelMod::LaunchKernel<Eigen::half>;
      break;
    default:
...
int BesselI0CpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
...
bool BesselI0CpuKernelMod::LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
  ...
  auto task = [this, &input, &output](size_t start, size_t end) {
    ...
    output.slice(offsets, extends) = input.slice(offsets, extends).bessel_i0();
  };
