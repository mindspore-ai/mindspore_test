...
#include "mindspore/ops/kernel/cpu/pyboost/customize/divs.h"
#include "mindspore/ops/kernel/cpu/pyboost/auto_generate/div.h"

namespace mindspore {
namespace kernel {
namespace pyboost {
void DivsCPUCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_tensor, const ScalarPtr &other) {
  ...
  // the Divs primitive does not support CPU, so use Div instead.
  const auto div_op = CREATE_PYBOOST_OP(Div, device_name);

  // handle type promotion manually since the CPU kernelmod Div does not support it
