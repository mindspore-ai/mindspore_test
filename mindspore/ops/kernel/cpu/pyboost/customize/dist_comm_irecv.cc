...
#include "mindspore/ops/kernel/cpu/pyboost/customize/dist_comm_irecv.h"
#include <memory>
#include <utility>
#include <string>
...
void DistCommIrecvCPUCustomize(const std::shared_ptr<OpRunner> &op, const TensorPtr &input_tensor,
  ...
  auto run_func = [op, input_tensor, src, group]() {
    ...
    bool ret = comm_lib->Recv(in_addr[0]->device_ptr(), in_addr[0]->size() / type_len, in_addr[0]->dtype_id(), src_rank,
                              group_str);
    if (!ret) {
      MS_LOG(EXCEPTION) << "Recv failed.";
    }
  };
