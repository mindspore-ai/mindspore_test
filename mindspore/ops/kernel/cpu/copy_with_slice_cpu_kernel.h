...
class CopyWithSliceCpuKernel : public NativeCpuKernelMod {
 ...
 private:
  using CopyWithSliceFunc =
    std::function<bool(CopyWithSliceCpuKernel *, const TensorStorageInfoPtr &, const device::DeviceAddressPtr &,
                       const TensorStorageInfoPtr &, const device::DeviceAddressPtr &)>;
