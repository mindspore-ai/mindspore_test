...
#include "kernel/cpu/correlate_cpu_kernel.h"
#include <functional>
#include <algorithm>
#include <utility>
#include <memory>
...
bool CorrelateCpuKernelMod::LaunchKernel(const std::vector<kernel::KernelTensor *> &inputs,
  ...
  auto task = [&long_array, &short_array, &out_array, this](int64_t start, int64_t end) {
    for (int64_t out_id = start; out_id < end; ++out_id) {
      ...
      for (int64_t dot_id = 0; dot_id < short_size_; dot_id++) {
        sum_temp += long_array[SizeToLong(out_id) + dot_id] * short_array[dot_id];
      }
