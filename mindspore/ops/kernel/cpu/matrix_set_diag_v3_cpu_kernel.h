...
class MatrixSetDiagV3CpuKernelMod : public NativeCpuKernelMod {
 ...
 private:
  ...
  using MatrixSetDiagV3Func =
    std::function<bool(MatrixSetDiagV3CpuKernelMod *, const std::vector<kernel::KernelTensor *> &,
  ...
  template <typename T>
  void singleCal(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs);
