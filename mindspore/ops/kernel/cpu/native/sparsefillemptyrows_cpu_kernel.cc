...
#include "kernel/cpu/native/sparsefillemptyrows_cpu_kernel.h"
#include <Eigen/Dense>
...
void SparseFillEmptyRowsCpuKernelMod::UpdateOutputShapeAndSize(const std::vector<KernelTensor *> &inputs,
  ...
  size_t out_indice_batch = LongToSize(std::accumulate(out_indice_shape.cbegin(), out_indice_shape.cend(),
                                                       static_cast<int64_t>(1), std::multiplies<int64_t>()));
  auto out_indice_dtype_size = GetTypeByte(TypeIdToType(output_indices_type_));
  size_t out_values_batch = LongToSize(std::accumulate(out_values_shape.cbegin(), out_values_shape.cend(),
                                                       static_cast<int64_t>(1), std::multiplies<int64_t>()));
  auto out_values_dtype_size = GetTypeByte(TypeIdToType(output_values_type_));
  size_t out_empty_row_indicator_batch =
    LongToSize(std::accumulate(out_empty_row_indicator_shape_.cbegin(), out_empty_row_indicator_shape_.cend(),
                               static_cast<int64_t>(1), std::multiplies<int64_t>()));
  ...
  size_t out_reverse_index_batch =
    LongToSize(std::accumulate(out_reverse_index_shape_.cbegin(), out_reverse_index_shape_.cend(),
                               static_cast<int64_t>(1), std::multiplies<int64_t>()));
