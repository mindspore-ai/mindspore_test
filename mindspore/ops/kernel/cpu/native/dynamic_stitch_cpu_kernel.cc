...
#include "kernel/cpu/native/dynamic_stitch_cpu_kernel.h"
#include <functional>
#include <algorithm>
#include <utility>
...
void DynamicStitchCpuKernelMod::UpdateOutputShapeAndSize(const std::vector<KernelTensor *> &inputs,
  ...
  size_t batch_size = LongToSize(
    std::accumulate(result_shape_.cbegin(), result_shape_.cend(), static_cast<int64_t>(1), std::multiplies<int64_t>()));
