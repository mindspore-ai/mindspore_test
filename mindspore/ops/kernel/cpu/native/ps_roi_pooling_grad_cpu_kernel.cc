...
#include <functional>
#include <map>
#include "kernel/cpu/native/ps_roi_pooling_grad_cpu_kernel.h"

#include "kernel/cpu/native/atomic_add.h"
...
int PSROIPoolingGradCpuKernelMod::ResizeCheckInputs(const std::vector<KernelTensor *> &inputs) const {
  ...
  auto dy_type = inputs[kIndex0]->dtype_id();
  ...
  auto rois_type = inputs[kIndex1]->dtype_id();
...
int PSROIPoolingGradCpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
  ...
  data_type_id_ = inputs[kIndex0]->dtype_id();
  auto rois_shape = inputs[kIndex1]->GetShapeVector();
