 ...
 */
#include "kernel/cpu/native/apply_proximal_gradient_descent_cpu_kernel.h"
#include <algorithm>
#include <functional>
...
int ApplyProximalGradientDescentCpuKernelMod::Resize(const std::vector<KernelTensor *> &inputs,
  ...
  input_elements_ =
    static_cast<size_t>(std::accumulate(var_shape.begin(), var_shape.end(), 1, std::multiplies<int64_t>()));
  if (batch_size_ <= 0) {
    MS_LOG(ERROR) << "For '" << kernel_name_
