...
TypePtr HeavisideType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  std::map<std::string, TypePtr> inputs_dtype = {{"x", x_dtype}, {"values", values_dtype}};
  (void)CheckAndConvertUtils::CheckScalarOrTensorTypesSame(inputs_dtype, valid_types, primitive->name(), false);
  return x_dtype;
}
}  // namespace

MIND_API_OPERATOR_IMPL(Heaviside, BaseOperator);
class OPS_API HeavisideInfer : public abstract::OpInferBase {
 public:
