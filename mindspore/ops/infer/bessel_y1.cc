...
namespace ops {
namespace {
abstract::ShapePtr BesselY1InferShape(const PrimitivePtr &, const std::vector<AbstractBasePtr> &input_args) {
...
class OPS_API AGBesselY1Infer : public abstract::OpInferBase {
 public:
  BaseShapePtr InferShape(const PrimitivePtr &primitive,
                          const std::vector<AbstractBasePtr> &input_args) const override {
    return BesselY1InferShape(primitive, input_args);
  }

  TypePtr InferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) const override {
    return BesselY1InferType(primitive, input_args);
  }
  AbstractBasePtr InferShapeAndType(const abstract::AnalysisEnginePtr &engine, const PrimitivePtr &primitive,
                                    const std::vector<AbstractBasePtr> &input_args) const override {
    return BesselY1Infer(engine, primitive, input_args);
  }
};

REGISTER_PRIMITIVE_OP_INFER_IMPL(BesselY1, prim::kPrimBesselY1, AGBesselY1Infer, false);
}  // namespace ops
}  // namespace mindspore
