 ...
 */
#include "infer/uniform_candidate_sampler.h"
...
abstract::TupleShapePtr UCSInferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  if (is_ascend && IsDynamic(input_shape)) {
    MS_EXCEPTION(ValueError)
      << "LogUniformCandidateSampler does not support dynamic shape scenarios on Ascend currently.";
  }
...
class OPS_API AGUniformCandidateSamplerInfer : public abstract::OpInferBase {
 public:
