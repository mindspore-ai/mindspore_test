...
#include "infer/ops_func_impl/apply_power_sign.h"
...
namespace ops {
namespace {
BaseShapePtr ApplyPowerSignInferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  auto logbase_shape = CheckAndConvertUtils::ConvertShapePtrToShapeMap(input_args[kInputIndex3]->GetShape())[kShape];
...
TypePtr ApplyPowerSignInferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  MS_EXCEPTION_IF_NULL(primitive);
  auto prim_name = primitive->name();
...
BaseShapePtr ApplyPowerSignFuncImpl::InferShape(const PrimitivePtr &primitive,
                                                const std::vector<AbstractBasePtr> &input_args) const {
  return ApplyPowerSignInferShape(primitive, input_args);
}

TypePtr ApplyPowerSignFuncImpl::InferType(const PrimitivePtr &primitive,
                                          const std::vector<AbstractBasePtr> &input_args) const {
  return ApplyPowerSignInferType(primitive, input_args);
}
}  // namespace ops
}  // namespace mindspore
