...
#include "infer/ops_func_impl/lamb.h"
...
TypePtr LambInferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  const std::set<TypePtr> float_set = {kFloat32};
...
BaseShapePtr LambFuncImpl::InferShape(const PrimitivePtr &primitive,
                                      const std::vector<AbstractBasePtr> &input_args) const {
  return LambInferShape(primitive, input_args);
}

TypePtr LambFuncImpl::InferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) const {
