...
#include "infer/ops_func_impl/apply_centered_rms_prop.h"
...
namespace ops {
namespace {
BaseShapePtr ApplyCenteredRMSPropInferShape(const PrimitivePtr &primitive,
...
TypePtr ApplyCenteredRMSPropInferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  auto mean_gradient_dtype = input_args[1]->GetType();
  auto mean_square_dtype = input_args[2]->GetType();
  auto moment_dtype = input_args[3]->GetType();
  auto grad_dtype = input_args[4]->GetType();
  auto learning_rate_dtype = input_args[5]->GetType();
  auto decay_dtype = input_args[6]->GetType();
  auto momentum_dtype = input_args[7]->GetType();
  auto epsilon_dtype = input_args[8]->GetType();
...
BaseShapePtr ApplyCenteredRMSPropFuncImpl::InferShape(const PrimitivePtr &primitive,
                                                      const std::vector<AbstractBasePtr> &input_args) const {
  return ApplyCenteredRMSPropInferShape(primitive, input_args);
}

TypePtr ApplyCenteredRMSPropFuncImpl::InferType(const PrimitivePtr &primitive,
                                                const std::vector<AbstractBasePtr> &input_args) const {
