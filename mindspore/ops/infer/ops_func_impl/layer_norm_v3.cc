...
namespace ops {
namespace {
ShapeVector CalLayerNormV3MeanAndVarShape(int64_t begin_norm_axis, const ShapeVector &input_shape) {
...
int64_t CheckAxisV3(const std::string &op, const std::string &args_name, int64_t axis_value, int64_t minimum,
...
BaseShapePtr LayerNormV3FuncImpl::InferShape(const PrimitivePtr &primitive,
  ...
  MS_CHECK_VALUE(
    ...
    CheckAndConvertUtils::FormatCommMsg("For 'LayerNormV3', evaluator gamma or beta can not be an AbstractScalar."));
  ...
  int64_t begin_norm_axis = CheckAxisV3(op_name, "begin_norm_axis", bna_value, -1, SizeToLong(x_rank), "input_x");

  std::vector<BaseShapePtr> shapes_list = {x_shape_ptr};
  auto mean_var_shape = CalLayerNormV3MeanAndVarShape(begin_norm_axis, x_shape);
...
TypePtr LayerNormV3FuncImpl::InferType(const PrimitivePtr &primitive,
                                       const std::vector<AbstractBasePtr> &input_args) const {
  ...
  std::vector<TypePtr> types_list = {x_type, gamma_type, beta_type};
