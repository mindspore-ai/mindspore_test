...
ShapeArray CalOutShape(const InferInfoPtr &ksize_ptr, const InferInfoPtr &strides_ptr, const InferInfoPtr &pads_ptr,
  ...
  if (!(ksize_opt.has_value() && strides_opt.has_value() && pads_opt.has_value())) {
    ...
    int64_t out_h = static_cast<int64_t>((out_shape[size - 2] - 1) * strides[kInputIndex0] - 2 * pads[kInputIndex0] +
                                         ksize[kInputIndex0]);
    int64_t out_w = static_cast<int64_t>((out_shape[size - 1] - 1) * strides[kInputIndex1] - 2 * pads[kInputIndex1] +
                                         ksize[kInputIndex1]);
...
ShapeArray MaxUnpool2DExtFuncImpl::InferShape(const PrimitivePtr &primitive,
  ...
  if (input_infos[kInputIndex0]->IsDynamic() && input_infos[kInputIndex1]->IsDynamic()) {
    if (output_shape_opt.has_value() && output_shape_opt.value().size() >= 2 &&
        !output_shape_opt.value().HasUnknownValue()) {
      auto output_shape_val = output_shape_opt.value();
      out_shape[last_dim - 1] = output_shape_val[size - 2];
      out_shape[last_dim] = output_shape_val[size - 1];
    } else {
      out_shape[last_dim - 1] = -1;
      out_shape[last_dim] = -1;
    }
  ...
  if (output_shape_opt.has_value() && output_shape_opt.value().size() >= 2 &&
    ...
    auto min_size_h = out_shape[last_dim - 1] - strides_out[kInputIndex0];
    auto max_size_h = out_shape[last_dim - 1] + strides_out[kInputIndex0];
    ...
    if ((min_size_h < attr_output_shape[size - 2] && attr_output_shape[size - 2] < max_size_h) &&
        (min_size_w < attr_output_shape[size - 1] && attr_output_shape[size - 1] < max_size_w)) {
      out_shape[last_dim - 1] = attr_output_shape[size - 2];
      out_shape[last_dim] = attr_output_shape[size - 1];
    }
  } else {
    out_shape[last_dim - 1] = -1;
    out_shape[last_dim] = -1;
