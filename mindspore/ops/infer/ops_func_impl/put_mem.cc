 ...
 */
#include "infer/ops_func_impl/put_mem.h"
#include <vector>
#include <memory>
#include <string>
#include <utility>
...
ShapeArray PutMemFuncImpl::InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const {
  return {input_infos[kInputIndex0]->GetShape()};
}

std::vector<TypeId> PutMemFuncImpl::InferType(const PrimitivePtr &primitive,
  ...
  if (std::find(target_support_dtype.begin(), target_support_dtype.end(), input_infos[kInputIndex0]->GetType()) ==
    ...
    MS_LOG(EXCEPTION) << "For 'PutMem', the 'target' must be Tensor(int32, int64, float16, bfloat16, float32), "
                         "but got: "
                      << input_infos[kInputIndex0]->GetType();
  }
  if (input_infos[kInputIndex0]->GetType() != input_infos[kInputIndex2]->GetType()) {
    MS_LOG(EXCEPTION) << "For 'PutMem', the 'target' and 'src' must be same dtype, but got: "
                      << input_infos[kInputIndex0]->GetType() << " and " << input_infos[kInputIndex2]->GetType();
  }
  std::vector<size_t> check_indices = {kInputIndex1, kInputIndex3, kInputIndex4};
  for (const auto &index : check_indices) {
    if (index < input_infos.size() && input_infos[index]->GetType() != kNumberTypeInt64) {
      MS_LOG(EXCEPTION) << "For 'PutMem', the target_offset, src_offset, size must be int64, but got: "
