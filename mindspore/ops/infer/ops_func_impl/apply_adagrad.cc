...
#include "infer/ops_func_impl/apply_adagrad.h"
...
namespace mindspore {
namespace ops {
BaseShapePtr ApplyAdagradFuncImpl::InferShape(const PrimitivePtr &primitive,
...
TypePtr ApplyAdagradFuncImpl::InferType(const PrimitivePtr &primitive,
                                        const std::vector<AbstractBasePtr> &input_args) const {
  MS_EXCEPTION_IF_NULL(primitive);
  auto prim_name = primitive->name();
