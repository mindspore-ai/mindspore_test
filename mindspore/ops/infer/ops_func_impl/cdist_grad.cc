...
#include "infer/ops_func_impl/cdist_grad.h"
#include <memory>
#include <set>
...
static inline bool CdistGradIsValidType(TypeId t) {
...
ShapeArray CdistGradFuncImpl::InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const {
  ...
  auto grad_shape = input_infos[kInputIndex0]->GetShape();
  auto x_shape = input_infos[kInputIndex1]->GetShape();
  auto y_shape = input_infos[kInputIndex2]->GetShape();
  auto cdist_shape = input_infos[kInputIndex3]->GetShape();
...
std::vector<TypeId> CdistGradFuncImpl::InferType(const PrimitivePtr &primitive,
                                                 const InferInfoPtrList &input_infos) const {
  auto prim_name = primitive->name();
  const auto grad_type = input_infos[kInputIndex0]->GetType();
  const auto input_x_type = input_infos[kInputIndex1]->GetType();
  const auto input_y_type = input_infos[kInputIndex2]->GetType();
  const auto cdist_type = input_infos[kInputIndex3]->GetType();
  if (grad_type != input_x_type || grad_type != input_y_type || grad_type != cdist_type) {
    MS_EXCEPTION(TypeError) << "For Primitive[" << prim_name << "], the input type must be same.\n"
                            << "name:[grad]:Tensor[" << TypeIdToString(grad_type) << "].\n"
                            << "name:[input_x]:Tensor[" << TypeIdToString(input_x_type) << "].\n"
                            << "name:[input_y]:Tensor[" << TypeIdToString(input_y_type) << "].\n"
                            << "name:[cdist]:Tensor[" << TypeIdToString(cdist_type) << "].\n";
  }
  if (!CdistGradIsValidType(input_x_type)) {
