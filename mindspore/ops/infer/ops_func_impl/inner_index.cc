...
std::vector<TypeId> InnerIndexFuncImpl::InferType(const PrimitivePtr &primitive,
...
ShapeVector InnerIndexFuncImpl::CheckAndCalOutputShapeInTupleCase(const ShapeVector &x_shape,
                                                                  const ShapeArray &indices_shapes) const {
  // 1. Get the expanded index shape.
  if (x_shape.size() < indices_shapes.size()) {
    MS_EXCEPTION(ValueError) << "For 'InnerIndex', too many indices for tensor of dimension " << x_shape.size()
                             << " (got " << indices_shapes.size() << ")";
  }
  ...
  for (size_t dim = 0; dim < fin_index_shapes.size(); dim++) {
    ...
    if (idx_shape.size() == kInnerIndexEmptyShape &&
  ...
  if (std::find(indexed_sizes.begin(), indexed_sizes.end(), 0) != indexed_sizes.end() &&
    ...
    MS_EXCEPTION(ValueError) << "For 'InnerIndex', if the input tensor of dimension with size 0"
...
std::tuple<ShapeVector, ShapeArray> InnerIndexFuncImpl::TransposeToFront(const ShapeVector &x_shape,
...
bool InnerIndexFuncImpl::IndexContiguous(const ShapeArray &index_shape) const {
  auto isEmpty = [](const ShapeVector &idx_shape) {
    return idx_shape.size() == kInnerIndexEmptyShape &&
           std::all_of(idx_shape.begin(), idx_shape.end(), [](int i) { return i == 0; });
  };
...
ShapeArray InnerIndexFuncImpl::ExpandIndexShape(const ShapeArray &to_expand) const {
  ...
  for (size_t i = 0; i < to_expand.size(); ++i) {
    ...
    if (elem_shape.size() == kInnerIndexEmptyShape &&
