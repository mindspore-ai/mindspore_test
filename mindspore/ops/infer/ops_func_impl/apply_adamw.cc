 ...
 */
#include "infer/ops_func_impl/apply_adamw.h"
...
namespace mindspore {
namespace ops {
BaseShapePtr ApplyAdamWFuncImpl::InferShape(const PrimitivePtr &primitive,
                                            const std::vector<AbstractBasePtr> &input_args) const {
  const int64_t adamw_size = 14;
  ...
  auto grad_shape_ptr = input_args[kInputIndex10]->GetShape();
...
TypePtr ApplyAdamWFuncImpl::InferType(const PrimitivePtr &prim, const std::vector<AbstractBasePtr> &input_args) const {
