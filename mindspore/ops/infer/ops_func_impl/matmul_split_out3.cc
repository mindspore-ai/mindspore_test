...
#include "infer/ops_func_impl/matmul_split_out3.h"
...
// MatmulSplitOut3
BaseShapePtr MatmulSplitOut3FuncImpl::InferShape(const PrimitivePtr &primitive,
                                                 const std::vector<AbstractBasePtr> &input_args) const {
  return MatmulFusionUtils::InferenceMultiMatmulInferShape(primitive, input_args);
}

TypePtr MatmulSplitOut3FuncImpl::InferType(const PrimitivePtr &primitive,
                                           const std::vector<AbstractBasePtr> &input_args) const {
