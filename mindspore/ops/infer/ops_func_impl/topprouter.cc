...
#include "infer/ops_func_impl/topprouter.h"
#include <algorithm>
#include <memory>
#include <set>
#include <utility>
...
BaseShapePtr TopPRouterFuncImpl::InferShape(const PrimitivePtr &primitive,
...
TypePtr TopPRouterFuncImpl::InferType(const PrimitivePtr &primitive,
                                      const std::vector<abstract::AbstractBasePtr> &input_args) const {
  auto prim_name = primitive->name();
  const std::set<TypePtr> valid_types = {kInt32, kInt64};
  std::set<TypePtr> scala_valid_types{kInt64};
  std::set<TypePtr> threshold_valid_types{kFloat32};
  ...
  auto threshold_type = input_args[kInputIndex4]->GetType();
  auto router_prob_type = input_args[kInputIndex5]->GetType();
  ...
  (void)CheckAndConvertUtils::CheckTypeValid("threshold", threshold_type, threshold_valid_types, prim_name);
  (void)CheckAndConvertUtils::CheckTypeValid("router_prob", router_prob_type, threshold_valid_types, prim_name);
