...
namespace mindspore {
namespace ops {
BaseShapePtr CummaxFuncImpl::InferShape(const PrimitivePtr &primitive,
...
TypePtr CummaxFuncImpl::InferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) const {
  auto x_type = input_args[kInputIndex0]->GetType();
  return std::make_shared<Tuple>(std::vector{x_type, kInt64});
}

TypePtrList CummaxFuncImpl::InferType(const PrimitivePtr &primitive, const ValuePtrList &input_values) const {
  const auto &input_tensor = input_values[kInputIndex0]->cast<tensor::TensorPtr>();
  MS_EXCEPTION_IF_NULL(input_tensor);
  return {input_tensor->Dtype(), kInt64};
}

ShapeArray CummaxFuncImpl::InferShape(const PrimitivePtr &primitive, const ValuePtrList &input_values) const {
...
REGISTER_SIMPLE_INFER(kNameCummax, CummaxFuncImpl)
}  // namespace ops
}  // namespace mindspore
