...
int32_t DropoutExtFuncImpl::CheckValidation(const PrimitivePtr &primitive,
  ...
  const auto &p_opt = GetScalarValue<pyfloat>(input_args[kIndex1]->GetValue());
  if (MS_UNLIKELY(!p_opt.has_value())) {
    return OP_CHECK_RETRY;
  }
  MS_CHECK_VALUE(p_opt.value() >= static_cast<pyfloat>(0.0) && p_opt.value() <= static_cast<pyfloat>(1.0),
