...
#include "infer/ops_func_impl/logsigmoid.h"
#include <algorithm>
#include <memory>
#include <set>
#include <utility>
...
namespace mindspore {
namespace ops {
ShapeArray LogSigmoidFuncImpl::InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const {
  auto &input = input_infos[kInputIndex0];
  return {input->GetShape(), input->GetShape()};
}

std::vector<TypeId> LogSigmoidFuncImpl::InferType(const PrimitivePtr &primitive,
