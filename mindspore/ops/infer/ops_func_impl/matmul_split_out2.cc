...
#include "infer/ops_func_impl/matmul_split_out2.h"
...
BaseShapePtr MatmulSplitOut2FuncImpl::InferShape(const PrimitivePtr &primitive,
                                                 const std::vector<AbstractBasePtr> &input_args) const {
  return MatmulFusionUtils::InferenceMultiMatmulInferShape(primitive, input_args);
}

TypePtr MatmulSplitOut2FuncImpl::InferType(const PrimitivePtr &primitive,
                                           const std::vector<AbstractBasePtr> &input_args) const {
