...
namespace mindspore {
namespace ops {
BaseShapePtr ResizeNearestNeighborGradFuncImpl::InferShape(const PrimitivePtr &primitive,
                                                           const std::vector<AbstractBasePtr> &input_args) const {
  const auto &grads_shape = input_args[kInputIndex0]->GetShape()->GetShapeVector();
  ...
  if (IsDynamicRank(grads_shape)) {
    ...
    MS_CHECK_VALUE(grads_shape.size() == shape_size,
                   CheckAndConvertUtils::FormatCommMsg("For '", primitive->name(),
                                                       "', the dimension of input_grads should equal to ", shape_size,
                                                       " but got ", grads_shape.size()));
    output_shape.insert(output_shape.end(), grads_shape.begin(), grads_shape.begin() + size_size);
  }
