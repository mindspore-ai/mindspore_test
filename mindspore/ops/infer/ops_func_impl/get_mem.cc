 ...
 */
#include "infer/ops_func_impl/get_mem.h"
#include <vector>
#include <memory>
#include <string>
#include <utility>
...
ShapeArray GetMemFuncImpl::InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const {
  return {input_infos[kInputIndex0]->GetShape()};
}

std::vector<TypeId> GetMemFuncImpl::InferType(const PrimitivePtr &primitive,
  ...
  if (std::find(target_support_dtype.begin(), target_support_dtype.end(), input_infos[kInputIndex0]->GetType()) ==
    ...
    MS_LOG(EXCEPTION) << "For 'GetMem', the 'target' must be Tensor(int32, int64, float16, bfloat16, float32), "
                         "but got: "
                      << input_infos[kInputIndex0]->GetType();
  }
  if (input_infos[kInputIndex0]->GetType() != input_infos[kInputIndex2]->GetType()) {
    MS_LOG(EXCEPTION) << "For 'GetMem', the 'target' and 'src' must be same dtype, but got: "
  ...
  for (const auto &index : check_indices) {
    if (index < input_infos.size() && input_infos[index]->GetType() != kNumberTypeInt64) {
      MS_LOG(EXCEPTION) << "For 'GetMem', the target_offset, src_offset, size must be int64, but got: "
