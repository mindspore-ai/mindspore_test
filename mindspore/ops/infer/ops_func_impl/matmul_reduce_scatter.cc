...
ShapeArray MatmulReduceScatterFuncImpl::InferShape(const PrimitivePtr &primitive,
  ...
  auto &input_tensor = input_infos[kMatmulReduceScatterInputInputIndex];
...
std::vector<TypeId> MatmulReduceScatterFuncImpl::InferType(const PrimitivePtr &primitive,
                                                           const InferInfoPtrList &input_infos) const {
  auto op_name = primitive->name();
  ...
  if (!input_infos[kMatmulReduceScatterInputBiasIndex]->IsNone()) {
