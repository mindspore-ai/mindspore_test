...
#include "infer/ops_func_impl/apply_adadelta.h"
...
namespace ops {
namespace {
BaseShapePtr ApplyAdadeltaInferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
...
TypePtr ApplyAdadeltaInferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  auto prim_name = primitive->name();
  ...
  (void)CheckAndConvertUtils::CheckTensorTypeSame(args_rho, valid_types, prim_name);

  std::map<std::string, TypePtr> args_epsilon;
  (void)args_epsilon.insert(std::make_pair("epsilon_type", epsilon_type));
  (void)CheckAndConvertUtils::CheckTensorTypeSame(args_epsilon, valid_types, prim_name);

  return std::make_shared<Tuple>(std::vector<TypePtr>{var_type, accum_type, accum_update_type});
}
}  // namespace
BaseShapePtr ApplyAdadeltaFuncImpl::InferShape(const PrimitivePtr &primitive,
                                               const std::vector<AbstractBasePtr> &input_args) const {
  return ApplyAdadeltaInferShape(primitive, input_args);
}

TypePtr ApplyAdadeltaFuncImpl::InferType(const PrimitivePtr &primitive,
                                         const std::vector<AbstractBasePtr> &input_args) const {
