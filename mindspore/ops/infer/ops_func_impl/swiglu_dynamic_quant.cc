...
namespace mindspore {
namespace ops {
abstract::BaseShapePtr SwiGLUDynamicQuantFuncImpl::InferShape(
  ...
  if (smooth_scale->GetType()->type_id() != kMetaTypeNone) {
    ...
    if (!IsDynamic(smooth_scale_shape) && is_x_static) {
      MS_CHECK_VALUE(smooth_scale_shape[0] == (int64_t)(x_shape[x_shape.size() - 1] / kSplitNum),
...
TypePtr SwiGLUDynamicQuantFuncImpl::InferType(const PrimitivePtr &prim,
  ...
  std::map<std::string, TypePtr> types = {{"x_type", x_type},
                                          {"smooth_scale_type", smooth_scale_type}};
