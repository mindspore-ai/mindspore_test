...
#include <string>
#include "infer/ops_func_impl/add_scalar.h"
...
TypeId GetOutputType(const PrimitivePtr &primitive, TypeId input_type, TypeId other_type, TypeId alpha_type) {
  ...
  if (alpha_type == kNumberTypeBool && !isBoolType(input_type)) {
    MS_EXCEPTION(ValueError) << "For '" << primitive->name() << "', boolean alpha need boolean input, but got "
                             << TypeIdToType(input_type)->ToString();
  }
...
ShapeArray AddScalarFuncImpl::InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const {
  return {input_infos[0]->GetShape()};
}

std::vector<TypeId> AddScalarFuncImpl::InferType(const PrimitivePtr &primitive,
