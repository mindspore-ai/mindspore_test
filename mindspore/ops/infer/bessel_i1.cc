...
namespace ops {
namespace {
abstract::ShapePtr BesselI1InferShape(const PrimitivePtr &, const std::vector<AbstractBasePtr> &input_args) {
...
MIND_API_OPERATOR_IMPL(BesselI1, BaseOperator);
...
class OPS_API AGBesselI1Infer : public abstract::OpInferBase {
 public:
  BaseShapePtr InferShape(const PrimitivePtr &primitive,
                          const std::vector<AbstractBasePtr> &input_args) const override {
    return BesselI1InferShape(primitive, input_args);
  }

  TypePtr InferType(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) const override {
    return BesselI1InferType(primitive, input_args);
  }
  AbstractBasePtr InferShapeAndType(const abstract::AnalysisEnginePtr &engine, const PrimitivePtr &primitive,
                                    const std::vector<AbstractBasePtr> &input_args) const override {
    return BesselI1Infer(engine, primitive, input_args);
  }
};

REGISTER_PRIMITIVE_OP_INFER_IMPL(BesselI1, prim::kPrimBesselI1, AGBesselI1Infer, false);
}  // namespace ops
}  // namespace mindspore
