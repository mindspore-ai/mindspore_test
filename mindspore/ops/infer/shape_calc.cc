...
bool TryGetValueArg(const AbstractBasePtr &abs, ShapeArray *args, std::vector<std::vector<size_t>> *pos_idx) {
  ...
  if (value_ptr->isa<Int64Imm>()) {
    auto scalar_optional = GetScalarValue<int64_t>(value_ptr);
    ...
    auto shape_value_optional = GetArrayValue<int64_t>(abs);
...
class OPS_API ShapeCalcInfer : public abstract::OpInferBase {
 public:
