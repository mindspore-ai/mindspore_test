...
ShapeArray ConvNdInferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos,
  ...
  if (is_batch) {
    ...
    return ConvNdCommonInferShape(primitive, input_infos, input_shape, weight_shape, output_shape, output_padding);
  } else {
    ...
    if (padding_str) {
      real_shape_vector_array =
        ConvNdPaddingCommonInferShape(primitive, input_infos, _input_shape, weight_shape, output_shape, output_padding);
    } else {
      real_shape_vector_array =
        ConvNdCommonInferShape(primitive, input_infos, _input_shape, weight_shape, output_shape, output_padding);
    }
...
ShapeArray ConvNdPaddingCommonInferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos,
  ...
  if (padding_opt.has_value()) {
    ...
    (void)CheckAndConvertUtils::CheckInteger("stride size", SizeToLong(stride.size()), kEqual, feature_len, prim_name);
    (void)CheckAndConvertUtils::CheckInteger("dilation size", SizeToLong(dilation.size()), kEqual, feature_len,
                                             prim_name);
    IndicesCheckPositiveVec("stride", stride, prim_name, true);
    IndicesCheckPositiveVec("dilation", dilation, prim_name, true);
    ...
    for (int i = 0; i < feature_len; i++) {
      nd_output_shape[i + 2] =
        GetOutputHWPadding(input_shape, weight_shape, i + 2, i, stride, padding_enum, dilation, output_padding);
    }
  }
  return {nd_output_shape};
}
...
ShapeArray ConvNdCommonInferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos,
  ...
  if (!input_infos[kIndex0]->IsDynamic() && !input_infos[kIndex1]->IsDynamic()) {
    ...
    (void)CheckAndConvertUtils::CheckInteger("stride size", SizeToLong(stride.size()), kEqual, feature_len, prim_name);
    (void)CheckAndConvertUtils::CheckInteger("padding size", SizeToLong(padding.size()), kEqual, feature_len,
                                             prim_name);
    (void)CheckAndConvertUtils::CheckInteger("dilation size", SizeToLong(dilation.size()), kEqual, feature_len,
                                             prim_name);
    IndicesCheckPositiveVec("stride", stride, prim_name, true);
    IndicesCheckPositiveVec("padding", padding, prim_name, false);
    IndicesCheckPositiveVec("dilation", dilation, prim_name, true);
  ...
  for (int i = 0; i < feature_len; i++) {
    nd_output_shape[i + 2] =
      GetOutputHW(input_shape, weight_shape, i + 2, i, stride, padding, dilation, false, output_padding);
  }
  return {nd_output_shape};
}
...
int64_t GetOutputHWPadding(const ShapeVector &input_shape, const ShapeVector &weight_shape, size_t shape_pos, size_t i,
  ...
  return (input_shape[shape_pos] - dilation[i] * (weight_shape[shape_pos] - 1) - 1) / stride[i] + 1;
}
