...
REG_BPROP_BUILDER("LessScalar").SetUnusedInputs({i0, i1, i2, i3}).SetBody(ReturnZeros);

REG_BPROP_BUILDER("InplaceLessScalar").SetUnusedInputs({i0, i1, i2, i3}).SetBody(ReturnZeros);

REG_BPROP_BUILDER("InplaceLessTensor").SetUnusedInputs({i0, i1, i2, i3}).SetBody(ReturnZeros);

REG_BPROP_BUILDER("InplaceLeTensor").SetUnusedInputs({i0, i1, i2}).SetBody(ReturnZeros);

REG_BPROP_BUILDER("InplaceLeScalar").SetUnusedInputs({i0, i2}).SetBody(ReturnZeros);
...
REG_BPROP_BUILDER("GreaterScalar").SetUnusedInputs({i0, i1, i2, i3}).SetBody(ReturnZeros);

REG_BPROP_BUILDER("InplaceGreaterScalar").SetUnusedInputs({i0, i1, i2, i3}).SetBody(ReturnZeros);

REG_BPROP_BUILDER("InplaceGreaterTensor").SetUnusedInputs({i0, i1, i2, i3}).SetBody(ReturnZeros);
...
REG_BPROP_BUILDER("InplaceGreaterEqualTensor").SetUnusedInputs({i0, i1, i2, i3}).SetBody(ReturnZeros);

REG_BPROP_BUILDER("InplaceGreaterEqualScalar").SetUnusedInputs({i0, i1, i2, i3}).SetBody(ReturnZeros);
...
REG_BPROP_BUILDER("TriangularSolve").FreeUselessValues_O({i1}).SetBody(BODYFUNC(ib) {
  auto b = ib->GetInput(kIndex0);
  auto A = ib->GetInput(kIndex1);
  auto upper = ib->GetInput(kIndex2);
  auto transpose = ib->GetInput(kIndex3);
  auto unitriangular = ib->GetInput(kIndex4);
  auto out = ib->GetInput(kIndex5);
  auto dout = ib->GetInput(kIndex6);
  NodePtr grad_b = nullptr;
  NodePtr grad_A = nullptr;
  auto grad_x = ib->TupleGetItem(dout, kIndex0);
  auto grad_m = ib->TupleGetItem(dout, kIndex1);
  auto x = ib->TupleGetItem(out, kIndex0);
  ...
  if (!transpose_opt.has_value()) {
    auto true_branch = [&](Emitter *e) -> NodePtrList {
      return {e->TupleGetItem(
        e->Emit("TriangularSolve", {grad_x, e->Conj(A), upper, e->Value<bool>(false), unitriangular}), kIndex0)};
    };
    auto false_branch = [&](Emitter *e) -> NodePtrList {
      return {e->TupleGetItem(
        e->Emit("TriangularSolve", {grad_x, e->Conj(A), upper, e->Value<bool>(true), unitriangular}), kIndex0)};
    };
...
REG_BPROP_BUILDER("Addr").SetUnusedInputs({i5}).SetBody(BODYFUNC(ib) {
  auto input = ib->GetInput(kIndex0);
  auto vec1 = ib->GetInput(kIndex1);
  auto vec2 = ib->GetInput(kIndex2);
  auto beta = ib->GetInput(kIndex3);
  auto alpha = ib->GetInput(kIndex4);
  auto dout = ib->GetInput(kIndex6);
  ...
  auto dvec1 = vec1->need_compute_grad_out() ? MaybeMultiply(ib, input_type, ib->Mv(dout, vec2), alpha, "alpha")
                                             : ib->OutZeros(vec1);
  NodePtr dvec2{nullptr};
  if (vec2->need_compute_grad_out()) {
    auto dout_shape = ib->GetShape(dout);
    if (dout_shape.size() == 2) {
      dvec2 = MaybeMultiply(ib, input_type, ib->Mv(ib->Transpose(dout, {1, 0}), vec1), alpha, "alpha");
    } else {
      dvec2 = MaybeMultiply(ib, input_type, ib->Mv(dout, vec1), alpha, "alpha");
    }
  } else {
    ib->OutZeros(vec2);
  }
  return {input_grad, dvec1, dvec2, ib->OutZeros(beta), ib->OutZeros(alpha)};
});
