...
class OperatorPopulateRegistry {
 public:
  ...
  ParameterPtrGen GetParameterPtrCreator(const std::string &name) const { return CreatePopulatePtr; }
...
class Registry {
 public:
  Registry(const std::string &name, int type, ParameterPtrGen creator) noexcept {
