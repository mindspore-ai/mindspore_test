...
Use a shell script to start the 2-card script. The sample shell script is as follows:
...
To start the 8-card script by using a shell script, the sample shell script is as follows:
...
To start the 2-card script by using a shell script, the sample shell script is as follows:
...
`AlltoAllVC` functionality is similar to `AlltoAllV`, but with better performance. `AlltoAllVC` will split and distribute input data along a specific dimension according to the `send_count_matrix`. The `send_count_matrix` stores the send-receive parameters for all ranks, where `send_count_matrix[i][j]` represents the data volume sent from rank i to rank j, with the basic unit being the data type of the Tensor.

The sample code is as follows: We use the `AlltoAllVC` operator for data exchange between 2 cards. The `send_count_matrix` stores that the data volume sent from card 0 to card 1 is 3, and the data volume sent from card 1 to card 0 is 3, meaning that data exchange is performed between the two cards.
