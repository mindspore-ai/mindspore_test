...
.. py:class:: mindspore.nn.InstanceNorm3d(num_features, eps=1e-5, momentum=0.1, affine=True, gamma_init='ones', beta_init='zeros', dtype=mstype.float32)
    ...
    参数：
        ...
        - **gamma_init** (Union[Tensor, str, Initializer, numbers.Number]) - gamma参数的初始化方法。str的值引用自函数 :func:`mindspore.common.initializer` ，包括 ``'zeros'`` 、 ``'ones'`` 等。使用Tensor作为初始化参数时，shape必须为 :math:`(C)`。默认值： ``'ones'`` 。
        - **beta_init** (Union[Tensor, str, Initializer, numbers.Number]) - beta参数的初始化方法。str的值引用自函数 :func:`mindspore.common.initializer` ，包括 ``'zeros'`` 、 ``'ones'`` 等。使用Tensor作为初始化参数时，shape必须为 :math:`(C)`。默认值： ``'zeros'`` 。
