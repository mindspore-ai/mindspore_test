...
.. py:class:: mindspore.nn.FTRL(params, initial_accum=0.1, learning_rate=0.001, lr_power=-0.5, l1=0.0, l2=0.0, use_locking=False, loss_scale=1.0, weight_decay=0.0)
    ...
    FTRL是一种在线凸优化算法，根据损失函数自适应地选择正则化函数。详见论文 `Adaptive Bound Optimization for Online Convex Optimization <https://arxiv.org/abs/1002.4908>`_。
    ...
    参数：
        ...
        - **initial_accum** (float，可选) - 累加器 `m` 的初始值，必须大于等于零。默认值： ``0.1`` 。
        - **learning_rate** (float，可选) - 学习速率值必须为零或正数，当前不支持动态学习率。默认值： ``0.001`` 。
        - **lr_power** (float，可选) - 学习率的幂值，控制训练期间学习率的下降方式，必须小于或等于零。如果lr_power为零，则使用固定的学习率。默认值： ``-0.5`` 。
        - **l1** (float，可选) - l1正则化强度，必须大于等于零。默认值： ``0.0`` 。
        - **l2** (float，可选) - l2正则化强度，必须大于等于零。默认值： ``0.0`` 。
        - **use_locking** (bool，可选) - 如果为True，则更新操作使用锁保护。默认值： ``False`` 。
