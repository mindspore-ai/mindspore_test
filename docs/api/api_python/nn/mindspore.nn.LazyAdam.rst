...
.. py:class:: mindspore.nn.LazyAdam(params, learning_rate=1e-3, beta1=0.9, beta2=0.999, eps=1e-8, use_locking=False, use_nesterov=False, weight_decay=0.0, loss_scale=1.0)
    ...
    .. note::
       ...
       需要注意的是，梯度稀疏时，该优化器只更新网络参数当前的索引位置，稀疏行为不等同于Adam算法。如果想执行稀疏策略，那么需要把target设置为CPU。
    ...
    参数：
        ...
        - **beta1** (float，可选) - `moment1` 的指数衰减率。参数范围（0.0,1.0）。默认值： ``0.9`` 。
        - **beta2** (float，可选) - `moment2` 的指数衰减率。参数范围（0.0,1.0）。默认值： ``0.999`` 。
        - **eps** (float，可选) - 将添加到分母中，以提高数值稳定性。必须大于0。默认值： ``1e-8`` 。
