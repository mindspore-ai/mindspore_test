...
.. py:class:: mindspore.nn.probability.distribution.Gumbel(loc, scale, seed=0, dtype=mstype.float32, name='Gumbel')
    ...
    参数：
        - **loc** (int, float, list, numpy.ndarray, Tensor) - Gumbel分布的位置，公式中的 :math:`a`。
        - **scale** (int, float, list, numpy.ndarray, Tensor) - Gumbel分布的比例，公式中的 :math:`b`。
    ...
    .. note::
        - `scale` 必须大于零。
        - `dtype` 必须是float，因为Gumbel分布是连续的。
        - GPU后端不支持 `kl_loss` 和 `cross_entropy` 。

    异常：
        - **ValueError** - `scale` 中元素小于等于0。
