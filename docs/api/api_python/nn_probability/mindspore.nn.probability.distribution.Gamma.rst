...
.. py:class:: mindspore.nn.probability.distribution.Gamma(concentration=None, rate=None, seed=None, dtype=mstype.float32, name='Gamma')
    ...
    参数：
        - **concentration** (int, float, list, numpy.ndarray, Tensor) - 浓度参数，也被称为伽马分布的 :math:`\alpha`。默认值： ``None`` 。
        - **rate** (int, float, list, numpy.ndarray, Tensor) - 逆尺度参数，也被称为伽马分布的 :math:`\beta`。默认值： ``None`` 。
    ...
    异常：
        - **ValueError** - `concentration` 或者 `rate` 中元素小于或等于0。
        - **TypeError** - `dtype` 不是float或float的子类。

    .. py:method:: concentration
        :property:

        返回分布的浓度（也称为伽马分布的 :math:`\alpha`）。
    ...
    .. py:method:: cross_entropy(dist, concentration_b, rate_b, concentration, rate)
        ...
        参数：
            - **dist** (str) - 分布的类型。
            - **concentration_b** (Tensor) - 分布b的 :math:`\alpha`。
            - **rate_b** (Tensor) - 分布b的 :math:`\beta`。
            - **concentration** (Tensor) - 分布a的 :math:`\alpha`。默认值： ``None`` 。
            - **rate** (Tensor) - 分布a的 :math:`\beta`。默认值： ``None`` 。
