...
.. py:function:: mindspore.parallel.parameter_broadcast(net, layout, cur_rank=0, initial_rank=0)
    ...
    参数：
        - **net** (Cell) - 将进行参数广播的网络。
        - **layout** (Dict) - 参数layout字典，来自于函数 :func:`mindspore.nn.Cell.parameter_layout_dict` ；也可以从从文件中读取。例如，通过AutoParallel.save_param_strategy_file(file_path)保存下来的strategy.ckpt。该字典的key是参数名称，value是参数的Layout。
