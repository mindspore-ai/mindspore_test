...
.. py:class:: mindspore.dataset.audio.AmplitudeToDB(stype=ScaleType.POWER, ref_value=1.0, amin=1e-10, top_db=80.0)
    ...
    参数：
        - **stype** (:class:`~.audio.ScaleType`, 可选) - 输入音频的原始标度，取值可为 ``ScaleType.POWER`` 或 ``ScaleType.MAGNITUDE`` 。默认值： ``ScaleType.POWER`` 。
        - **ref_value** (float, 可选) - 用于计算分贝系数 `db_multiplier` 的乘数参考值。默认值： ``1.0`` 。计算公式为
