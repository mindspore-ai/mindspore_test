...
.. py:class:: mindspore.mint.optim.Adam(params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0, amsgrad=False, *, maximize=False)
    ...
    异常：
        - **ValueError** - `lr` 不是int、float或Tensor。
        - **ValueError** - `lr` 小于0。
        - **ValueError** - `eps` 小于0。
