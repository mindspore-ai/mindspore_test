...
.. py:class:: mindspore.mint.nn.InstanceNorm3d(num_features, eps=1e-5, momentum=0.1, affine=False, track_running_stats=False, device=None, dtype=None)

    该层对5D输入（具有额外通道维度的3D输入的小批量数据）应用实例归一化（Instance Normalization）。
    ...
    输入：
        - **input** (Tensor) - 输入shape为 :math:`(N, C, D, H, W)` 或 :math:`(C, D, H, W)`。
