...
.. py:function:: mindspore.mint.min(input)
    ...
    返回：
        ...
        - **index** (Tensor) - 输入Tensor的沿给定维度的最小值索引，数据类型为 `int64` 。如果 `keepdim` 为 ``True`` ，输出Tensor的维度是 :math:`(input_1, input_2, ...,input_{axis-1}, 1, input_{axis+1}, ..., input_N)` 。否则输出维度为 :math:`(input_1, input_2, ...,input_{axis-1}, input_{axis+1}, ..., input_N)` 。
