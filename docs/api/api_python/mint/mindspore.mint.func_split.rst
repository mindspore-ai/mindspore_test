...
.. py:function:: mindspore.mint.split(tensor, split_size_or_sections, dim=0)
    ...
    参数：
        ...
        - **split_size_or_sections** (Union[int, tuple(int), list(int)]) - 如果 split_size_or_sections 是int类型， tensor将被均匀的切分成块，每块的大小为 split_size_or_sections ，若 tensor.shape[dim] 不能被 split_size_or_sections 整除，则最后一块大小将小于 split_size_or_sections ；如果 split_size_or_sections 是list类型，tensor 将沿 dim 轴被切分成 len(split_size_or_sections) 块，大小为 split_size_or_sections。
    ...
    异常：
        ...
        - **TypeError** - `split_size_or_sections` 中的每个元素都不是int类型。
