mindspore.mint.nn.UpsamplingNearest2d
=====================================

.. py:class:: mindspore.mint.nn.UpsamplingNearest2d(size=None, scale_factor=None)

    执行最近邻上采样操作。

    此运算符使用指定的 `size` 或 `scale_factor` 缩放因子放大输入体积，过程使用最近邻算法。
    ...
    输入：
        - **input** (Tensor) - Shape为 :math:`(N, C, H_{in}, W_{in})` 的四维Tensor。支持的数据类型：[uint8, float16, float32, float64]。
    ...
    异常：
        ...
        - **TypeError** - `input` 的数据类型不是uint8、float16、float32或float64。
