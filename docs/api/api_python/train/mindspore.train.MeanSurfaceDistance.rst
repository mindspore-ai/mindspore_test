...
.. py:class:: mindspore.train.MeanSurfaceDistance(symmetric=False, distance_metric="euclidean")
    ...
    参数：
        ...
        - **symmetric** (bool) - 是否计算 `y_pred` 和 `y` 之间的对称平均平面距离。如果为 ``False`` ，计算方式为  :math:`AvgSurDis(y\_pred \rightarrow y)` ；如果为 ``True`` ，计算方式为 :math:`MeanSurDis(y\_pred \leftrightarrow y)` 。默认值： ``False`` 。
