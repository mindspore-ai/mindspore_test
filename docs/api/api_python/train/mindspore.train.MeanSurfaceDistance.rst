...
.. py:class:: mindspore.train.MeanSurfaceDistance(symmetric=False, distance_metric="euclidean")
    ...
    参数：
        - **symmetric** (bool，可选) - 是否计算 `y_pred` 和 `y` 之间的对称平均表面距离。如果为 ``False`` ，计算方式为  :math:`AvgSurDis(y\_pred \rightarrow y)` ；如果为 ``True`` ，计算方式为 :math:`MeanSurDis(y\_pred \leftrightarrow y)` 。默认值： ``False`` 。
        - **distance_metric** (str，可选) - 支持如下三种距离计算方法： ``"euclidean"`` （欧式距离）、 ``"chessboard"`` （棋盘距离、切比雪夫距离） 或 ``"taxicab"`` （出租车距离、曼哈顿距离）。默认值： ``"euclidean"`` 。
