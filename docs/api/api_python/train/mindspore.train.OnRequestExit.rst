...
.. py:class:: mindspore.train.OnRequestExit(save_ckpt=True, save_mindir=True, file_name='Net', directory='./', config_file=None, sig=signal.SIGTERM)
    ...
    异常：
        ...
        - **ValueError** - `sig` 不是int值，或不是 ``signal.SIGTERM``。
