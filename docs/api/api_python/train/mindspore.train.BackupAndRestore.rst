...
.. py:class:: mindspore.train.BackupAndRestore(backup_dir, save_freq="epoch", delete_checkpoint=True)
    ...
    参数：
        ...
        - **save_freq** (Union["epoch", int]，可选) - 当设置为 ``"epoch"`` 时，在每个epoch进行备份；当设置为整数时，将在每隔 `save_freq` 个epoch进行备份。默认值： ``"epoch"`` 。
        - **delete_checkpoint** (bool，可选) - 如果 `delete_checkpoint` 为 ``True`` ，将在训练结束的时候删除备份文件，否则保留备份文件。默认值： ``True`` 。
