...
.. py:class:: mindspore.train.ROC(class_num=None, pos_label=None)
    ...
    参数：
        - **class_num** (int，可选) - 类别数。对于二分类问题，此入参可以不设置。默认值： ``None`` 。
        - **pos_label** (int，可选) - 正类的类别值。二分类问题中，用户可以自行设置正类的类别值为其他值，如果不设置此入参（即 `pos_label` 为 ``None`` ）时，正类的类别值默认为1。多分类问题中，用户不应设置此参数，因为它将在[0, num_classes-1]范围内迭代更改。默认值： ``None`` 。
