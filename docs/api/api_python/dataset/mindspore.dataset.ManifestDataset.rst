...
.. py:class:: mindspore.dataset.ManifestDataset(dataset_file, usage='train', num_samples=None, num_parallel_workers=None, shuffle=None, sampler=None, class_indexing=None, decode=False, num_shards=None, shard_id=None, cache=None)
    ...
    生成的数据集有两列： `[image, label]` 。 `image` 列的数据类型为uint8。 `label` 列的数据类型为uint64。
    ...
    异常：
        - **RuntimeError** - `dataset_file` 路径下不包含任何数据文件。
    ...
    **关于Manifest数据集：**
    ...
    Manifest文件包含数据集中的文件列表，包括文件名和文件ID等基本文件信息，以及扩展文件元数据。
    Manifest是华为ModelArts支持的数据格式文件，详细说明请参见 `Manifest文档 <https://support.huaweicloud.com/intl/zh-cn/sdkreference-modelarts/modelarts_04_0349.html>`_ 。
