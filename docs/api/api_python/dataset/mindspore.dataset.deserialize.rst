...
.. py:function:: mindspore.dataset.deserialize(input_dict=None, json_filepath=None)
    ...
    数据处理管道反序列化，支持输入Python字典或使用 :func:`mindspore.dataset.serialize` 接口生成的JSON文件。

    参数：
        - **input_dict** (dict) - 以Python字典存储的数据处理管道。默认值： ``None`` 。
        - **json_filepath** (str) - 数据处理管道JSON文件的路径，该文件以通用JSON格式存储了数据处理管道信息，用户可通过 :func:`mindspore.dataset.serialize` 接口生成。默认值： ``None`` 。
