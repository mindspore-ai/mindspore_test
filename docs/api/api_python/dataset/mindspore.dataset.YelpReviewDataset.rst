...
.. py:class:: mindspore.dataset.YelpReviewDataset(dataset_dir, usage=None, num_samples=None, shuffle=Shuffle.GLOBAL, num_shards=None, shard_id=None, num_parallel_workers=None, cache=None)
    ...
    参数：
        ...
        - **usage** (str, 可选) - 指定数据集的子集，可取值为 ``'train'`` 、 ``'test'`` 或 ``'all'`` 。默认值： ``None`` ，读取所有样本。

          - 对于Polarity数据集， ``'train'`` 将读取560,000个训练样本， ``'test'`` 将读取38,000个测试样本， ``'all'`` 将读取所有598,000个样本。
          - 对于Full数据集， ``'train'`` 将读取650,000个训练样本， ``'test'`` 将读取50,000个测试样本， ``'all'`` 将读取所有700,000个样本。
    ...
    **关于YelpReview数据集：**
    ...
    Yelp Review Full数据集包括来自Yelp的评论数据。这些数据是从2015年的Yelp数据集挑战赛数据中提取的，主要用于文本分类。

    Yelp Review Polarity数据集在Full数据集的基础上，对产品评分进行了分级，评论分数1和2视为负面评论，3和4视为正面评论。

    Yelp Review Polarity和Yelp Review Full数据集具有相同的目录结构。
