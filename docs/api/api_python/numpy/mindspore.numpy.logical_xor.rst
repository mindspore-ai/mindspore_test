...
.. py:function:: mindspore.numpy.logical_xor(x1, x2, dtype=None)

    逐元素计算 ``x1`` 和 ``x2`` 的逻辑异或（XOR）的真值。
    ...
    返回：
        Tensor或标量。对 ``x1`` 和 ``x2`` 中的元素执行逻辑异或（XOR）操作的Boolean结果；shape由广播确定。如果 ``x1`` 和 ``x2`` 都是标量，则返回标量。