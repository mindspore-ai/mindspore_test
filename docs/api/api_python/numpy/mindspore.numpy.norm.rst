...
.. py:function:: mindspore.numpy.norm(x, ord=None, axis=None, keepdims=False)
    ...
    参数：
        ...
        - **axis** (Union[None, int, 元素为整数的2-tuple], 可选) – 如果 `axis` 是整数，则指定计算向量范数所沿的 `x` 的轴。如果 `axis` 是2-tuple，则指定保存2-D矩阵的轴，并计算这些矩阵的矩阵范数。如果 `axis` 为 `None` ，则返回向量范数(当 `x` 为1-D时)或矩阵范数(当 `x` 为2-D时)。默认为 `None` 。
