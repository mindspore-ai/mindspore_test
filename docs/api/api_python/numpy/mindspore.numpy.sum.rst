...
.. py:function:: mindspore.numpy.sum(a, axis=None, dtype=None, keepdims=False, initial=None)
    ...
    参数：
        ...
        - **axis** (Union[None, int, tuple(int)]) - 执行求和操作的单个或多个轴。 默认值： `None` 。如果为 `None` ，则求输入数组中所有元素的和。如果 `axis` 为负数，则从最后一个轴到第一个轴计数。 如果 `axis` 是元素为整数的tuple，则对tuple中指定的所有轴进行求和，而非前述的单个轴或所有轴。
        - **dtype** (:class:`mindspore.dtype`, 可选) - 默认值： `None` 。 覆盖输出Tensor的dtype。
        - **keepdims** (bool) - 如果设置为 `True` ，减少的轴在结果中保留为大小为1的维度。 若使用此选项，结果会广播到和输入Tensor同一个维度数。 如果传入默认值，则 `keepdims` 参数不会传递到ndarray子类的std方法中，而任何非默认值将会传递。 如果子类中方法未实现 `keepdims` ，则会引发异常。 默认值： `False` 。
        - **initial** (scalar) - 求和的初始值，如果为 `None`，不会添加额外的初始值，从第一个元素的值开始累加。 默认值： `None` 。 
