...
.. py:function:: mindspore.numpy.sum(a, axis=None, dtype=None, keepdims=False, initial=None)
    ...
    参数：
        ...
        - **keepdims** - 如果设置为 `True` ，减少的轴在结果中保留为大小为1的维度。 若使用此选项，结果会广播到和输入Tensor同一个维度数。 如果传入默认值，则 `keepdims` 参数不会传递到ndarray子类的std方法中，而任何非默认值将会传递。 如果子类中方法未实现 `keepdims` ，则会引发异常。 默认值： `False` 。
