...
.. py:class:: mindspore.runtime.Event(enable_timing=False, blocking=False)
    ...
    .. py:method:: mindspore.runtime.Event.elapsed_time(end_event)
        ...
        异常：
            - **TypeError** - 参数 `end_event` 不是一个 :class:`mindspore.runtime.Event`。

    .. py:method:: mindspore.runtime.Event.query()
    ...
    .. py:method:: mindspore.runtime.Event.record(stream=None)

        在给定的流中记录事件。

        如果未指定 `stream`，将使用 :func:`mindspore.runtime.current_stream` 。`stream` 的设备必须与event的设备匹配。
    ...
    .. py:method:: mindspore.runtime.Event.wait(stream=None)

        使提交给给定流的所有未来工作等待此事件。

        如果未指定 `stream`，将使用 :func:`mindspore.runtime.current_stream()` 。
