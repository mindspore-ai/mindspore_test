...
.. py:function:: mindspore.ops.logical_and(input, other)
    ...
    输入必须是两个Tensor或一个Tensor和一个bool类型值。

    当输入是两个Tensor时，它们的shape可以广播。

    当输入是一个Tensor和一个bool类型值时，bool对象只能是一个常量。
    ...
    参数：
        - **input** (Union[Tensor, bool]) - 第一个输入是bool类型或数据类型可被隐式转换为bool的Tensor。
        - **other** (Union[Tensor, bool]) - 当第一个输入是Tensor时，第二个输入是bool类型值或者数据类型可被隐式转换为bool的Tensor。
