...
.. py:function:: mindspore.ops.pad(input_x, padding, mode='constant', value=None)
    ...
    参数：
        ...
        - **mode** (str，可选) - Pad的填充模式，可选择 ``'constant'`` 、 ``'reflect'`` 、 ``'replicate'`` 或者 ``'circular'`` 。默认 ``'constant'`` 。
        ...
        - **value** (Union[int, float, None]，可选) - 仅在 ``'constant'`` 模式下生效，设置在 ``'constant'`` 模式下的填充值，如果值为 ``None`` ，则会使用0作为默认填充值。默认 ``None`` 。
