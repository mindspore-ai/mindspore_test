...
.. py:class:: mindspore.ops.ApplyAdamWithAmsgradV2(use_locking=False)
    ...
    参数：
        - **use_locking** (bool) - 如果为 ``True`` ， `var` ， `m` 和 `v` 的更新将受到锁的保护。否则，可能出现较少的冲突。默认值为 ``False`` 。
    ...
    输入：
        ...
        - **vhat** (Union[Parameter, Tensor]) - 公式中的 :math:`\hat v_t` 。shape和类型与 `var` 相同。
    ...
    异常：
        - **TypeError** - 如果 `var` 、 `m` 、 `v` 、 `vhat` 不是Parameter或者Tensor。
