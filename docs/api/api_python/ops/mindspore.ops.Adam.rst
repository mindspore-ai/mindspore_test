...
.. py:class:: mindspore.ops.Adam(use_locking=False, use_nesterov=False)
    ...
    输入：
        - **var** (Union[Parameter, Tensor]) - 需更新的权重。shape： :math:`(N, *)` ，其中 :math:`*` 表示任意数量的附加维度，其数据类型可以是float16或float32。
        - **m** (Union[Parameter, Tensor]) - 更新公式中的第一个动量矩阵，shape应与 `var` 相同。
        - **v** (Union[Parameter, Tensor]) - 更新公式中的第二个动量矩阵，shape应与 `var` 相同。
