...
.. py:function:: mindspore.ops.tensor_scatter_add(input_x, indices, updates)
    ...
    参数：
        - **input_x** (Tensor) - 输入tensor。 `input_x` 的维度必须不小于 :math:`indices.\text{shape}[-1]` 。
        - **indices** (Tensor) - 输入tensor的索引，数据类型为mindspore.int32或mindspore.int64。其rank必须至少为2。
        - **updates** (Tensor) - 指定与 `input_x` 相加的Tensor，其数据类型与 `input_x` 相同，并且shape应等于 :math:`indices.\text{shape}[:-1] + input\_x.\text{shape}[indices.\text{shape}[-1]:]` 。
