...
.. py:class:: mindspore.ops.MultitypeFuncGraph(name, read_value=False)
    ...
    MultitypeFuncGraph是一个用于生成重载函数的类，使用不同类型作为输入。使用 `name` 初始化一个MultitypeFuncGraph对象，然后用带有输入类型的 `register` 注册器装饰注册类型。这样该函数可以使用不同的类型作为输入调用，一般与 `HyperMap` 、 `Map` 结合使用。

    参数：
        - **name** (str) - 操作名。
        - **read_value** (bool, 可选) - 如果注册函数不需要对输入的值进行更改，即所有输入均为按值传递，则将 `read_value` 设置为 ``True`` 。默认值： ``False`` 。
