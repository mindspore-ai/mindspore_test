...
.. py:class:: mindspore.ops.TBERegOp(op_name)
    ...
    .. py:method:: async_flag(async_flag=False)
        ...
        定义算子的计算效率，标识是否支持异步计算。

        参数：
            - **async_flag** (bool，可选) - 该算子是否支持异步计算的标识。默认值： ``False`` 。
    ...
    .. py:method:: attr(name=None, param_type=None, value_type=None, value=None, default_value=None, **kwargs)
        ...
        参数：
            - **name** (str，可选) - 表示算子属性的名称。默认值： ``None`` 。
            - **param_type** (str，可选) - 表示算子属性的参数类型。默认值： ``None`` 。
            - **value_type** (str，可选) - 表示算子属性的类型。默认值： ``None`` 。
            - **value** (str，可选) - 表示算子属性的值。默认值： ``None`` 。
            - **default_value** (str，可选) - 表示算子属性的默认值。默认值： ``None`` 。
    ...
    .. py:method:: compute_cost(compute_cost=10)
        ...
        参数：
            - **compute_cost** (int，可选) - 表示计算成本的值。默认值： ``10`` 。
    ...
    .. py:method:: dynamic_compile_static(dynamic_compile_static=False)
        ...
        参数：
            - **dynamic_compile_static** (bool，可选) - 该算子是否支持动静合一的标识。默认值： ``False`` 。
    ...
    .. py:method:: dynamic_shape(dynamic_shape=False)
        ...
        参数：
            - **dynamic_shape** (bool，可选) - 该算子是否支持动态shape的标识。默认值： ``False`` 。
    ...
    .. py:method:: input(index=None, name=None, need_compile=None, param_type=None, shape=None, value_depend=None, **kwargs)
        ...
        参数：
            - **index** (int，可选) - 表示输入的顺序。默认值： ``None`` 。
            - **name** (str，可选) - 表示当前输入的名称。默认值： ``None`` 。
            - **need_compile** (bool，可选) - 表示输入是否需要编译。默认值： ``None`` 。
            - **param_type** (str，可选) - 表示输入的类型。默认值： ``None`` 。
            - **shape** (str，可选) - 表示输入的shape。默认值： ``None`` 。
            - **value_depend** (str，可选) - 表示输入是否为常量值。默认值： ``None`` 。
    ...
    .. py:method:: is_dynamic_format(is_dynamic_format=False)
        ...
        参数：
            - **is_dynamic_format** (bool，可选) - 该算子是否需要op_select_format函数来动态选择合适的数据格式和数据类型的标识。默认值： ``False`` 。
    ...
    .. py:method:: op_pattern(pattern=None)
        ...
        参数：
            - **pattern** (str，可选) - 算子支持的行为类型，如"broadcast"、"reduce"等。默认值： ``None`` 。
    ...
    .. py:method:: output(index=None, name=None, need_compile=None, param_type=None, shape=None, **kwargs)
        ...
        参数：
            - **index** (int，可选) - 表示输出的顺序。默认值： ``None`` 。
            - **name** (str，可选) - 表示输出的名称。默认值： ``None`` 。
            - **need_compile** (bool，可选) - 表示输出是否需要编译。默认值： ``None`` 。
            - **param_type** (str，可选) - 表示输出的类型。默认值： ``None`` 。
            - **shape** (str，可选) - 表示输出的shape。默认值： ``None`` 。
            - **kwargs** (dict) - 表示输出的其他信息。

    .. py:method:: partial_flag(partial_flag=True)

        定义算子的计算效率，标识是否支持部分计算。

        参数：
            - **partial_flag** (bool，可选) - 该算子是否支持部分计算的标识。默认值： ``True`` 。
