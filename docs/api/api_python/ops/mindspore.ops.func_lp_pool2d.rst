...
.. py:function:: mindspore.ops.lp_pool2d(x, norm_type, kernel_size, stride=None, ceil_mode=False)
    ...
    参数：
        ...
        - **stride** (Union[int, tuple[int]]) - 池化操作的移动步长，如果为整数，则代表stride的高和宽。如果为tuple，其值必须包含两个整数值分别表示stride的高和宽。默认 ``None`` ，表示移动步长为 `kernel_size` 。
        - **ceil_mode** (bool) - 若为 ``True`` ，使用ceil模式来计算输出shape。若为 ``False`` ，使用floor模式来计算输出shape。默认 ``False`` 。

    返回：
        - **output** - LPPool2d的计算结果，shape为 :math:`(N, C, H_{in}, W_{in})` 的Tensor，与输入 `x` 的类型一致，其中
