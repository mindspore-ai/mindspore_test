...
.. py:class:: mindspore.ops.CTCLossV2(blank=0, reduction="none", zero_infinity=False)
    ...
    输入：
        - **log_probs** (Tensor) - 输入Tensor，是一个shape为 :math:`(T, N, C)` 的三维Tensor。 :math:`T` 表示输入长度， :math:`N` 表示批大小， :math:`C` 表示类别数，包含空白标签。支持的数据类型：float32、float64。
        - **targets** (Tensor) - 标签序列，是一个shape为 :math:`(N, S)` 的二维Tensor。 :math:`S` 表示最大标签长度。支持的数据类型：int32、int64。
