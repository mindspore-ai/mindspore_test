...
.. py:function:: mindspore.ops.assign_add(variable, value)
    ...
    .. note::
        由于 `variable` 的数据类型为Parameter或者Tensor，不能更改，因此只允许 `value` 的数据类型转换为 `variable` 的数据类型。不同设备支持的转换类型会有所不同，建议在使用此函数时使用相同的数据类型。
    ...
    异常：
        - **TypeError** - `value` 既不是数值型也不是Tensor。
        - **RuntimeError** - `value` 的数据类型不能转换为 `variable` 的数据类型。
