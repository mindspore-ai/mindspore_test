...
.. py:function:: mindspore.ops.all_gather_matmul(input, x2, group, world_size, *, bias=None, gather_index=0,\
                                                 gather_output=True, comm_turn=0, trans_self=False, trans_x2=False)
    ...
    参数：
        ...
        - **group** (str) - 通信组名称，可以由 ``create_group`` 方法创建，也可以使用默认组
    ...
    关键字参数：
        ...
        - **gather_index** (int, 可选) - 表示 allgather 操作对象， ``0`` 表示对 ``input`` 做 gather， ``1`` 表示对 ``x2`` 做 gather。默认为
          ``0`` ，当前仅支持 ``0`` 。
        - **gather_output** (bool, 可选) - 表示是否需要 gather 输出。默认为 ``True`` 。
