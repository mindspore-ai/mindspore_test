...
.. py:class:: mindspore.ops.UniformReal(seed=0, seed2=0)
    ...
    .. note::
        ...
        - 目前在Ascend平台上不支持 `shape` 为Tensor类型，CPU/GPU平台支持。当输入为Tensor的时候，支持的数据类型：

          - GPU：int32、int64。
          - CPU：int16、int32、int64。

    .. warning::
        Ascend后端不支持随机数重现功能，参数 `seed` 和 `seed2` 不起作用。
    ...
    输出：
        Tensor。shape与输入 `shape` 相同。数据类型为float32。
