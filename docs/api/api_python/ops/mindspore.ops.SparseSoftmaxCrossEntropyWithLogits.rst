...
.. py:class:: mindspore.ops.SparseSoftmaxCrossEntropyWithLogits(is_grad=False)
    ...
    将预测值设置为 `X` ，输入标签设置为 `Y` ，输出设置为 `loss` 。计算公式如下：
    ...
    参数：
        - **is_grad** (bool，可选) - 如果为 ``True`` ，则返回计算的梯度。默认值： ``False`` 。
    ...
    输出：
        Tensor，如果 `is_grad` 为 ``False`` ，则输出Tensor是损失值；如果 `is_grad` 为 ``True`` ，则输出Tensor是输入的梯度，其shape与 `logits` 相同。
