...
.. py:class:: mindspore.ops.ScatterMax(use_locking=False)
    ...
    输入：
        - **input_x** (Union[Parameter, Tensor]) - ScatterMax的输入，任意维度的Parameter或Tensor。shape： :math:`(N, *)` ，其中 :math:`*` 表示任意数量的附加维度。
    ...
    异常：
        ...
        - **TypeError** - `indices` 不是mindspore.int32或者mindspore.int64。
