...
.. py:function:: mindspore.ops.grid_sample(input, grid, mode='bilinear', padding_mode='zeros', align_corners=None)
    ...
    参数：
        ...
        - **mode** (str，可选) - 插值方法。可选方法为 ``'bilinear'``， ``'nearest'``。默认 ``'bilinear'`` 。注： ``'bilinear'`` 还不支持。当 `mode` 为 ``'bilinear'``，且输入为5-D，则 `mode` 为 ``'trilinear'``。但是，当输入为4-D，则 `mode` 为 ``'bilinear'``。默认 ``'bilinear'`` 。
