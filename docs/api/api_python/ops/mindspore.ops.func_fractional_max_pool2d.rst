...
.. py:function:: mindspore.ops.fractional_max_pool2d(input, kernel_size, output_size=None, output_ratio=None, return_indices=False, _random_samples=None)
    ...
    参数：
        ...
        - **output_size** (Union[int, tuple[int]]，可选) - 目标输出shape。如果是int，则表示输出目标的高和宽。如果是tuple，其值必须包含两个int值分别表示目标输出的高和宽。默认 ``None`` 。
        ...
        - **_random_samples** (Tensor，可选) - 3D Tensor，分数最大池化的随机步长。支持的数据类型：float16、float32、double。数值范围[0, 1)。shape为 :math:`(N, C, 2)` 或 :math:`(1, C, 2)` 的Tensor。默认 ``None`` ， `_random_samples` 的值由区间[0, 1)上的均匀分布随机生成。
