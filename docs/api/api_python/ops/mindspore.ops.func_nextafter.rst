...
.. py:function:: mindspore.ops.nextafter(input, other)
    ...
    逐元素返回 `input` 指向 `other` 的下一个可表示的浮点值。

    比如有两个数 :math:`a` 和 :math:`b` ，数据类型为float32，并且设float32数据类型的可表示值增量为 :math:`eps` 。如果 :math:`a < b` ，那么 :math:`a` 指向 :math:`b` 的下一个可表示值就是 :math:`a+eps` ，而 :math:`b` 指向 :math:`a` 的下一个可表示值就是 :math:`b-eps` 。
    ...
    异常：
        ...
        - **TypeError** - `input` 和 `other` 的数据类型非float32或float64。
