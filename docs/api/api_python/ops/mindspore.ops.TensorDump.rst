...
.. py:class:: mindspore.ops.TensorDump(input_output='out')
    ...
    参数：
        - **input_output** (str，可选) - 控制Tensordump行为模式的参数，可选的值为 ['out', 'in', 'all'] 中的一个，默认值： ``out`` 。
          ...
          当input_output参数被配置为'all'或'in'时，生成的输入分片所对应的npy文件命名格式为：fileName_cNodeID_dumpMode_rankID_dtype_id.npy。

          当input_output参数被配置为'all'或'out'时，生成的输出分片所对应的npy文件命名格式为：fileName_dtype_id.npy。
