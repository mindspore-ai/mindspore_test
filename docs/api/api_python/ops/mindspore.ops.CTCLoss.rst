...
.. py:class:: mindspore.ops.CTCLoss(preprocess_collapse_repeated=False, ctc_merge_repeated=True, ignore_longer_outputs_than_inputs=False)
    ...
    参数：
        - **preprocess_collapse_repeated** (bool，可选) - 如果为 ``True`` ，在CTC计算之前将折叠重复标签。默认值： ``False`` 。
        - **ctc_merge_repeated** (bool，可选) - 如果为 ``False`` ，在CTC计算过程中，重复的非空白标签不会被合并，这些标签将被解释为单独的标签。这是CTC的简化版本。默认值： ``True`` 。
        - **ignore_longer_outputs_than_inputs** (bool，可选) - 如果为 ``True`` ，则输出比输入长的序列将被忽略。默认值： ``False`` 。

    输入：
        - **x** (Tensor) - 输入Tensor，是一个shape为 :math:`(max\_time, batch\_size, num\_classes)` 的三维Tensor。 `num_classes` 表示类别数，必须是 `num_labels + 1` ， `num_labels` 表示实际标签的数量。保留空白标签。默认空白标签为 `num_classes - 1` 。数据类型必须为float16、float32或float64。
