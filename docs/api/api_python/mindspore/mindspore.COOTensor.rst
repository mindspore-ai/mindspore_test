...
.. py:class:: mindspore.COOTensor(indices=None, values=None, shape=None, coo_tensor=None)
    ...
    COOTensor的算术运算包括：加（+）、减（-）、乘（*）、除（/）。详细的算术运算支持请参考 `运算符 <https://www.mindspore.cn/docs/zh-CN/master/model_train/program_form/static_graph.html#%E8%BF%90%E7%AE%97%E7%AC%A6>`_。
    ...
    .. py:method:: add(other: COOTensor, thresh: Tensor)
        ...
        参数：
            ...
            - **thresh** (Tensor) - 零维，表示阈值，用来决定COOTensor.add结果中的indice/value对是否出现。如果结果中value的数据类型为实数，则 `thresh` 的数据类型应该与它的数据类型一致。如果结果中value小于 `thresh`，它将会被丢掉。
