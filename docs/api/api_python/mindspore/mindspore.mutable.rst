...
.. py:function:: mindspore.mutable(input_data, dynamic_len=False)
    ...
    当前除了Tensor，所有顶层网络的输入，例如标量、tuple、list和dict，都被当做常量值。常量值是不能求导的，而且在编译优化阶段常量值会被忽略掉。
    ...
    异常：
        - **TypeError** - 如果 `input_data` 不是Tensor、scalar、tuple、list 或dict中的任一种类型或者不是它们的嵌套结构。
        - **TypeError** - 如果 `dynamic_len` 被设置为 ``True`` 并且 `input_data` 不是tuple或者list。
        - **ValueError** - 如果 `dynamic_len` 被设置为 ``True`` ， `input_data` 是tuple或者list的情况下，其中的元素的shape或者类型不一致。
