...
.. py:function:: mindspore.vmap(fn, in_axes=0, out_axes=0)
    ...
    Vmap由Jax率先提出，它消除了算子对batch维度的限制，并提供更加方便、统一的运算符表达。同时，用户还可以与 :func:`mindspore.grad` 等其他功能模块组合使用，提高开发效率，详情请参见教程 `自动向量化Vmap <https://www.mindspore.cn/docs/zh-CN/master/model_train/train_process/optimize/vmap.html>`_ 。
    ...
    参数：
        ...
        - **out_axes** (Union[int, list, tuple]) - 指定映射轴呈现在输出中的索引位置。默认值： ``0`` 。

          - 如果 `out_axes` 是一个整数，则 `fn` 的所有输出都根据此axis指定。
          - 如果 `out_axes` 是一个tuple或list，仅支持由整数或None组成，其长度应与 `fn` 的输出个数相等。请注意，每个输出对应的整数轴索引的取值范围必须在 :math:`[-ndim, ndim)` 中，其中 `ndim` 是 `vmap` 映射后的函数的输出的维度。
          - 所有具有非None映射轴的输出只能指定非None的 `out_axes` ，如果具有None映射轴的输出指定非None的 `out_axes` ，结果将沿映射轴进行广播。
