...
.. py:function:: mindspore.vmap(fn, in_axes=0, out_axes=0)
    ...
    Vmap由Jax率先提出，它消除了算子对batch维度的限制，并提供更加方便、统一的运算符表达。同时，用户还可以与 :func:`mindspore.grad` 等其它功能模块组合使用，提高开发效率，详情请参见教程 `自动向量化Vmap <https://www.mindspore.cn/docs/zh-CN/master/model_train/train_process/optimize/vmap.html>`_ 。
