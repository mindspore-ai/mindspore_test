...
.. py:function:: mindspore.vmap(fn, in_axes=0, out_axes=0)
    ...
    Vmap由Jax率先提出，它消除了算子对batch维度的限制，并提供更加方便、统一的运算符表达。同时，用户还可以与 :func:`mindspore.grad` 等其他功能模块组合使用，提高开发效率。
    ...
    参数：
        ...
        - **in_axes** (Union[int, list, tuple], 可选) - 指定输入参数映射的轴索引。默认值： ``0`` 。
        ...
        - **out_axes** (Union[int, list, tuple], 可选) - 指定映射轴呈现在输出中的索引位置。默认值： ``0`` 。
