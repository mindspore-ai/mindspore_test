...
.. py:function:: mindspore.get_ps_context(attr_key)
    ...
    参数：
        - **attr_key** (str) - 属性的key。
          ...
          - scheduler_manage_port (int，可选)：调度器HTTP端口，对外开放用于接收和处理用户扩容/缩容等请求。默认值： ``11202`` 。
          - enable_ssl (bool，可选)：设置是否打开SSL认证。默认值： ``False`` 。
          - client_password (str，可选)：用于解密客户端证书密钥的密码。默认值： ``''`` 。
          - server_password (str，可选)：用于解密服务端证书密钥的密码。默认值： ``''`` 。
