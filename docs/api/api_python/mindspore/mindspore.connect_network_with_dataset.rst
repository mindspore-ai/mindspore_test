...
.. py:function:: mindspore.connect_network_with_dataset(network, dataset_helper)
    ...
    将 `network` 与 `dataset_helper` 中的数据集连接，只支持 `下沉模式 <https://www.mindspore.cn/docs/zh-CN/master/model_train/train_process/optimize/sink_mode.html>`_，(dataset_sink_mode=True)。
