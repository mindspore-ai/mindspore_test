...
.. py:class:: mindspore.Parameter(default_input, name=None, requires_grad=True, layerwise_parallel=False, parallel_optimizer=True, storage_format="", device=None)
    ...
    参数：
        ...
        - **device** (str) - 仅限Ascend，用于指定存储Parameter的设备。默认情况下，Parameter将在计算时存储在设备上。当device被指定为 ``"CPU"`` 时，Parameter将在需要使用时加载到设备上，并在使用后卸载至CPU。在 :func:`mindspore.runtime.set_memory()` 设置 ``'optimize_level'`` 为 ``'O1'`` 时不生效。可以通过指定device为 ``"CPU"`` 节省显存。
