...
.. py:method:: mindspore.Tensor.min() -> Tensor
    ...
    返回：
        Tensor，值为输入Tensor的最小值，类型与输入Tensor相同。
    ...
    .. py:method:: mindspore.Tensor.min(axis=None, keepdims=False, *, initial=None, where=True, return_indices=False) -> Tensor|number.Number
        :noindex:
    ...
    关键字参数：
        ...
        - **return_indices** (bool, 可选) - 是否返回最小值的下标。默认值： ``False`` 。如果 `axis` 是list或int类型的tuple，则必须取值为 ``False`` 。
