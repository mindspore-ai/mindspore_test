...
.. py:method:: mindspore.Tensor.init_data(slice_index=None, shape=None, opt_shard_group=None)
    ...
    参数：
        ...
        - **opt_shard_group** (str) - 优化器分片组，在自动或半自动并行模式下用于获取参数的切片。关于优化器分组，请参考 `优化器并行 <https://www.mindspore.cn/tutorials/zh-CN/master/parallel/optimizer_parallel.html>`_ 。默认值： ``None`` 。
