...
.. py:method:: Tensor.div(value, *, rounding_mode=None)
    ...
    参数：
        - **value** (Union[Tensor, Number, bool]) - 另一个输入，为数值型，或bool，或数据类型为数值型或bool的Tensor。
    ...
    异常：
        - **TypeError** - 如果 `self` 和 `value` 不是以下之一：Tensor、Number、bool。
