...
.. py:method:: mindspore.Tensor.max()
    ...
    详情请参考 :func:`mindspore.ops.extend.max`。

    .. py:method:: mindspore.Tensor.max(input, dim, keepdim=False)
        :noindex:
    ...
    返回：
        tuple(Tensor)，如果 `dim` 不是为默认值 ``None`` ，返回两个元素类型为Tensor的tuple，包含输入Tensor沿指定维度 `dim` 的最大值和相应的索引。

        - **values** (Tensor) - 输入Tensor沿给定维度的最大值，数据类型和 `self` 相同。如果 `keepdim` 为 ``True`` ，输出Tensor的维度是 :math:`(self_1, self_2, ...,self_{axis-1}, 1, self_{axis+1}, ..., self_N)` 。否则输出维度为 :math:`(self_1, self_2, ...,self_{axis-1}, self_{axis+1}, ..., self_N)` 。
