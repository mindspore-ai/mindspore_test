...
.. py:method:: mindspore.Tensor.select(dim, index)
    ...
    参数：
        ...
        - **y** (Union[Tensor, int, float]) - 备选的Tensor或者数字。如果 `y` 是一个Tensor，那么shape是或者可以被广播为：:math:`(x_1, x_2, ..., x_N, ..., x_R)`。如果 `y` 是int或者float，那么将会被转化为int32或者float32类型，并且被广播为与 `self` 相同的shape。 `self` 和 `y` 中至少要有一个Tensor。
