...
.. py:method:: mindspore.Tensor.baddbmm(batch1, batch2, *, beta=1, alpha=1)
    ...
    将输入的两个三维矩阵batch1与batch2相乘，并将结果与self相加。
    计算公式定义如下：
    ...
    参数：
        - **batch1** (Tensor) - 公式中的 :math:`batch1` 。必须为三维的Tensor，类型与 `self` 一致。
        - **batch2** (Tensor) - 公式中的 :math:`batch2` 。必须为三维的Tensor，类型与 `self` 一致。
    ...
    异常：
        ...
        - **TypeError** - `self` 、 `batch1` 和 `batch2` 的数据类型不一致。
