...
.. py:class:: mindspore.CSRTensor(indptr=None, indices=None, values=None, shape=None, csr_tensor=None)
    ...
    参数：
        - **indptr** (Tensor) - shape为 :math:`(M)` 的一维整数Tensor，其中 :math:`M` 等于 `shape[0] + 1` ，表示每行非零元素在 `values` 中存储的起止位置。默认值： ``None`` 。支持的数据类型为int16、int32和int64。
        - **indices** (Tensor) - shape为 :math:`(N)` 的一维整数Tensor，其中 :math:`N` 等于非零元素数量，表示每个元素的列索引值。默认值： ``None`` 。支持的数据类型为int16、int32和int64。
        - **values** (Tensor) - Tensor， `values` 的零维长度必须与 `indices` 的零维长度相等(values.shape[0] == indices.shape[0])。 `values` 用来表示索引对应的数值。默认值： ``None`` 。
