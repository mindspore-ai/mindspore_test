...
.. py:function:: mindspore.jvp(fn, inputs, v, has_aux=False)
    ...
    计算给定网络的雅可比向量积(Jacobian-vector product, JVP)。JVP的计算过程可参考 `前向模式自动微分 <https://www.mindspore.cn/docs/zh-CN/master/design/programming_paradigm.html#前向自动微分>`_。
    ...
    返回：
        ...
        - **aux_value** (Union[Tensor, tuple[Tensor]], 可选) - 只有`has_aux` 值为 ``True`` ，才返回 `aux_value` 。`aux_value` 是 `fn(inputs)` 的第一个除外的其他输出，且不参与 `fn` 的求导。
