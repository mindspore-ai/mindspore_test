...
.. py:function:: mindspore.load_checkpoint_async(ckpt_file_name, net=None, strict_load=False, filter_prefix=None, dec_key=None, dec_mode="AES-GCM", specify_prefix=None, choice_func=None)
    ...
    返回：
        自定义的内部类， 调用其 `result` 方法可以得到 :func:`mindspore.load_checkpoint` 返回的结果。
