...
.. py:function:: mindspore.saved_tensors_hooks(pack_hook, unpack_hook)

    一个上下文管理器，用于自定义已保存张量（Saved Tensor）的打包（pack）和解包（unpack）方式。
    ...
    .. note::
        当前该上下文管理器在 Graph 模式与 Jit 模式下不支持。

    .. warning::
        不允许在 ``pack_hook`` 中对传入的原始张量进行原地（in-place）修改。

    .. warning::
        为避免产生引用循环, ``pack_hook`` 返回的对象不能直接持有原始张量的引用。
