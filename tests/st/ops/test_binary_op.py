...
def test_binary_op_reference_forward(mode, op_info):
    ...
    Expectation: MindSpore matches PyTorch for outputs.
    '''
...
def test_binary_op_reference_backward(mode, op_info):
    ...
    Expectation: MindSpore matches PyTorch for gradients.
    '''
    fact = BinaryOpsFactory(
        op_info=get_op_info(op_info),
    )
...
def test_binary_op_dynamic_forward(mode, op_info):
    ...
    Expectation: Outputs match PyTorch.
    '''
    fact = BinaryOpsFactory(
        op_info=get_op_info(op_info),
    )
    fact.set_context_mode(mode=mode)
    fact.test_binary_op_dynamic(dynamic_mode='dynamic_shape')
    fact.test_binary_op_dynamic(dynamic_mode='dynamic_rank')
...
def test_binary_op_dynamic_backward(mode, op_info):
    ...
    fact.set_context_mode(mode=mode)
    fact.test_binary_op_dynamic(dynamic_mode='dynamic_shape', grad_cmp=True)
    fact.test_binary_op_dynamic(dynamic_mode='dynamic_rank', grad_cmp=True)
