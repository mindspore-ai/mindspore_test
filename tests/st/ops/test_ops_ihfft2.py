...
from mindspore.ops import ihfft2
from tests.mark_utils import arg_mark


ms.context.set_context(jit_level="O0")

class IHFFT2Net(nn.Cell):
    def __init__(self):
        super(IHFFT2Net, self).__init__()
        self.ihfft2 = ihfft2

    def construct(self, x, s, dim):
        return self.ihfft2(x, s, dim)

class IHFFT2GradNet(nn.Cell):
    def __init__(self, net, dout):
        super(IHFFT2GradNet, self).__init__()
        self.net = net
        self.dout = dout
...
def generate_expect_forward_output(x, s, dim):
    return np.fft.rfft2(x, s, dim, norm="forward")


def generate_expect_backward_output(x, dout, s, dim):
    return np.fft.ifft2(dout, s, dim).astype(x.dtype)
...
def test_ops_ihfft2_normal(mode):
    """
    Feature: ops.ihfft2
    Description: test function ihfft2 forward and auto grad.
    Expectation: success
    """
    ...
    net = IHFFT2Net()
    grad_net = IHFFT2GradNet(net, ms.Tensor(dout))
    grad_net.set_train()
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_ihfft2_dynamic_shape(mode):
    """
    Feature: ops.ihfft2
    Description: test function ihfft2 forward and auto grad with dynamic shape.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (2,)
    ...
    grad_net = IHFFT2GradNet(net, ms.Tensor(dout1))
    grad_net.set_train()
    ...
    grad_net = IHFFT2GradNet(net, ms.Tensor(dout2))
    grad_net.set_train()
    grad_net.set_inputs(x_dyn, n_dyn, dim_dyn)
...
@arg_mark(plat_marks=['platform_ascend', 'cpu_linux', 'cpu_windows',
                      'cpu_macos'], level_mark='level1', card_mark='onecard', essential_mark='unessential')
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_ihfft2_dynamic_rank(mode):
    """
    Feature: ops.ihfft2
    Description: test function ihfft2 forward and auto grad with dynamic rank.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (2,)
