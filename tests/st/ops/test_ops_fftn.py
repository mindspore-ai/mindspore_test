...
def test_ops_fftn_normal(mode):
    """
    Feature: ops.fftn
    Description: test function fftn forward and backward.
    Expectation: success
    """
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_fftn_dynamic_shape(mode):
    """
    Feature: ops.fftn
    Description: test function fftn forward and auto grad with dynamic shape.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (2,)
...
@arg_mark(plat_marks=['platform_ascend', 'cpu_linux', 'cpu_windows', 'cpu_macos'], level_mark='level1',
          card_mark='onecard', essential_mark='unessential')
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_fftn_dynamic_rank(mode):
    """
    Feature: ops.fftn
    Description: test function fftn forward and auto grad with dynamic rank.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (2,)
