...
from mindspore.mint.optim import Adam
...
class AdamFactory():
    ...
    def forward_pytorch_impl(self):
        ...
        if not self.group:
            optimizer = torch.optim.Adam(model.parameters(), lr=self.lr, eps=self.eps, betas=self.betas,
    ...
    def forward_mindspore_impl(self):
        ...
        if not self.group:
            optimizer = Adam(model.trainable_params(), lr=self.lr, eps=self.eps, betas=self.betas,
                             weight_decay=self.weight_decay, amsgrad=self.amsgrad, maximize=self.maximize)
        else:
...
def test_adam_basic(mode):
    ...
    fact.result_cmp()
    fact = AdamFactory(False, False, betas=(0.8, 0.888), weight_decay=1e-3)
    fact.result_cmp()


@arg_mark(plat_marks=['platform_ascend'], level_mark='level1', card_mark='onecard', essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_adam_basic_amsgrad(mode):
    """
    Feature: Test adam.
    Description: Test adam with default parameter.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.LAX)
    fact = AdamFactory(False, False, lr=1e-2, betas=(0.8, 0.888), weight_decay=1e-3, amsgrad=True)
    fact.result_cmp()
    fact = AdamFactory(False, False, amsgrad=True)
    fact.result_cmp()


@arg_mark(plat_marks=['platform_ascend'], level_mark='level1', card_mark='onecard', essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_adam_basic_maximize(mode):
    """
    Feature: Test adam.
    Description: Test adam with default parameter.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.LAX)
    fact = AdamFactory(False, False, maximize=True)
    fact.result_cmp()
    fact = AdamFactory(False, False, amsgrad=True, maximize=True)
    fact.result_cmp()


@arg_mark(plat_marks=['platform_ascend'], level_mark='level1', card_mark='onecard', essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_adam_group(mode):
    """
    Feature: Test adam.
...
@arg_mark(plat_marks=['platform_ascend'], level_mark='level1', card_mark='onecard', essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_adam_group_lr_dynamic(mode):
    """
    Feature: Test adam.
...
def test_adam_group_lr_dynamic_change_param(mode):
    """
    Feature: Test adam.
