...
class PoissonDSFactory:
    ...
    def __init__(self, max_dims, rate_dims):
        ...
        self.shape_map = np.random.randint(1, max_dims, 5, dtype=np.int32)
    ...
    def forward_compare(self):
        ...
        out_shape = self.shape_map
        assert len(ms_shape) == len(out_shape) + len(self.rate_map.shape)
        for index, dim in enumerate(out_shape):
    ...
    def _forward_mindspore(self):
        ...
        shape_dyn = Tensor(shape=[None for _ in shape_map_tensor.shape], dtype=self.shape_dtype)
        rate_dyn = Tensor(shape=[None for _ in rate_tensor.shape], dtype=self.rate_dtype)
        net.set_inputs(shape_dyn, rate_dyn)
        output = net(shape_map_tensor, rate_tensor)
