...
def test_mint_deg2rad_forward(context_mode):
    """
    Feature: pyboost function.
    Description: test function mint.deg2rad forward.
...
@pytest.mark.parametrize("context_mode", [ms.PYNATIVE_MODE, ms.GRAPH_MODE])
@test_utils.run_test_with_On
def test_mint_deg2rad_backward(context_mode):
    """
    Feature: pyboost function.
    Description: test function mint.deg2rad backward.
    Expectation: expect correct result.
    """
    ms.context.set_context(mode=context_mode)
    if context_mode == ms.GRAPH_MODE:
...
def test_mint_deg2rad_dynamic_shape():
    """
