...
def inplace_atan_forward_func(input2):
    return Net()(input2)

def inplace_atan_backward_func(input3):
...
@ops_binary_cases(OpsBinaryCase(input_info=[((1, 2, 3, 4), np.float32)],
                                ...
                                extra_info='SD5B'))
def ops_atan_case1(input_binary_data=None, output_binary_data=None):
    output = inplace_atan_forward_func(Tensor(input_binary_data[0]))
    ...
    grad_output = inplace_atan_backward_func(Tensor(input_binary_data[0]))
...
def test_atan__dynamic_shape():
    ...
    TEST_OP(inplace_atan_forward_func, [[input4], [input5]], "inplace_atan",
