...
def check_args(inputs_seq, yaml_name, disable_input_check, disable_yaml_check, disable_tensor_dynamic_type,
    ...
    if ignore_output_index is not None and not isinstance(ignore_output_index, (int, list)):
        raise TypeError(f"Arg 'ignore_output_index' must be int or list[int], but got {type(ignore_output_index)}.")
...
def convert_input_args(ignore_output_index):
...
def TEST_OP(op, inputs_seq, yaml_name, *, disable_input_check=False, disable_yaml_check=False, disable_mode=[],
    ...
    Keyword Args:
        ...
        disable_resize (bool): Disable test op Resize function.
            ...
            Default: ``False`` .
        ignore_output_index (int, list[int]): Ignore `index` output compare. Default None.
