...
class Conv2dTransposeNet(nn.Cell):
    def __init__(self):
        ...
        out_channel = 1
        kernel_size = 3
        self.conv_input = P.Conv2DTranspose(
            out_channel, kernel_size, pad_mode="valid", pad=0, mode=1, stride=1, dilation=1, group=1
        )
...
def dynamic_shape():
    ...
    out = Tensor(
        np.array([[[[-5, -4, 0, 8], [-10, -2, 2, 3], [0, -2, -4, -7], [-3, -2, -3, -16]]]]).astype(np.float32)
    )
...
def test_dynamic_conv2dtranspose():
    ...
    Description: Test case of dynamic shape for Conv2dTranspose grad operator.
    Expectation: success.
    """
    # Graph mode
