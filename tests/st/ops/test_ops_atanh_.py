...
def inplace_atanh_forward_func(input2):
    return Net()(input2)

@ops_binary_cases(OpsBinaryCase(input_info=[((1, 2, 3, 4), np.float32)],
                                output_info=[((1, 2, 3, 4), np.float32)],
                                extra_info='SD5B'))
def ops_atanh_case1(input_binary_data=None, output_binary_data=None):
    output = inplace_atanh_forward_func(Tensor(input_binary_data[0]))
...
def test_atanh__dynamic_shape():
    ...
    TEST_OP(inplace_atanh_forward_func, [[input3], [input4]], "inplace_atanh",
