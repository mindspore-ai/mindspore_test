...
class HFFT2Net(nn.Cell):
    ...
    def construct(self, x, s, dim):
        return self.hfft2(x, s, dim)

class HFFT2GradNet(nn.Cell):
    def __init__(self, net, dout):
        super(HFFT2GradNet, self).__init__()
        self.net = net
        self.dout = dout
...
def test_ops_hfft2_normal(mode):
    """
    Feature: ops.hfft2
    Description: test function hfft2 forward and auto grad.
    Expectation: success
    """
    ...
    net = HFFT2Net()
    grad_net = HFFT2GradNet(net, ms.Tensor(dout))
    grad_net.set_train()
...
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_hfft2_dynamic_shape(mode):
    """
    Feature: ops.hfft2
    Description: test function hfft2 forward and auto grad with dynamic shape.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (5, 7)
    ...
    grad_net = HFFT2GradNet(net, ms.Tensor(dout1))
    grad_net.set_train()
    ...
    grad_net = HFFT2GradNet(net, ms.Tensor(dout2))
    grad_net.set_train()
    grad_net.set_inputs(x_dyn, n_dyn, dim_dyn)
...
@arg_mark(plat_marks=['platform_ascend', 'cpu_linux', 'cpu_windows',
                      'cpu_macos'], level_mark='level1', card_mark='onecard', essential_mark='unessential')
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_ops_hfft2_dynamic_rank(mode):
    """
    Feature: ops.hfft2
    Description: test function hfft2 forward and auto grad with dynamic rank.
    Expectation: success
    """
    ms.context.set_context(mode=mode)
    s = (5, 7)
