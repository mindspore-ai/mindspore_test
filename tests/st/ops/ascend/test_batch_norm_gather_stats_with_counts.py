...
def test_batch_norm_gather_stats_with_counts_fwd(mode):
    ...
    input_data = Tensor(np.linspace(0, 10, 2 * 3 * 12 * 12).reshape(2, 3, 12, 12), mstype.float16)
    mean = Tensor(np.linspace(0.1, 1, 4 * 3).reshape(4, 3), mstype.float16)
    invstd = Tensor(np.linspace(0.1, 1, 4 * 3).reshape(4, 3), mstype.float16)
    running_mean = None
    running_var = None
    momentum = 1e-3
    eps = 1e-5
    counts = Tensor(np.array([4, 5, 6, 4]), mstype.float16)
    mean_all, invstd_all = batch_norm_gather_stats_with_counts_net(input_data, mean, invstd, running_mean,
                                                                   running_var, momentum, eps, counts)
    expected_mean_all = np.array([0.4746, 0.5565, 0.6383], np.float32)
    expected_invstd_all = np.array([0.2019, 0.3367, 0.4529], np.float32)
