...
def run_dynamic_quant(dtype, batch=1, seq=1, hidden=512):
...
@arg_mark(
    plat_marks=["platform_ascend310p", "platform_ascend910b"],
    level_mark="level0",
    card_mark="onecard",
...
def test_dynamic_quant_bf16():
    """
    Feature:ops.DynamicQuantExt use internal op
    Description: ops.DynamicQuantExt with bfloat16
    Expectation: Success
    """
    run_dynamic_quant(mindspore.bfloat16)
