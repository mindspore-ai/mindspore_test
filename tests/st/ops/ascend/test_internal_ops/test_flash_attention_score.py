...
class FlashAttentionScoreTest:
    ...
    def calc_expect_func(self, input_type, layout: str, batch: int, q_heads: int,
                         ...
                         drop_prop=0.0, low_tri=False, is_amask=False,
                         is_alibi=False, rand_kv_seqlen=False):
        ...
        self.i_init = {
            ...
            "keep_prob": 1 - drop_prop,  # dtype: float
            "scale_value": 1 / math.sqrt(float(embed)),  # dtype: float
            "pre_tokens": 2147483647,  # dtype: int
            "next_tokens": 0 if low_tri else 2147483647,  # dtype: int
            "inner_precise": 0,  # dtype: int
            "input_layout": layout,  # dtype: str
            "sparse_mode": 0  # dtype: int
        }
        self.o_golden = {
        ...
        if is_alibi:
            alibi = np.random.uniform(-10.0, 0.0, size=(batch, q_heads,
                                                        q_seqlen, max_seq)).astype(np.float16).astype(np.float32)
        ...
        for (idx, hidx) in it.product(range(batch), range(q_heads)):
        ...
        if layout == "BSH":  # BNSD to BSH of o
            ...
            self.i_construct["key"] = ms.Tensor(k_bsh).astype(ms_type)
            self.i_construct["value"] = ms.Tensor(v_bsh).astype(ms_type)
            ...
            self.i_construct["key"] = ms.Tensor(k).astype(ms_type)
            self.i_construct["value"] = ms.Tensor(v).astype(ms_type)

        if is_alibi:
            self.i_construct["real_shift"] = ms.Tensor(alibi).astype(ms_type)
        if is_amask:
            amask = np.expand_dims(amask, 1)
            self.i_construct["attn_mask"] = ms.Tensor(amask.astype(np.float16))
        if is_dropout:
            self.i_construct["drop_mask"] = ms.Tensor(dropout_uint8.astype(np.uint8))

        _score_max = _score_max.reshape(batch, q_heads, q_seqlen)
        self.o_golden["softmax_max"] = _score_max.astype(np.float32)
        _score_sum = _score_sum.reshape(batch, q_heads, q_seqlen)
        self.o_golden["softmax_sum"] = _score_sum.astype(np.float32)

        _p = _p.reshape(batch, q_heads, q_seqlen, kv_seqlen[0])
        self.o_golden["softmax_out"] = _p.astype(input_type)
        q_seqlen_output = np.ones(batch)
        q_seqlen_output = q_seqlen_output * q_seqlen
        self.i_construct["actual_seq_qlen"] = q_seqlen_output.astype(np.int32).tolist()
        self.i_construct["actual_seq_kvlen"] = kv_seqlen.astype(np.int32).tolist()
    ...
    def calc_actual_func(self):
        ...
        net = FlashAttentionScoreNet(**self.i_init)
        self.o_ascend = net(*tuple(self.i_construct.values()))

    def compare(self):
        actual = self.o_ascend[3]
