...
def record_hccl_op_status():
    """
    Feature: test record hcom op status.
    Description: test record hcom op status to json files.
    Expectation: none.
    """
    ...
    model.train(2, ds_train, callbacks=[LossMonitor()], dataset_sink_mode=True)
    ...
    prefix = "ms_status_record"
    pattern = os.path.join(save_path, f"{prefix}*")
    files = [True for f in glob.glob(pattern) if os.path.isfile(f)]

    assert all(files) is True


if __name__ == "__main__":
    record_hccl_op_status()
