...
def relu6_forward_func(x):
    return ops.ReLU6()(x)


@test_utils.run_with_cell
def relu6_backward_func(x):
...
def test_relu6(mode):
    ...
    out = relu6_forward_func(x)
    expect_out = np.array([[[[0, 1, 6],
                             [1, 0, 1],
                             [6, 1, 0.]]]]).astype(np.float32)
    assert (out.asnumpy() == expect_out).all()

    grad = relu6_backward_func(x)
    expect_grad = np.array([[[[0, 1, 0],
                              [1, 0, 1],
