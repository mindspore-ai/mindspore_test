...
class DynamicShapeInplaceSub(Cell):
    def __init__(self, indices):
        super().__init__()
        self.inplacesub = P.InplaceSub(indices)
    ...
    def construct(self, x, v, select_indices):
        ...
        out = self.inplacesub(x, v)
        return out


class InplaceSubDynamicShapeMock():
    ...
    def forward_dynamic_shape_cmp(self):
        ...
        jit(ps_net.construct, capture_mode="ast")(self.input_x, self.input_v, self.select_indices)
        context.set_context(mode=context.GRAPH_MODE)
        out_psjit = self.forward_mindspore_dynamic_shape_impl(ps_net)

        pi_net = DynamicShapeInplaceSub(self.indices)
        jit(pi_net.construct, capture_mode="bytecode")(self.input_x, self.input_v, self.select_indices)
