...
class CosDynamicShapeFactory():
    ...
    def forward_cmp(self):
        ...
        jit(ps_net.construct, capture_mode="ast")(self.input_x, self.indices)
        context.set_context(mode=context.GRAPH_MODE)
        out_ps = self.forward_mindspore_impl(ps_net)
        pi_net = CosDynamicShapeNetMS()
        jit(pi_net.construct, capture_mode="bytecode")(self.input_x, self.indices)
    ...
    def grad_cmp(self):
        ...
        pi_net = CosDynamicShapeNetMS()
        jit(pi_net.construct, capture_mode="ast")(self.input_x, self.indices)
        context.set_context(mode=context.PYNATIVE_MODE)
