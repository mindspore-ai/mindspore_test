...
from mindspore.ops.communication import reduce_scatter_tensor, set_comm_ops_inplace
...
init()
#msrun --worker_num=2 --local_worker_num=2 --master_port=1128 --log_dir=msrun_log --join=True --cluster_time_out=300  test_ops_reduce_scatter_tensor.py


def test_hccl_reduce_scatter_tensor_func_dynstatic1():
    """
    ...
    def test(out_tensor, in_tensor):
        ...
        Expectation: expect correct result.
        """
        output, h = reduce_scatter_tensor(out_tensor, in_tensor, async_op=True)
        h.wait()
        return output
    ...
    x = np.ones([6, 4]).astype(np.float32)
...
def test_hccl_reduce_scatter_tensor_func_dynstatic2():
    """
    Feature: test 'reduce_scatter_tensor' communication function.
    Description: test 'reduce_scatter_tensor' communication function.
    Expectation: expect correct result.
    ...
    def test_inplace(out_tensor, in_tensor):
        ...
        Expectation: expect correct result.
        """
        h = reduce_scatter_tensor(out_tensor, in_tensor, async_op=True)
