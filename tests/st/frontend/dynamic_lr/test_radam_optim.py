...
class OptimFactory:
    def __init__(self, group=False, lr_dynamic=False, lr_change=False,
                 ...
                 weight_decay=0.0, dtype=np.float32):
        super().__init__()
    ...
    def forward_pytorch_impl(self):
        ...
        if not self.group:
            optimizer = torch.optim.RAdam(model.parameters(), lr=self.lr, betas=self.betas,
                                          eps=self.eps,
            ...
            optimizer = torch.optim.RAdam(params=group_params, lr=self.lr, betas=self.betas,
                                          eps=self.eps,
                                          weight_decay=self.weight_decay)
    ...
    def forward_mindspore_impl(self):
        ...
        if not self.group:
            optimizer = RAdam(params=model_ms.trainable_params(), lr=self.lr, betas=self.betas,
                              eps=self.eps,
                              weight_decay=self.weight_decay)
        else:
            ...
            group_params = [
                {'params': bias_params, 'weight_decay': 0.01, 'lr': 0.9, "rho": 0.9},
                {'params': no_bias_params, 'lr': 0.66, "rho": 0.7, "eps": 1e-10}]
            optimizer = RAdam(params=group_params, lr=self.lr, betas=self.betas,
                              eps=self.eps,
                              weight_decay=self.weight_decay)
...
def test_radam_group_dynamic():
    """
    Feature: test radam group dynamic.
    Description: test radam group dynamic.
