...
class OptimFactory:
    def __init__(self, group=False, lr_dynamic=False, lr_change=False,
                 ...
                 weight_decay=0.0, maximize=False, epochs=1, steps=1, dtype=np.float32):
        super().__init__()
    ...
    def forward_pytorch_impl(self):
        ...
        if not self.group:
            optimizer = torch.optim.Adamax(model.parameters(), lr=self.lr, betas=self.betas,
                                           eps=self.eps,
                                           weight_decay=self.weight_decay, maximize=self.maximize)
        else:
            ...
            group_params = [
                {'params': bias_params, 'weight_decay': 0.01, 'lr': 0.9, "betas": (0.1, 0.7)},
                {'params': no_bias_params, 'lr': 0.66, "betas": (0.3, 0.6), "eps": 1e-10}]
            optimizer = torch.optim.Adamax(params=group_params, lr=self.lr, betas=self.betas,
                                           eps=self.eps,
                                           weight_decay=self.weight_decay, maximize=self.maximize)
    ...
    def forward_mindspore_impl(self):
        ...
        if not self.group:
            optimizer = Adamax(params=model_ms.trainable_params(), lr=self.lr, betas=self.betas,
                               eps=self.eps,
                               weight_decay=self.weight_decay, maximize=self.maximize)
        else:
            ...
            group_params = [
                {'params': bias_params, 'weight_decay': 0.01, 'lr': 0.9, "betas": (0.1, 0.7)},
                {'params': no_bias_params, 'lr': 0.66, "betas": (0.3, 0.6), "eps": 1e-10}]
            optimizer = Adamax(params=group_params, lr=self.lr, betas=self.betas,
                               eps=self.eps,
                               weight_decay=self.weight_decay, maximize=self.maximize)
...
def test_adamax_group_dynamic():
    """
    Feature: test dynamic lr with Adamax.
    Description: test dynamic lr.
    Expectation: the result match with expected result.
    """
    fact = OptimFactory(group=True, lr_dynamic=True, lr_change=False, out_change=False,
                        inside_change=False, lr=0.02, betas=(0.1, 0.8),
                        eps=0.1, epochs=2, steps=2,
                        weight_decay=0.9,
                        maximize=True)
