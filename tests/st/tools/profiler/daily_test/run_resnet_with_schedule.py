...
from mindspore.profiler import mstx, tensorboard_trace_handler, schedule, ProfilerLevel
...
def train_process(data_directory, prof_dir):
    """Train ResNet50 with profiling schedule Level1 configuration."""
    cfg = Config()
    sched = schedule(wait=1, warmup=1, active=2, repeat=1, skip_first=0)
    prof = mindspore.profiler.Profiler(schedule=sched, profiler_level=ProfilerLevel.Level1, activities=cfg.activities,
    ...
    def forward_fn(data, label):
        """Forward pass computing loss and marking MSTX ranges."""
    ...
    def train_step(data, label):
        """Backward pass computes gradients and performs optimization update."""
