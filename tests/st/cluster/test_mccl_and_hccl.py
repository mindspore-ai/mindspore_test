...
def run_test(dtype_str):
    """Run test of mccl (inter-node) and hccl (intra-node)"""
    ...
    if rank in hccl_rank_list1:
        print(f"Start create intra-node group hccl_intra_node1 with {hccl_rank_list1}")
        create_group("hccl_intra_node1", hccl_rank_list1)
    if rank in hccl_rank_list2:
        print(f"Start create intra-node group hccl_intra_node2 with {hccl_rank_list2}")
        create_group("hccl_intra_node2", hccl_rank_list2)

    class AllReduceNet(nn.Cell):
        """
        The AllReduce Net containing two AllReduce operations
        """
