...
def test_tensor_min(mode):
    ...
    x = Tensor(np.array([[0.0, 0.3, 0.4, 0.5, 0.1],
                         [3.2, 0.4, 0.1, 2.9, 4.0]]), ms.float32)
    net = Net()
    output, index = net(x, 0, True)
    expect_output = Tensor([0., 0.3, 0.1, 0.5, 0.1])
    index_output = Tensor([0, 0, 1, 0, 0])
    assert np.allclose(output.asnumpy(), expect_output.asnumpy())
    assert np.allclose(index.asnumpy(), index_output.asnumpy())
