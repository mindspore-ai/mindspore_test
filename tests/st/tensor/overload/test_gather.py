...
def test_gather_pyboost(mode):
    ...
    assert np.allclose(input_x.gather(1, index).asnumpy(), expect_out)
    assert np.allclose(input_x.gather(dim=1, index=index).asnumpy(), expect_out)
    assert np.allclose(input_x.gather(index=index, dim=1).asnumpy(), expect_out)
...
def test_gather_python(mode):
    ...
    assert np.allclose(input_params1.gather(input_indices1, axis=axis1).asnumpy(), expect_ouput1)
    assert np.allclose(input_params1.gather(input_indices1, axis1, batch_dims=0).asnumpy(), expect_ouput1)
