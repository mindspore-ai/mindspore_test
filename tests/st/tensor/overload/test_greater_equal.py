...
def test_tensor_greater_equal_tensor_tensor(mode):
    """
    Feature: tensor.greater_equal
...
@arg_mark(plat_marks=['cpu_linux', 'cpu_windows', 'cpu_macos', 'platform_gpu', 'platform_ascend'],
          ...
          essential_mark='unessential')
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_tensor_greater_equal_tensor_scalar(mode):
    """
    Feature: tensor.greater_equal
    Description: Verify the result of tensor when dims argument is tuple.
    Expectation: success
    ...
    y = 2
    output = net(x, y)
    expect_output = Tensor([False, True, True])
    assert np.allclose(output.asnumpy(), expect_output.asnumpy())
