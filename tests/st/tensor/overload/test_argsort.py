...
class ArgsortExt(nn.Cell):
    def construct(self, input_x, dim=-1, descending=False):
        return input_x.argsort(dim=dim, descending=descending)
...
def test_tensor_argsort(mode):
    ...
    if ms.get_context("device_target") == "Ascend":
        net2 = ArgsortExt()
        out2 = net2(x)
        assert np.allclose(out2.asnumpy(), np.array(expect))
