# Copyright 2024 Huawei Technologies Co., Ltd
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ============================================================================
import numpy as np
import pytest
from tests.mark_utils import arg_mark
import mindspore as ms
import mindspore.nn as nn
from mindspore import Tensor


class Net(nn.Cell):

    def __init__(self, x):
        super(Net, self).__init__()
        self.x = x

    def construct(self, *args, **kwargs):
        return self.x.nan_to_num(*args, **kwargs)


@arg_mark(plat_marks=['cpu_linux', 'cpu_windows', 'cpu_macos', 'platform_ascend910b'],
          level_mark='level0',
          card_mark='onecard',
          essential_mark='unessential')
@pytest.mark.parametrize('mode', [ms.GRAPH_MODE, ms.PYNATIVE_MODE])
def test_tensor_nan_to_num(mode):
    """
    Feature: tensor.nan_to_num
    Description: Verify the result of tensor.nan_to_num
    Expectation: success
    """
    ms.set_context(mode=mode)
    x = Tensor(np.array([float('nan'), float('inf'), -float('inf'), 5.0]), ms.float32)
    net = Net(x)

    # test 1: using positional args
    output_x = net(1, 2, 3)
    expect_x = Tensor(np.array([1, 2, 3, 5.0]), ms.float32)
    assert np.allclose(output_x.asnumpy(), expect_x.asnumpy())

    # test 2: using default args
    output_x = net()
    float32_max = np.finfo(np.float32).max
    expect_x = Tensor(np.array([0, float32_max, -float32_max, 5]), ms.float32)
    assert np.allclose(output_x.asnumpy(), expect_x.asnumpy())

    # test 3: using kv args
    output_x = net(nan=1, posinf=2, neginf=-3)
    expect_x = Tensor(np.array([1, 2, -3, 5]), ms.float32)
    assert np.allclose(output_x.asnumpy(), expect_x.asnumpy())
