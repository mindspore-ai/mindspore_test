...
def test_graceful_exit_ascend_8p():
    """
    ...
    tmp = r"rank_" + str(device_id)

    reset_json = os.path.join(os.getcwd(), tmp + "/reset.json")
    ckpt = os.path.join(os.getcwd(), tmp + "/LeNet_train.ckpt")
    if not os.path.exists(reset_json):
        os.mkdir(os.path.join(os.getcwd(), tmp))
        with open(reset_json, "w") as f:
            f.write('{"GracefulExit": 1}')
    assert os.path.exists(reset_json)
