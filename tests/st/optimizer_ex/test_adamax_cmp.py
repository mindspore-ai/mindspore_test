...
class AdamaxFactory():
    ...
    def forward_pytorch_impl(self):
        ...
        if not self.group:
            optimizer = torch.optim.Adamax(model.parameters(), self.lr)
        else:
            ...
            optimizer = torch.optim.Adamax(params=group_params, lr=self.lr)
    ...
    def forward_mindspore_impl(self):
        ...
        if not self.group:
            optimizer = Adamax(model.trainable_params(), self.lr)
        else:
            ...
            group_params = [{'params': bias_params, 'weight_decay': 0.0, 'lr': 0.9, "alpha": 0.9, "lambd": 0.02},
                            {'params': no_bias_params, 'lr': 0.66, "t0": 1e5}]
            optimizer = Adamax(params=group_params, lr=self.lr)
...
def test_adamax_basic(mode):
    ...
    fact = AdamaxFactory(False, False)
    fact.result_cmp()
...
def test_adamax_group(mode):
    """
    Feature: Test adamax.
    Description: Test adamax with grouped params.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = AdamaxFactory(True, False)
    fact.result_cmp()
...
def test_adamax_lr_dynamic(mode):
    """
    Feature: Test adamax.
    Description: Test adamax when lr is dynamic.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = AdamaxFactory(False, True)
    fact.result_cmp()
...
def test_adamax_group_lr_dynamic(mode):
    """
    Feature: Test adamax.
    Description: Test adamax with grouped params when lr is dynamic.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = AdamaxFactory(True, True)
    fact.result_cmp()
...
def test_adamax_group_lr_dynamic_change_param(mode):
    """
    Feature: Test adamax.
    Description: Test adamax with grouped params when optimizer params are changed.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = AdamaxFactory(True, True, True)
    fact.result_cmp()
