...
from mindspore.experimental.optim import RMSprop
...
class RMSpropFactory():
    ...
    def forward_pytorch_impl(self):
        ...
        if not self.group:
            optimizer = torch.optim.RMSprop(model.parameters(), self.lr)
        else:
            ...
            group_params = [{'params': bias_params, 'weight_decay': 0.0, 'lr': 0.9, "alpha": 0.9, "centered": True},
                            {'params': no_bias_params, 'lr': 0.66, "momentum": 0.1}]
            optimizer = torch.optim.RMSprop(params=group_params, lr=self.lr)
        ...
        for _ in range(self.epochs):
            ...
            if self.lr_dynamic:
                lr_scheduler.step()
            if self.if_change:
                optimizer.param_groups[1]["alpha"] = 0.77
                optimizer.param_groups[1]["centered"] = False
        output = model(data)
    ...
    def forward_mindspore_impl(self):
        ...
        if not self.group:
            optimizer = RMSprop(model.trainable_params(), self.lr)
        else:
            ...
            group_params = [{'params': bias_params, 'weight_decay': 0.0, 'lr': 0.9, "alpha": 0.9, "centered": True},
                            {'params': no_bias_params, 'lr': 0.66, "momentum": 0.1}]
            optimizer = RMSprop(params=group_params, lr=self.lr)
...
def test_rmsprop_basic(mode):
    """
    Feature: Test rmsprop.
    Description: Test rmsprop with default parameter.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = RMSpropFactory(False, False)
    fact.result_cmp()
...
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_rmsprop_group(mode):
    """
    Feature: Test rmsprop.
    Description: Test rmsprop with grouped params.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = RMSpropFactory(True, False)
    fact.result_cmp()
...
@arg_mark(plat_marks=['platform_ascend'],
          ...
          essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_rmsprop_lr_dynamic(mode):
    """
    Feature: Test rmsprop.
    Description: Test rmsprop when lr is dynamic.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = RMSpropFactory(False, True)
    fact.result_cmp()
...
@arg_mark(plat_marks=['platform_ascend'],
          ...
          essential_mark='unessential')
@pytest.mark.parametrize('mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_rmsprop_group_lr_dynamic(mode):
    """
    Feature: Test rmsprop.
    Description: Test rmsprop with grouped params when lr is dynamic.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = RMSpropFactory(True, True)
    fact.result_cmp()
...
def test_rmsprop_group_lr_dynamic_change_param(mode):
    """
    Feature: Test rmsprop.
    Description: Test rmsprop with grouped params when optimizer params are changed.
    Expectation: success.
    """
    mindspore.set_context(mode=mode, jit_syntax_level=mindspore.STRICT)
    fact = RMSpropFactory(True, True, True)
    fact.result_cmp()
