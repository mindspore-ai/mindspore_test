...
def test_ckpt_adaptive_frequency():
    """
    Feature: ckpt adaptive frequency.
    Description: ckpt adaptive frequency.
    Expectation: No exception.
    """
    context.set_context(mode=context.GRAPH_MODE, device_target="Ascend")

    data_path = os.path.join('/home/workspace/mindspore_dataset/mnist', "train")
    ckpt_path = "./ckpt_adaptive_frequency"
    num_class = 10
    lr = 0.01
    momentum = 0.9
    epoch_size = 10
    ...
    config_ck = CheckpointConfig(save_checkpoint_steps=save_checkpoint_steps,
                                 keep_checkpoint_max=keep_checkpoint_max, adaptive_save_frequency=True)
    ckpoint_cb = ModelCheckpoint(prefix="ckpt_adaptive_frequency", directory=ckpt_path, config=config_ck)
