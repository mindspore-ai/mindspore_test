...
def flipud_func(x):
    return mint.flipud(x)


@test_utils.run_with_cell
def flipud_forward_func(x):
    return flipud_func(x)


@test_utils.run_with_cell
def fliplr_backward_func(x):
    return ops.grad(flipud_func, (0,))(x)
...
def test_flipud_normal(mode):
    ...
    out = flipud_forward_func(x)
    expect_out = np.array([[[4., 5.], [6., 7.]], [[0., 1.], [2., 3.]]])
