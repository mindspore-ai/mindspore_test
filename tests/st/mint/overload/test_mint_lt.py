...
def test_ops_lt_normal(mode):
    """
    Feature: pyboost function.
    Description: test function lt forward.
...
def test_ops_lt_dynamic():
    """
    Feature: pyboost function.
    Description: test function lt forward with dynamic shape.
    Expectation: expect correct result.
    """
    ...
    TEST_OP(lt_forward_func, [[x, y], [x2, y2]], '',
            disable_yaml_check=True, disable_mode=['GRAPH_MODE'])
    y3 = 1
    y4 = 2
    TEST_OP(lt_forward_func, [[x, y3], [x2, y4]], '',
            disable_yaml_check=True, disable_mode=['GRAPH_MODE'])
