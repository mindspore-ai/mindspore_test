...
def test_ngram_operation_01():
    ...
    dataset = dataset.map(operations=nlp.Ngram(1, separator=" "), input_columns=["text"])
    i = 0
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram([2, 3], separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram([1, 1], separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram([2], separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram([3, 1, 2], separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=("_", 2), separator=" "), input_columns=["text"])
...
def test_ngram_operation_02():
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=("__", 2), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=(" ", 2), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=("_", 1), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=("_", 0), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=("_", 3), separator=" "), input_columns=["text"])
...
def test_ngram_operation_03():
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=("_", 2), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=("__", 2), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=(" ", 2), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=("_", 1), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=("_", 0), separator=" "), input_columns=["text"])
...
def test_ngram_operation_04():
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=("", 0), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(3, separator='-'), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    dataset = dataset.map(operations=nlp.Ngram(4, separator=" "), input_columns=["text"])
...
def test_ngram_exception_01():
    ...
    with pytest.raises(ValueError, match="Input gram_0 must be greater than 0"):
        dataset.map(operations=nlp.Ngram(0, separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="Input gram_0 must be greater than 0"):
        dataset.map(operations=nlp.Ngram(-1, separator=" "), input_columns=["text"])
    ...
    with pytest.raises(ValueError, match="n needs to be a non-empty list of positive integers"):
        dataset.map(operations=nlp.Ngram(3.0, separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="n needs to be a non-empty list of positive integers"):
        dataset.map(operations=nlp.Ngram('sjack &%3e1', separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="Input gram_0 must be greater than 0"):
        dataset.map(operations=nlp.Ngram([0, 1], separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="Input gram_0 must be greater than 0"):
        dataset.map(operations=nlp.Ngram([-2, 3], separator=" "), input_columns=["text"])
    ...
    with pytest.raises(TypeError, match="Argument gram\\[0\\] with value 2.0 is not of type"):
        dataset.map(operations=nlp.Ngram([2.0, 3.0], separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="n needs to be a non-empty list of positive integers"):
        dataset.map(operations=nlp.Ngram(n=None, separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="n needs to be a non-empty list of positive integers"):
        dataset.map(operations=nlp.Ngram(n=[], separator=" "), input_columns=["text"])
    ...
    with pytest.raises(ValueError, match="str is pad token and int is pad_width"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=(1, 2), separator=" "), input_columns=["text"])
...
def test_ngram_exception_02():
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="str is pad token and int is pad_width"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=(None, 2), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="str is pad token and int is pad_width"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=("_", 2.5), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="str is pad token and int is pad_width"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=("_", None), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="left_pad needs to be a tuple"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, left_pad=[], separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="str is pad token and int is pad_width"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=(1, 2), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="str is pad token and int is pad_width"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=(None, 2), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="padding width need to be positive numbers"):
        dataset.map(operations=nlp.Ngram(n=3, right_pad=("_", -1), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="str is pad token and int is pad_width"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=("_", 2.5), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="str is pad token and int is pad_width"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=("_", None), separator=" "), input_columns=["text"])
    ...
    dataset = ds.GeneratorDataset(gen(plates_mottos), column_names=["text"])
    with pytest.raises(ValueError, match="right_pad needs to be a tuple"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, right_pad=[], separator=" "), input_columns=["text"])
    ...
    with pytest.raises(TypeError, match="Argument separator with value 1 is not of type"):
        dataset = dataset.map(operations=nlp.Ngram(n=3, separator=1), input_columns=["text"])
...
def test_ngram_exception_03():
    ...
    with pytest.raises(TypeError, match="missing a required argument: 'n'"):
        dataset = dataset.map(operations=nlp.Ngram(), input_columns=["text"])
