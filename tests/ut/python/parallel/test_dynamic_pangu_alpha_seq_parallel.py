...
class Mapping(nn.Cell):
    def __init__(self, config, input_size, output_size, scale=1.0):
        ...
        self.cast = P.Cast()
        self.add = P.TensorAdd().shard(((config.dp * config.mp, 1), (1,)))
        self.matmul = P.MatMul().shard(((config.dp, config.mp), (config.mp, 1)), ((config.dp * config.mp, 1),))
...
class Output(nn.Cell):
    def __init__(self, config, scale=1.0):
        ...
        self.dropout.dropout.shard(((config.dp, config.mp, 1),))
...
class Block(nn.Cell):
    def __init__(self, config, layer_idx):
        ...
        if config.self_layernorm:
            ...
            self.layernorm1.layer_norm.shard(((config.dp, config.mp, 1), (1,), (1,)))
            self.layernorm2 = nn.LayerNorm((config.embedding_size,)).to_float(mstype.float32)
            self.layernorm2.layer_norm.shard(((config.dp, config.mp, 1), (1,), (1,)))
        ...
        self.add = P.TensorAdd().shard(((config.dp, config.mp, 1), (config.dp, config.mp, 1)))
        self.last_add = P.TensorAdd().shard(
            ((config.dp, config.mp, 1), (config.dp, config.mp, 1)))
...
def test_pangu_alpha_seq_dim_dynamic_and_dp_mp_op_sp():
    '''
    Feature: seq dim is dynamic and using data parallel and mode parallel and optimizer parallel and seq parallel
    Description: dp/mp/op/sp
    ...
    s1 = Symbol(divisor=16, remainder=1)
    s2 = Symbol(divisor=16)
    input_ids = Tensor(shape=[32, s1], dtype=mstype.int32)
    input_position = Tensor(shape=[32, s2], dtype=mstype.int32)
    attention_mask = Tensor(shape=[32, s2, s2], dtype=mstype.float16)
