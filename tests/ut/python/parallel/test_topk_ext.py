...
class TopkExtNet(Cell):
    def __init__(self, w1, k, strategy1=None, strategy2=None):
        super().__init__()
        self.mul = P.Mul().shard(strategy1)
        self.w1 = Parameter(w1, "w1")
        self.topk_ext = ms.ops.auto_generate.TopkExt().shard(strategy2)
        self.k = k

    def construct(self, x, b):
        out = self.mul(x, self.w1)
        out, _ = self.topk_ext(out, self.k)
        return out
...
def test_topk_ext_data_parallel():
    """
    ...
    strategy1 = ((8, 1), (8, 1))
    strategy2 = ((8, 1),)
    net = TopkExtNet(_w1, _k, strategy1, strategy2)
    compile_net(net)


def test_topk_ext_model_parallel():
    """
...
def test_topk_ext_auto_parallel():
    """
...
def test_topk_ext_strategy_error():
    """
