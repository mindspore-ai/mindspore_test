...
def test_dctn_auto_parallel():
    """
    Feature: test dctn auto parallel
    Description: auto parallel
    Expectation: compile success
    """
    context.set_auto_parallel_context(parallel_mode="auto_parallel", search_mode="sharding_propagation", device_num=4,
...
def test_dctn_model_parallel():
    """
    Feature: test dctn model parallel
    Description: model parallel
    Expectation: compile success
    """
    context.set_auto_parallel_context(parallel_mode="semi_auto_parallel", device_num=4, global_rank=0)
    net = GradWrap(NetWithLoss(Net(P.DCTN(), tp=2, n=(4, 4), axis=(-2, -1), norm="ortho", strategy=((2, 1, 1),))))
    net.set_train()
    _cell_graph_executor.compile(net, x_)


def test_dctn_strategy_error():
    """
    Feature: test invalid strategy for dctn
    Description: illegal strategy
    Expectation: raise RuntimeError
    """
    context.set_auto_parallel_context(parallel_mode="semi_auto_parallel", device_num=4, global_rank=0)
    net = GradWrap(NetWithLoss(Net(P.DCTN(), tp=2, n=(4, 4), axis=(-2, -1), norm="ortho", strategy=((1, 2, 2),))))
    net.set_train()
    with pytest.raises(RuntimeError):
        _cell_graph_executor.compile(net, x_)


def test_idctn_auto_parallel():
    """
    Feature: test idctn auto parallel
    Description: auto parallel
    Expectation: compile success
    """
    context.set_auto_parallel_context(parallel_mode="auto_parallel", search_mode="sharding_propagation", device_num=4,
...
def test_idctn_model_parallel():
    """
    Feature: test idctn model parallel
    Description: model parallel
    Expectation: compile success
    """
    context.set_auto_parallel_context(parallel_mode="semi_auto_parallel", device_num=4, global_rank=0)
    net = GradWrap(NetWithLoss(Net(P.IDCTN(), tp=2, n=(4, 4), axis=(-2, -1), norm="ortho", strategy=((2, 1, 1),))))
    net.set_train()
    _cell_graph_executor.compile(net, x_)


def test_idctn_strategy_error():
    """
    Feature: test invalid strategy for idctn
    Description: illegal strategy
    Expectation: raise RuntimeError
    """
    context.set_auto_parallel_context(parallel_mode="semi_auto_parallel", device_num=4, global_rank=0)
    net = GradWrap(NetWithLoss(Net(P.IDCTN(), tp=2, n=(4, 4), axis=(-2, -1), norm="ortho", strategy=((1, 2, 2),))))
    net.set_train()
    with pytest.raises(RuntimeError):
        _cell_graph_executor.compile(net, x_)
