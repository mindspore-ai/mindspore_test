...
class Net(Cell):
    def __init__(
        ...
        super().__init__()
        weight_np = np.full(input_size, 0.5, dtype=np.float32)
    ...
    def construct(self, inputs, label):
        x = self.reshape(inputs, self.input_shape)
        x = self.mul(inputs, x)
        return x
...
_x = FakeData(size=240, batch_size=1, image_size=(120,), use_parallel=True)
