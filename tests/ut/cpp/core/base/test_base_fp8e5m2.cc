...
int main() {
  ...
  for (float f32 : test_values) {
    uint8_t float8_bits = Float8_e5m2::FromFloat32(f32);
    Float8_e5m2 float8 = Float8_e5m2::FromRaw(float8_bits);
    float f32_converted = Float8_e5m2::ToFloat32(float8);
    ...
    std::cout << "Float8_e5m2 hex: 0x" << std::hex << std::setw(2) << std::setfill('0')
              << static_cast<int>(float8_bits) << std::dec << std::endl;
    std::cout << "Float8_e5m2 bits: " << std::bitset<8>(float8_bits).to_string().substr(0,1)
              << " | " << std::bitset<8>(float8_bits).to_string().substr(1,5)
              << " | " << std::bitset<8>(float8_bits).to_string().substr(6,2) << std::endl;
    std::cout << "               (S |  E   | M )" << std::endl;
    ...
    const float epsilon = abs(f32)/4.0;
