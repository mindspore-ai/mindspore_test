...
/// Feature: Test SqrtGrad expander
/// Description: test op with different inputs
/// Expectation: Can be expanded only when its input data types are supported.
class TestSqrtGradExpander : public TestGraphKernelExpander, public testing::WithParamInterface<Params> {
  void SetUp() override { SetDeviceTarget(kAscendDevice); }
};

TEST_P(TestSqrtGradExpander, sqrt_grad) {
  ...
  auto op = c.NewCNodeWithBuildInfo("SqrtGrad", inputs);
  c.SetOutput(op);
  auto fg = c.GetGraph();
...
INSTANTIATE_TEST_CASE_P(TestOpSqrtGrad, TestSqrtGradExpander,
