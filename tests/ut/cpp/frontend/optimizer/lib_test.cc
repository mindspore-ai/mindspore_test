...
TEST_F(TestOptLib, test_inline) {
  ...
  tensor::TensorPtr x_tensor = tensor::from_spec(kFloat32->type_id(), std::vector<int64_t>{2, 3}, device::DeviceType::kCPU);
  tensor::TensorPtr y_tensor = tensor::from_spec(kFloat32->type_id(), std::vector<int64_t>{2, 3}, device::DeviceType::kCPU);
...
TEST_F(TestOptLib, DISABLED_test_elim_cast_same_dtype) {
  ...
  if (inputs.size() > 2) {
    ...
    tensor::TensorPtr x_tensor = tensor::from_spec(kFloat32->type_id(), shp, device::DeviceType::kCPU);
...
TEST_F(TestOptLib, DISABLED_test_reducesum_one) {
  ...
  tensor::TensorPtr x_tensor2 = tensor::from_spec(kFloat32->type_id(), shp2, device::DeviceType::kCPU);
