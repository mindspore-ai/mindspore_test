...
static KernelGraphPtr CreateKernelGraph() {
  ...
  tensor::TensorPtr y_tensor = tensor::from_spec(kFloat32->type_id(), shp, device::DeviceType::kCPU);
  y_tensor->set_device_info(device_info);
  tensor::TensorPtr z_tensor = tensor::from_spec(kFloat32->type_id(), shp, device::DeviceType::kCPU);
