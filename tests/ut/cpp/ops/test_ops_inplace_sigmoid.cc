...
std::vector<GeneralInferParam> prepare_params() {
  ...
  generator
    .FeedInputArgs({InferInfoParam{ShapeVector{0}, kNumberTypeFloat32}})
    .FeedExpectedOutput({{0}}, {kNumberTypeFloat32});
  
  generator
    .FeedInputArgs({InferInfoParam{ShapeVector{3, 4, 2}, kNumberTypeComplex64}})
    .FeedExpectedOutput({{3, 4, 2}}, {kNumberTypeComplex64});
  
  return generator.Generate();
}
}  // namespace

INSTANTIATE_TEST_CASE_P(InplaceSigmoid, GeneralInferTest, testing::ValuesIn(prepare_params()));
}  // namespace mindspore::ops