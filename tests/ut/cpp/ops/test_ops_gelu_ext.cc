...
std::vector<GeneralInferParam> prepare_params() {
  ...
  generator
    .FeedInputArgs({
      InferInfoParam{ShapeVector{1, 2, 3, 4, 5, 6, 7, 8}, kNumberTypeBFloat16},
      // groups
      InferInfoParam{ShapeVector{}, kNumberTypeInt64, CreateScalar<int64_t>(0)},
    })
    .FeedExpectedOutput({{1, 2, 3, 4, 5, 6, 7, 8}}, {kNumberTypeBFloat16});

  return generator.Generate();
}
}  // namespace

INSTANTIATE_TEST_CASE_P(GeluExt, GeneralInferTest, testing::ValuesIn(prepare_params()));
}  // namespace mindspore::ops
