...
std::vector<GeneralInferParam> prepare_params() {
  ...
  generator
    .FeedInputArgs({
      InferInfoParam{ShapeVector{3, 3}, kNumberTypeFloat64},
      InferInfoParam{ShapeVector{}, kNumberTypeInt64, kValueAny},
      InferInfoParam{ShapeVector{2, 3}, kNumberTypeFloat64},
      InferInfoParam{ShapeVector{3, 3}, kNumberTypeFloat64},
    })
    .FeedExpectedOutput({{3, 3}}, {kNumberTypeFloat64});
  return generator.Generate();
}
}  // namespace

INSTANTIATE_TEST_CASE_P(InplaceScatterAdd, GeneralInferTest, testing::ValuesIn(prepare_params()));
}  // namespace mindspore::ops
