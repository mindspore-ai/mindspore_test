...
#include "infer/ops_func_impl/tanh_grad.h"
...
TEST_P(TestTanhGrad, dyn_shape) {
  ...
  DoFuncImplInferAndCompare<TanhGradFuncImpl>("TanhGrad", {y, dy}, expect_shape, expect_type);
}

namespace {
auto TanhGradOpTypeCases = testing::ValuesIn({
...
INSTANTIATE_TEST_CASE_P(TestTanhGradGroup, TestTanhGrad,
                        testing::Combine(EltwiseGradDynShapeTestCases, TanhGradOpTypeCases));
}  // namespace ops
}  // namespace mindspore
