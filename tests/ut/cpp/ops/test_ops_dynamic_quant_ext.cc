...
#include "infer/ops_func_impl/dynamic_quant_ext.h"
...
class TestDynamicQuantExt : public TestOps,
                            public testing::WithParamInterface<std::tuple<DynamicQuantExtShape, DynamicQuantExtType>> {
};
...
TEST_P(TestDynamicQuantExt, dyn_shape) {
  ...
  auto expect_shape = std::make_shared<abstract::TupleShape>(
    std::vector<BaseShapePtr>{std::make_shared<abstract::Shape>(shape_param.x_shape),
...
auto dynamic_quant_ext_shape_cases =
  testing::Values(DynamicQuantExtShape{{5, 4}, {4}, {5}}, DynamicQuantExtShape{{16, 8, 3}, {3}, {16, 8}},
                  DynamicQuantExtShape{{64}, {64}, {}}, DynamicQuantExtShape{{-2}, {-2}, {-2}},
                  DynamicQuantExtShape{{-1, 2, 3, -1},
                                       {
                                         -1,
                                       },
                                       {-1, 2, 3}});
...
INSTANTIATE_TEST_CASE_P(TestDynamicQuantExtGroup, TestDynamicQuantExt,
                        testing::Combine(dynamic_quant_ext_shape_cases, dynamic_quant_ext_type_cases));
}  // namespace ops
}  // namespace mindspore
