...
class TestLstsqV2Grad : public TestOps,
                        public testing::WithParamInterface<std::tuple<LstsqV2GradShape, LstsqV2GradType>> {};
...
TEST_P(TestLstsqV2Grad, dyn_shape) {
  ...
  std::vector<TypePtr> types_list = {std::make_shared<TensorType>(type_param.ga_type),
                                     std::make_shared<TensorType>(type_param.gb_type)};
...
auto lstsqv2_grad_shape_cases =
  testing::Values(LstsqV2GradShape{{5, 6, 7}, {1, 6, 6}, {5, 6, 7}, {1, 6, 6}, {5, 6, 7}},
...
INSTANTIATE_TEST_CASE_P(TestOpsFuncImpl, TestLstsqV2Grad,
                        testing::Combine(lstsqv2_grad_shape_cases, lstsqv2_grad_type_cases));
