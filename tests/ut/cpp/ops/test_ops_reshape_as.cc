...
std::vector<GeneralInferParam> prepare_params() {
  ...
  generator
    .FeedInputArgs({InferInfoParam{ShapeVector{2, 3, 6}, kNumberTypeFloat32},
                    InferInfoParam{ShapeVector{6, 3, 2}, kNumberTypeFloat32}})
    .FeedExpectedOutput({{6, 3, 2}}, {kNumberTypeFloat32});
  generator
  ...
  generator
    .FeedInputArgs({InferInfoParam{ShapeVector{1, 1, 1, 1, 1, 1, 1, 8}, kNumberTypeFloat32},
                    InferInfoParam{ShapeVector{1, 1, 1, 1, 1, 2, 2, 2}, kNumberTypeFloat32}})
    .FeedExpectedOutput({{1, 1, 1, 1, 1, 2, 2, 2}}, {kNumberTypeFloat32});
  return generator.Generate();
}
}  // namespace

INSTANTIATE_TEST_CASE_P(ReshapeAs, GeneralInferTest, testing::ValuesIn(prepare_params()));
}  // namespace mindspore::ops
