...
std::vector <GeneralInferParam> prepare_params() {
  ...
  generator
      .FeedInputArgs({InferInfoParam{ShapeVector{2, -1}, kNumberTypeInt8}})
      .FeedExpectedOutput({{2, -1}}, {kNumberTypeInt8});
  generator
      .FeedInputArgs({InferInfoParam{ShapeVector{-1, -1}, kNumberTypeInt32}})
      .FeedExpectedOutput({{-1, -1}}, {kNumberTypeInt32});
  generator
      .FeedInputArgs({InferInfoParam{ShapeVector{-2}, kNumberTypeUInt8}})
      .FeedExpectedOutput({{-2}}, {kNumberTypeUInt8});
  return generator.Generate();
}
}

INSTANTIATE_TEST_CASE_P(Trunc, GeneralInferTest, testing::ValuesIn(prepare_params())
);
}  // namespace mindspore::ops