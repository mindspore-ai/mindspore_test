...
/// Feature: Test Square expander
/// Description: test op with different inputs
/// Expectation: Can be expanded only when its input data types are supported.
class TestSquareExpander : public TestGraphKernelExpander, public testing::WithParamInterface<Params> {
  void SetUp() override { SetDeviceTarget(kAscendDevice); }
};

TEST_P(TestSquareExpander, square) {
  const auto &param = GetParam();
  ConstructGraph c;
  auto x = c.NewTensorInput("x", param.input_type, {10, 20});
  auto op = c.NewCNodeWithBuildInfo("Square", {x});
  c.SetOutput(op);
  auto fg = c.GetGraph();
...
INSTANTIATE_TEST_CASE_P(TestOpSquare, TestSquareExpander,
                        testing::Values(Params{true, kFloat16}, Params{true, kFloat32}, Params{true, kBFloat16},
                                        Params{true, kInt32}, Params{false, kFloat64}, Params{false, kInt64},
                                        Params{false, kInt8}));
}  // namespace mindspore::graphkernel::test
