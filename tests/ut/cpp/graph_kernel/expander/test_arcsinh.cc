...
/// Feature: Test graph kernel AsinhExt expander
/// Description: AsinhExt will expanded
/// Expectation: After expand, the output shape and data type of sub graph should match expect
class TestAsinhExtExpander : public TestGraphKernelExpander, public testing::WithParamInterface<AsinhExtParams> {
  void SetUp() override {
...
TEST_P(TestAsinhExtExpander, AsinhExt) {
  const auto &param = GetParam();
  ConstructGraph c;
  auto shape = c.NewTensorInput("input_shape", param.type, param.input_shape);
  auto op = c.NewCNodeWithBuildInfo("AsinhExt", {shape}, {});
  c.SetOutput(op);
...
INSTANTIATE_TEST_CASE_P(TestOpAsinhExt, TestAsinhExtExpander,
                        testing::Values(AsinhExtParams{{16, 16}, {16, 16}, kFloat16},
                                        AsinhExtParams{{16, 16}, {16, 16}, kFloat32}));
}  // namespace mindspore::graphkernel::test