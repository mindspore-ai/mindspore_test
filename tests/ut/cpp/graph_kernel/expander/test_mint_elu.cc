...
/// Feature: Test graph kernel EluExt expander
/// Description: EluExt will expanded
/// Expectation: After expand, the output shape and data type of sub graph should match expect
class TestEluExtExpander : public TestGraphKernelExpander, public testing::WithParamInterface<EluExtParams> {
  void SetUp() override {
...
TEST_P(TestEluExtExpander, EluExt) {
  const auto &param = GetParam();
  ConstructGraph c;
  auto shape = c.NewTensorInput("input_shape", param.type, param.input_shape);
  auto alpha = c.NewScalarInput("alpha", MakeValue(param.alpha), kFloat32);
  auto op = c.NewCNodeWithBuildInfo("EluExt", {shape, alpha}, {});
  c.SetOutput(op);
