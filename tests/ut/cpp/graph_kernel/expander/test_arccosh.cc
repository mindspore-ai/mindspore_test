...
/// Feature: Test graph kernel AcoshExt expander
/// Description: AcoshExt will expanded
/// Expectation: After expand, the output shape and data type of sub graph should match expect
class TestAcoshExtExpander : public TestGraphKernelExpander, public testing::WithParamInterface<AcoshExtParams> {
  void SetUp() override {
...
TEST_P(TestAcoshExtExpander, AcoshExt) {
  const auto &param = GetParam();
  ConstructGraph c;
  auto shape = c.NewTensorInput("input_shape", param.type, param.input_shape);
  auto op = c.NewCNodeWithBuildInfo("AcoshExt", {shape}, {});
  c.SetOutput(op);
...
INSTANTIATE_TEST_CASE_P(TestOpAcoshExt, TestAcoshExtExpander,
                        testing::Values(AcoshExtParams{{16, 16}, {16, 16}, kFloat16},
                                        AcoshExtParams{{16, 16}, {16, 16}, kFloat32}));
}  // namespace mindspore::graphkernel::test