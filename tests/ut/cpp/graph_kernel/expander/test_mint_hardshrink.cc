...
/// Feature: Test graph kernel HShrink expander
/// Description: HShrink will expanded
/// Expectation: After expand, the output shape and data type of sub graph should match expect
class TestHShrinkExpander : public TestGraphKernelExpander, public testing::WithParamInterface<HShrinkParams> {
  void SetUp() override {
...
TEST_P(TestHShrinkExpander, HShrink) {
  const auto &param = GetParam();
  ConstructGraph c;
  auto shape = c.NewTensorInput("input_shape", param.type, param.input_shape);
  auto lambd = c.NewScalarInput("lambd", MakeValue(param.lambd), kFloat32);
  auto op = c.NewCNodeWithBuildInfo("HShrink", {shape, lambd}, {});
  c.SetOutput(op);
